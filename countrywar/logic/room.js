...
const { COUNTRYWAR_EventType } = require('../../common/error.js');
...
Room.prototype = {
    ...
    _init: function () {
        ...
        if (!(!this.cities || Object.keys(this.cities).length == 0)) {
            ...
            this.cities = {};
            // 初始化城池数据
            var cityCount = Object.keys(conf_mgr.gConfCountryWarCity).length;
            for (var i = 0; i < cityCount; i++) {
                var cityData = conf_mgr.gConfCountryWarCity[i + 1];
    ...
    generateRobots: function () {
        ...
        var maxId = 0;
        for (var i in conf_mgr.gConfName) {
            if (+i > maxId) maxId = +i;
        }

        var robotLevel = this.averageLevel;
        if (robotLevel == 0) {
            robotLevel = parseInt(conf_mgr.gConfCountryWarBase.guardDefaultLevel.value);
        }

        var robotFightForce = conf_mgr.gConfCountryWarGuard[robotLevel].fightPower;
        ...
        for (var cityId in this.cities) {
            ...
            var armyNum = conf_mgr.gConfCountryWarCity[cityId].armyNum;
            var armyForce = Math.floor(robotFightForce * (1 + conf_mgr.gConfCountryWarCity[cityId].armyFightforce));
            for (var i = 1; i <= armyNum; i++) {
                ...
                var name = conf_mgr.gConfName[firstNameId].first + conf_mgr.gConfName[secondNameId][male];
    ...
    resetTime: function () {
        var strBegin = common.GLOBAL_SERVER_INFO_DICT.countryWarBeginTime.split(":");
        if (strBegin.length != 2) {
            return;
        }

        var strEnd = common.GLOBAL_SERVER_INFO_DICT.countryWarEndTime.split(":");
        if (strEnd.length != 2) {
            return;
        }
    ...
    onStart: function () {
        // 重置玩家国战数据
        var openDays = conf_mgr.gConfCountryWarBase.openDays;
    ...
    initCountryTask: function () {
        ...
        for (var j = 1; j <= 3; j++) {
            this.tasks[j] = {};
            var taskTypeCount = Object.keys(conf_mgr.gConfCountryWarTask).length;
            for (var i = 1; i <= taskTypeCount; i++) {
                var taskArr = [];
                var taskIdCount = Object.keys(conf_mgr.gConfCountryWarTask[i]).length;
                for (var k = 1; k <= taskIdCount; k++) {
                    var taskData = conf_mgr.gConfCountryWarTask[i][k];
    ...
    processCountryTask: function () {
        for (var i = 1; i <= 3; i++) {
            for (var taskType in this.tasks[i]) {
                ...
                if (!taskObj.finish) {
                    var taskData = conf_mgr.gConfCountryWarTask[taskType][taskObj.taskId];
                    if (taskData) {
                        var needSave = false;
    ...
    onCityOwnerChanged: function (city_id, hold_country) {
        ...
        this.addEvent(city_id, COUNTRYWAR_EventType.EVENT_OCCUPY_CITY, hold_country);
        ...
        if (curTime - countryWarBeginTime < parseInt(conf_mgr.gConfCountryWarBase.countTime.value)) {
    ...
    calcCountryScore: function () {
        ...
        var curTime = common.getTime();
        if (this.scoreCalcTime + parseInt(conf_mgr.gConfCountryWarBase.cityScoreTime.value) > curTime) {
            return;
        }
        ...
        var count = Math.floor(timeDiff / parseInt(conf_mgr.gConfCountryWarBase.cityScoreTime.value));
        for (var cityId in this.cities) {
            var score = parseInt(conf_mgr.gConfCountryWarCity[cityId].cityScore) * count;
            if (score > 0) {
        ...
        this.scoreCalcTime += (count * parseInt(conf_mgr.gConfCountryWarBase.cityScoreTime.value));
        this.markDirty('scoreCalcTime');
    ...
    onReachCity: function (uid, targetCity, resp) {
        ...
        var reachTime = moveObj.startTime + index * parseInt(conf_mgr.gConfCountryWarBase.pathTime.value);
        var curTime = common.getTime();
    ...
    processCall: function () {
        ...
        for (var i = 1; i <= 3; i++) {
            for (var cityId in this.callOfDuty[i]) {
                if (curTime > this.callOfDuty[i][cityId].time + parseInt(conf_mgr.gConfCountryWarBase.callKeepTime.value)) {
                    // 已经到时间了
    ...
    sendPersonalRankMail: function () {
        ...
        var mail = {
            ...
            time: time,
            expire: time + conf_mgr.gConfMail[31].time * OneDayTime,
            attach: {
                ranks: ranks,
                huge: 1,
            },
    ...
    sendWeekRankMail: function () {
        ...
        var mail = {
            ...
            time: time,
            expire: time + conf_mgr.gConfMail[32].time * OneDayTime,
            attach: {
                ranks: playerRanks,
                campRanks: campRank,
                huge: 1,
    ...
    syncPlayerMove: function (uid, from, to) {
        ...
        if (!this.moveSync[key] || (this.moveSync[key] && this.moveSync[key].deleteTime < curTime)) {
            ...
            this.moveSync[key].deleteTime = curTime + parseInt(conf_mgr.gConfCountryWarBase.pathTime.value);
    ...
    insertChatLog: function (chatInfo) {
        ...
        if (this.chatLog.length > parseInt(conf_mgr.gConfCountryWarBase.chatLogCountMax.value)) {
            this.chatLog.pop();
        }
    },
    ...
    insertShoutLog: function (chatInfo) {
        ...
        if (this.shoutLog.length > parseInt(conf_mgr.gConfCountryWarBase.chatLogCountMax.value)) {
            this.shoutLog.pop();
        }
    },
    ...
    addEvent: function (cityId, event_id, param1, param2, param3, param4, param5, param6) {
        ...
        var maxCount = 50;//parseInt(conf_mgr.gConfTerritoryWarBase.eventRecordNum.value);
    ...
    getCityCampPlayerList: function (cityId, camp) {
        ...
        for (var i = 0; i < this.cities[cityId].players[camp].length; i++) {
            ...
            if (user) {
                ...
                var positionInfo = conf_mgr.gConfPosition[user.info.position];
                ...
                if (robot) {
                    ...
                    var positionInfo = conf_mgr.gConfPosition[32];
                    playerObj.score = parseInt(positionInfo.gloryRatio);
