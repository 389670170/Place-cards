...
UserInfo.prototype = {
    ...
    getUserFightInfo: function (uid, notUseDefPos, useCountryWarTeam, cityId) {
        var cityInfo = conf_mgr.gConfCountryWarCity[cityId];
        var info = {};
    ...
    initUserCountryWarData: function (uid, roomId, camp, serverId, initCity) {
        ...
        for (var slot in this.users[uid].pos) {
            ...
            if (conf_mgr.gConfHero[hid]) {
                var legionType = conf_mgr.gConfHero[hid].legionType;
                var soldierLevel = this.users[uid].pos[slot].soldier.level;
                var soldierStar = this.users[uid].pos[slot].soldier.star;
                var soldierNum = conf_mgr.gConfSoldierLevel[legionType][soldierLevel][soldierStar].num;
    ...
    update: function (uid, updates, roomId, camp, serverId) {
        ...
        if (!userInfo) {
            ...
            awards = [['user', 'goods', parseInt(conf_mgr.gConfCountryWarBase.baseGoods.value)]];
    ...
    onKillPlayer: function (killer_uid, dead_uid, dead_country) {
        ...
        if (this.users[killer_uid]) {
            ...
            if (this.users[dead_uid]) {
                positionInfo = conf_mgr.gConfPosition[this.users[dead_uid].info.position];
            } else {
                // 机器人按最低官职算
                positionInfo = conf_mgr.gConfPosition[32];
            }
            ...
            if (!isDroid(dead_uid)) {
                ...
                if (continuousKillCount > Object.keys(conf_mgr.gConfCountryWarKillScore).length) {
                    continuousKillCount = Object.keys(conf_mgr.gConfCountryWarKillScore).length;
                }
                score[0] += conf_mgr.gConfCountryWarKillScore[continuousKillCount].additionalGlory;
            }
        ...
        if (this.users[dead_uid]) {
            ...
            score[1] = conf_mgr.gConfCountryWarBase.minimumGlory.value;
            ...
            var positionInfo = conf_mgr.gConfPosition[this.users[dead_uid].info.position];
            if (positionInfo && positionInfo.pisition <= parseInt(conf_mgr.gConfCountryWarBase.callNeedPosition.value)) {
                // 击杀的是官阶一品及以上
    ...
    isPlayerDead: function (uid) {
        ...
        if (this.users[uid].countryWar.deathTime + parseInt(conf_mgr.gConfCountryWarBase.reliveTime.value) > common.getTime())
            return true;
    ...
    getShortestPath: function (uid, start_city, end_city) {
        ...
        while (visitQueue.length > 0) {
            ...
            if (!conf_mgr.gConfCountryWarCity[top]) {
                var a = 0;
            }

            // 将top节点未访问过的相邻节点放入队列
            var neighbors = conf_mgr.gConfCountryWarCity[top].adjoin;
    ...
    getPathTime: function (path) {
        return (path.length - 1) * parseInt(conf_mgr.gConfCountryWarBase.pathTime.value); // 暂时先定每两座城池之间需要2秒
    },
    ...
    canReachTargetCity: function (uid, target_city) {
        ...
        if (targetCityInfo.hold_camp == userCountry) {
            ...
            var neighborCities = conf_mgr.gConfCountryWarCity[target_city].adjoin;
    ...
    isTaskFinish: function (uid, task_id) {
        ...
        var taskData = conf_mgr.gConfCountryWarTaskPersonal[task_id];
        if (!taskData) {
            return false;
        }
    ...
    hasPersonalTaskAwards: function (uid) {
        ...
        for (var id in conf_mgr.gConfCountryWarTaskPersonal) {
            var taskInfo = conf_mgr.gConfCountryWarTaskPersonal[id];
            if (taskInfo) {
    ...
    hasCountryTaskAwards: function (uid, serverId) {
        for (var type in conf_mgr.gConfCountryWarTask) {
            var taskList = conf_mgr.gConfCountryWarTask[type];
            if (taskList) {
