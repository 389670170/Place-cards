...
const { COUNTRYWAR_ErrorCode, COUNTRYWAR_EventType } = require('../../common/error.js');
...
CountryWar.prototype = {
    ...
    buildServerRoomTab: function () {
        if (!this.serverRoomTab || !this.serverCampTable ||
            ...
            for (var groupId in conf_mgr.gConfCountryRoom) {
                for (var i = 1; i <= 3; i++) {
                    var campList = conf_mgr.gConfCountryRoom[groupId]['camp' + i];
    ...
    resetTime: function () {
        var strBegin = conf_mgr.gConfCountryWarBase.openTime.value[0].split(':');
        if (strBegin.length != 2) {
            return;
        }

        var strEnd = conf_mgr.gConfCountryWarBase.openTime.value[1].split(':');
        if (strEnd.length != 2) {
            return;
        }
        ...
        var openDay = weekDay;
        var diffDay = 0;
        for (var i = 0; i < conf_mgr.gConfCountryWarBase.openDays.value.length; i++) {
            var find = false;
            for (var j = 0; j < conf_mgr.gConfCountryWarBase.openDays.value.length; j++) {
                if (openDay == parseInt(conf_mgr.gConfCountryWarBase.openDays.value[j])) {
                    find = true;
                    break;
                }
            }
        ...
        this.fightBeginTime = this.beginTime + parseInt(conf_mgr.gConfCountryWarBase.readyTime.value);
    },
    ...
    getServerState: function (sid) {
        var curTime = common.getTime();
        if (this.servers[sid]) {
            var days = parseInt(conf_mgr.gConfCountryWarBase.serverOpenDayLimit.value);
...
exports.get = function (req, res, resp, onReqHandled) {
    do {
        ...
        if (!room) {
            // 房间还没开
            resp.code = COUNTRYWAR_ErrorCode.ERROR_ROOM_NOT_OPEN; resp.desc = 'roomId not open'; break;
        }

        if (!room.isRoomOpen()) {
            // 房间还没开
            resp.code = COUNTRYWAR_ErrorCode.ERROR_ROOM_NOT_OPEN; resp.desc = 'roomId is not open'; break;
        }
...
// 获取国战开启时间
exports.get_open_time = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var serverId = req.args.serverId;
        var roomId = gCountryWar.getRoomIdByServerId(serverId);

        var serverList = conf_mgr.gConfCountryRoom[roomId];
...
// 获取指定城池信息
exports.get_city_info = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_CAN_NOT_FIND_USER; resp.desc = 'user not found'; break;
        }
...
// 前往指定城池
exports.move_to_city = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        ...
        if (gUserInfo.isPlayerDead(uid)) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_YOU_ARE_DEAD; resp.desc = 'you are dead'; break;
        }

        // 检查目标城池是否可达
        if (!gUserInfo.canReachTargetCity(uid, targetCity)) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_CAN_NOT_REACH; resp.desc = 'can not reach target city'; break;
        }

        // 检测能否进入
        if (!gUserInfo.canEnterTargetCity(uid, targetCity)) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_CAN_NOT_ENTER; resp.desc = 'can not enter target city'; break;
        }
        ...
        if (targetCityInfo && targetCityInfo.hold_camp != userInfo.countryWar.camp && !gCountryWar.isFightTime()) {
            // 准备阶段不能进入地方城池
            resp.code = COUNTRYWAR_ErrorCode.ERROR_READY_TIME; resp.desc = 'ready time can not enter enemy city'; break;
        }
        ...
        if (path.length == 0) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_CAN_NOT_FIND_PATH; resp.desc = 'path empty'; break;
        }
        ...
        if (reachTime > gCountryWar.getEndTime()) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_MOVE_TIME_NOT_ENOUGH; resp.desc = 'left time not enough'; break;
        }

        // 检查当前城池是否处于交战状态，处于交战状态的城池不能直接离开
        var cityInfo = gCountryWar.getCityInfo(roomId, curCity);
        if (cityInfo.fight_state) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_CITY_IN_FIGHT; resp.desc = 'cur city in fight'; break;
        }
...
exports.special_move = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var targetCity = +req.args.target_city;
        var userInfo = gUserInfo.getUser(uid);
        ...
        if (!cities) {
            resp.code = resp.code = COUNTRYWAR_ErrorCode.ERROR_IN_PROTECT_TIME; resp.desc = 'cities not found'; break;
        }
        ...
        if (attackCount < defenseCount * 2) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_CAN_NOT_SPECIAL_MOVE; resp.desc = 'attackCount not enough'; break;
        }

        // 检查突进目标城池与当前城池是否相邻
        var isNeighbor = false;
        var cityConf = conf_mgr.gConfCountryWarCity[userCity];
        ...
        if (!isNeighbor) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_TARGET_CITY_IS_NOT_NEIGHBOR; resp.desc = 'not neighbor city'; break;
        }

        var path = [userCity, targetCity];
        var curTime = common.getTime();
        var reachTime = curTime + parseInt(conf_mgr.gConfCountryWarBase.pathTime.value);
...
// 到达
exports.reach_city = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        ...
        if (!gUserInfo.canEnterTargetCity(uid, target_city)) {
            if (moveInfo && moveInfo.path.length >= 2) {
                ...
                resp.data.time = reachTime;
                resp.data.startTime = curTime;
                resp.code = COUNTRYWAR_ErrorCode.ERROR_CAN_NOT_ENTER;
...
// 发布召集令
exports.broadcast_call_of_duty = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var targetCity = +req.args.target_city;
        if (isNaN(targetCity)) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_CITY_NOT_EXIST; resp.desc = 'args target_city invalid'; break;
        }
        ...
        if (!gCountryWar.isFightTime()) {
            // 准备阶段不能发布号令
            resp.code = COUNTRYWAR_ErrorCode.ERROR_READY_TIME; resp.desc = 'ready time can not broadcast calls'; break;
        }
        ...
        // 检查玩家官职
        var needPosition = parseInt(conf_mgr.gConfCountryWarBase.callNeedPosition.value);
        if (userInfo.info.position > needPosition) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_COUNTRY_POSITION_NOT_ENOUGH; resp.desc = 'position not enough'; break;
        }
        ...
        if (cityInfo.players[camp].length == 0) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_NO_SELF_SIDE_PLAYER; resp.desc = 'self side player count is 0'; break;
        }

        // 判断集结令数量是否达到上限
        var callOfDuty = gCountryWar.getCallOfDuty(roomId, camp);
        var curCallCount = Object.keys(callOfDuty).length;
        if (curCallCount >= parseInt(conf_mgr.gConfCountryWarBase.callNumLimit.value)) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_CALL_COUNT_MAX; resp.desc = 'max count'; break;
        }

        // 检查目标城池是否已经有集结令
        if (callOfDuty[targetCity] && callOfDuty[targetCity].time + parseInt(conf_mgr.gConfCountryWarBase.callKeepTime.value) > common.getTime()) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_TARGET_CITY_EXIST_CALL; resp.desc = 'exist one'; break;
        }

        gCountryWar.sendCall(serverId, roomId, uid, targetCity);
        resp.data.awards = [['user', 'country_score', parseInt(conf_mgr.gConfCountryWarBase.callScoreAward.value)]];
        resp.data.count = gUserInfo.getBroadcastCallCount(uid);
        resp.data.callList = gCountryWar.getCallOfDuty(roomId, camp);

        gCountryWar.addEvent(roomId, targetCity, COUNTRYWAR_EventType.EVENT_BROADCAST_CALL, serverId, userInfo.info.un, camp, targetCity);
    } while (false);

    onReqHandled(res, resp, 1);
};

exports.get_broadcast_call_count = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
...
// 响应集结令
exports.respond_call_of_duty = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var targetCity = +req.args.target_city;
        if (isNaN(targetCity)) {
        ...
        if (!gCountryWar.isFightTime()) {
            // 准备阶段不能发布号令
            resp.code = COUNTRYWAR_ErrorCode.ERROR_READY_TIME; resp.desc = 'ready time can not reply calls'; break;
        }
        ...
        //    resp.code = COUNTRYWAR_ErrorCode.ERROR_CITY_IS_IN_FIGHT; resp.desc = 'fight state'; break;
        //}
        ...
        if (!calls[targetCity]) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_CALL_NOT_EXIST; resp.desc = 'call not exist'; break;
        }

        // 检查是否已经失效
        if (calls[targetCity].time > common.getTime() + parseInt(conf_mgr.gConfCountryWarBase.callKeepTime.value)) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_CALL_OUT_OF_TIME; resp.desc = 'call time out'; break;
        }

        // 检查是否已经响应过此集结令
        //if (calls[targetCity].replys.indexOf(uid) >= 0) {
        //    resp.code = COUNTRYWAR_ErrorCode.ERROR_ALREADY_REPLY_CALL; resp.desc = 'already reply this call'; break;
        //}
...
// 随机匹配对手
exports.match_enemy = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        ...
        if (!gCountryWar.isFightTime()) {
            // 准备阶段不能匹配敌人
            resp.code = COUNTRYWAR_ErrorCode.ERROR_READY_TIME; resp.desc = 'ready time can not match enemy'; break;
        }
        ...
        if (gUserInfo.isPlayerInCoolTime(uid)) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_IS_IN_COOL_TIME; resp.desc = 'in cool time'; break;
        }
        ...
        if (count == 0) {
            ...
            resp.code = COUNTRYWAR_ErrorCode.ERROR_NO_MATCH_PLAYER; resp.desc = 'no idle player'; break;
        }
        ...
        if (!enemy) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_NO_MATCH_PLAYER; resp.desc = 'match_enemy no enemy data'; break;
        }
...
// 单挑，匹配杀人数最高的
exports.solo_fight = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var targetCountry = req.args.target;
        if (isNaN(targetCountry)) {
        ...
        if (!enemy) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_NO_MATCH_PLAYER; resp.desc = 'solo_fight no enemy data'; break;
        }
...
// 死亡回城
exports.death = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        var userCity = userInfo.countryWar.city;
...
// 撤军回城
exports.back_to_city = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
        ...
        gCountryWar.addEvent(roomId, userCity, COUNTRYWAR_EventType.EVENT_BACK_TO_CITY, serverId, userInfo.info.un, userInfo.countryWar.camp);
    } while (false);
...
// 快速复活
exports.fast_relive = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
        ...
        if (userInfo.countryWar.deathTime == 0 ||
            (userInfo.countryWar.deathTime + parseInt(conf_mgr.gConfCountryWarBase.reliveTime.value) < common.getTime())) {
            // 没有死，或者已经自动复活了
...
exports.fight = function (req, res, resp, onReqHandled) {
    do {
        ...
        if (!enemyInfo) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_CAN_NOT_FIND_USER; resp.desc = 'enemy user not found'; break;
        }
        ...
        var curTime = common.getTime();
        var coolTime = curTime + parseInt(conf_mgr.gConfCountryWarBase.battleDiffTime.value);
...
// 领取任务奖励
exports.get_personal_task_award = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        ...
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_CAN_NOT_FIND_USER; resp.desc = 'user not exist'; break;
        }

        if (gUserInfo.isPersonalTaskAwardGot(uid, taskId)) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_HAS_GOT_TASK_AWARD; resp.desc = 'has got'; break;
        }

        if (!gUserInfo.isTaskFinish(uid, taskId)) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_TASK_NOT_FINISH; resp.desc = 'task not finish'; break;
        }

        var taskData = conf_mgr.gConfCountryWarTaskPersonal[taskId];
        if (!taskData) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_TASK_CONFIG_ERROR; resp.desc = 'task data not found'; break;
        }
...
// 领取国家任务奖励
exports.get_country_task_award = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        ...
        var taskData = conf_mgr.gConfCountryWarTask[task_type][task_id];
        if (!taskData) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_TASK_CONFIG_ERROR; resp.desc = 'task data not found'; break;
        }
...
// 获取国家任务列表
exports.get_country_task = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
...
// 获取排行榜信息
exports.get_rank_list = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
...
// 获取国家排行榜信息
exports.get_country_rank_list = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var serverId = req.args.serverId;
        var roomId = gCountryWar.getRoomIdByServerId(serverId);
 ...
 */
exports.get_call_list = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_CAN_NOT_FIND_USER; resp.desc = 'can not find user'; break;
        }
        ...
        for (var cityId in callList) {
            if (curTime >= callList[cityId].time + parseInt(conf_mgr.gConfCountryWarBase.callKeepTime.value)) {
                continue;
            }
...
// 获取个人积分，国家积分
exports.get_score = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_CAN_NOT_FIND_USER; resp.desc = 'user not fount!'; break;
        }
...
// 获取城池战斗信息
exports.get_city_battle_info = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
        ...
        var cityInfo = gCountryWar.getCityInfo(roomId, targetCity, true);
        if (!cityInfo) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_CITY_NOT_EXIST; resp.desc = 'can not find city info'; break;
        }
...
// 设置防守阵型
exports.set_team = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
...
// 获取阵型
exports.get_team = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
...
// 增加积分
exports.add_score = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
...
// 聊天
exports.chat = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
        ...
        if (content && content.length > conf_mgr.gConfGlobal.chatContentMaxLength) {
...
exports.shout = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
...
// 获取聊天记录
exports.get_chat_log = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
...
exports.get_shout_log = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
...
// 移除战斗冷却
exports.clear_fight_cool_time = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
...
// 获取指定城池指定阵营的玩家列表
exports.get_city_camp_players = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
