...
const { OneDayTime } = require('./enum.js');
...
exports.cloneHeroInitAttr = function () {
    var initAttr = {};
    for (var attr in Attribute) {
...
function initWorldUserGuard(argument) {
    ...
    for (var id in conf_mgr.gConfGuardField) {
...
global.generateDrop = function (id, level, useSeed) {
    ...
    var dropConf = conf_mgr.gConfDrop[id];
    if (!dropConf) {
        return [];
    }
...
global.isActivityStart_no_player = function (activity, time) {
    do {
        ...
        var activityConf = conf_mgr.gConfActivities[activity];
        if (!activityConf) {
        ...
        if (activityConf.openDay) {
            ...
            var resetTime = conf_mgr.gConfGlobal.resetHour * 3600;
            ...
            if (activityConf.endTime && common.GLOBAL_SERVER_INFO_DICT.serverStartTime > activityConf.endTime) {
                return ActivityProcess.CLOSE;
            }
        }
        ...
        var endTime = 0;
        if (type == 0) { // 开服活动
            // var serverOpenDayTime = common.getTime(getGameDate(common.GLOBAL_SERVER_INFO_DICT.serverStartTime)) + gConfGlobalNew.resetHour * 3600;
            var serverOpenDayTime = common.getTime(getGameDate(common.GLOBAL_SERVER_INFO_DICT.serverStartTime)) + conf_mgr.gConfGlobal.resetHour * 3600;
...
global.getGameDate = function (time) {
    if (time) {
        ...
        return common.getDate(time - conf_mgr.gConfGlobal.resetHour * 3600);
    } else {
        // return common.getDate(common.getTime() - gConfGlobalNew.resetHour * 3600);
        return common.getDate(common.getTime() - conf_mgr.gConfGlobal.resetHour * 3600);
    }
};
...
global.isActivityStart = function (player, activity, time) {
    do {
        ...
        if (type == 0) {                                    // 开服活动
            ...
            // var openTime = common.getTime(player.user.activity[activity].open_day) + conf_mgr.gConfGlobal.resetHour * 3600;
            // endTime = openTime + (activityConf.openDay + activityConf.duration) * 86400;
            return ActivityProcess.CLOSE;
        } else if (type == 3) {                             // 首登活动
            var openTime = common.getTime(getGameDate(player.user.info.create)) + activityConf.openLoginDay * 86400 + conf_mgr.gConfGlobal.resetHour * 3600;
...
global.getGameDay = function (time) {
    ...
    if (time) {
        ...
        day = new Date(time * 1000 - conf_mgr.gConfGlobal.resetHour * 3600 * 1000).getDay();
    } else {
        // day = new Date(new Date() - gConfGlobalNew.resetHour * 3600 * 1000).getDay();
        day = new Date(new Date() - conf_mgr.gConfGlobal.resetHour * 3600 * 1000).getDay();
    }
...
global.getActivityOpenDay = function (key) {
    ...
    var conf = conf_mgr.gConfActivities[key];
    if (conf.type == 0) {
        var date1 = getGameDate(common.GLOBAL_SERVER_INFO_DICT.serverStartTime);
...
global.getResetTime = function (time) {
    ...
    return common.getTime(getGameDate(time)) + conf_mgr.gConfGlobal.resetHour * 3600;
};
...
global.getWeekResetTime = function (time) {
    ...
    return beginOfToday - (weekDay - 1) * OneDayTime + conf_mgr.gConfGlobal.resetHour * 3600;
};
...
global.getMonthResetTime = function () {
    ...
    return beginOfToday - (monthDay - 1) * OneDayTime + conf_mgr.gConfGlobal.resetHour * 3600;
};
    ...
    for (var attr in attrs) {
        if (+attrs[attr] > 0) {
            if (log) {
                ...
                DEBUG("=========attr:" + attr + "=" + attrs[attr] + "*" + conf_mgr.gConfAttribute[attr].factor + "*" + confExtraFF[conf_mgr.gConfAttribute[attr].aPrKey] + '=' + (+attrs[attr]) * conf_mgr.gConfAttribute[attr].factor * confExtraFF[conf_mgr.gConfAttribute[attr].aPrKey]);
            }

            // fightForce += (+attrs[attr]) * gConfAttribute[attr].factor * confExtraFF[gConfAttribute[attr].aPrKey];
            fightForce += (+attrs[attr]) * conf_mgr.gConfAttribute[attr].factor * confExtraFF[conf_mgr.gConfAttribute[attr].aPrKey];
        }
    }
...
/** 计算英雄的真实战力 */
exports.calcFightForceNew = function (hero, attrs) {
    var roleLevel = hero.level;
    var awakeLevel = hero.awake;
    var tier = hero.tier;
    var star_lv = conf_mgr.getHeroStar(hero);
    var combatConf = conf_mgr.getHeroCombatConf(hero.rid, hero.awake);

    var fightForce = 0
    // var attrPConf = gConfAttPrCoEff[roleLevel];
    var attrPConf = conf_mgr.gConfAttPrCoEff[roleLevel];

    //ERROR('----------------------');
    for (var attr in attrs) {
        var attrValue = +attrs[attr];
        // var key = gConfAttribute[attr]['aPrKey'];
        var key = conf_mgr.gConfAttribute[attr]['aPrKey'];
        // var factor = gConfAttribute[attr]['factor'];
        var factor = conf_mgr.gConfAttribute[attr]['factor'];
    ...
    getSkillAddFightForce = function (awakeLevel, roleLevel, starLv) {
        ...
        var destinyConf = conf_mgr.gConfDestiny[awakeLevel];
        // var attrPConf = gConfAttPrCoEff[roleLevel];
        var attrPConf = conf_mgr.gConfAttPrCoEff[roleLevel];
    ...
    getSpecialInnateAddFightForce = function (combatConf, tier) {
        ...
        var maxNum = Math.min(+conf_mgr.gConfGlobal['innateNum' + baseStarLv], tier);

        for (var i = 1; i <= maxNum; i++) {
            var innateId = innateGroupCfg['level' + i];
            // var targetType = gConfInnate[innateId]['target'];
            var targetType = conf_mgr.gConfInnate[innateId]['target'];

            if (targetType == 0) {
                if (innateId == 1001) {
                    // addPower = addPower + (+gConfGlobalNew['innateSpecialPower1']);
                    addPower = addPower + (+conf_mgr.gConfGlobal['innateSpecialPower1']);
                } else if (innateId == 1002) {
                    // addPower = addPower + (+gConfGlobalNew['innateSpecialPower2']);
                    addPower = addPower + (+conf_mgr.gConfGlobal['innateSpecialPower2']);
                }
            }
        }
    ...
    var skillPower = getSkillAddFightForce(awakeLevel, roleLevel, star_lv);
...
global.calcFightForceNoExtra = function (attrs, confExtraFF) {
    ...
    for (var attr in attrs) {
        ...
        if (+attrs[attr] && conf_mgr.gConfAttribute[attr].influenceRobot) {
            // fightForce += (+attrs[attr]) * gConfAttribute[attr].factor * confExtraFF[gConfAttribute[attr].aPrKey];
            fightForce += (+attrs[attr]) * conf_mgr.gConfAttribute[attr].factor * confExtraFF[conf_mgr.gConfAttribute[attr].aPrKey];
        }
    }
...
exports.getLoginData = function (user) {
    var data = {};
    for (var id in user) {
        if (!(id in gClientLoginData)) { continue; }
        data[id] = user[id];
...
exports.reformAwards = reformAwards;
...
global.getMercenaryTimeGold = function (now, mInfo, hid, level) {
    ...
    if (duration > 0) {
        ...
        var awardCount = Math.floor(duration / conf_mgr.gConfGlobal.mercenaryTimeGoldInterval / 60);
        // timeGold += awardCount * gConfLevel[level].mercenaryTimeGold;
        timeGold += awardCount * conf_mgr.gConfLevel[level].mercenaryTimeGold;
    }
    return timeGold;
};
...
global.generateRobot = function (id, level, fightForce, userMaxFightForce) {
    ...
    var robotTypeConf = conf_mgr.gConfRobotType[id];
    if (!robotTypeConf) {
        return null;
    }

    // var robotAttConf = gConfRobotAtt[level];
    var robotAttConf = conf_mgr.gConfRobotAtt[level];
    if (!robotAttConf) {
        return null;
    }
    ...
    //     var hidArr = clone(conf_mgr.gRobotHeroIds);

    //     // 先把品质超出的剔除
    //     if (maxQuality && maxQuality > 0) {
    //         for (var i = 0; i < hidArr.length; i++) {
    //             var heroConf = conf_mgr.getHeroCombatConf(hidArr[i]);
    ...
    function randArrayWithNum(arr, num) {
        ...
        for (var i = 0, max = Math.min(myarr.length, num); i < max; i) {
            ...
            var tHeroBaseInfo = conf_mgr.gConfHero[value];
            if (!tHeroBaseInfo) {
                continue;
            }
    ...
    for (var i = 0; i < heroCount; i++) {
        ...
        var leftff = heroff - calcFightForce(attrs, conf_mgr.gConfAttPrCoEff[1]);

        var rid = hids[i];
        var combatConf = conf_mgr.getHeroCombatConf(rid);
        // var confDestiny = gConfDestiny[robotAttConf.skillLevel];
        var confDestiny = conf_mgr.gConfDestiny[robotAttConf.skillLevel];
        var skillff = 0;
        if (confDestiny) {
            // var confExtaFF = gConfAttPrCoEff[level];
            var confExtaFF = conf_mgr.gConfAttPrCoEff[level];
            var tDestinyValue = 0;
        ...
        for (var attr in weights) {
            ...
            if (conf_mgr.gConfAttribute[attr].factor) {
                // attrs[attr] = Math.floor(robotTypeConf.diff * attff / (gConfAttribute[attr].factor * gConfAttPrCoEff[1]['aPrC' + attr]));
                attrs[attr] = Math.floor(robotTypeConf.diff * attff / (conf_mgr.gConfAttribute[attr].factor * conf_mgr.gConfAttPrCoEff[1]['aPrC' + attr]));
                if (!attrs[attr]) {
        ...
        heroff = calcFightForce(attrs, conf_mgr.gConfAttPrCoEff[1]) + (skillff || 1000);// by fish FAQ
        posObj[i + 1] = {
            rid: parseInt(hids[i]),
            slot: slots[i],
            level: level,
...
global.randomPosObj = function (confRobot, heroCount, fightForce, soldierLevel, soldierStar, maxQuality) {
    ...
    var robotHeroIds = clone(conf_mgr.gRobotHeroIds);
    if (maxQuality && maxQuality > 0) {
        for (var i = 0; i < robotHeroIds.length; i++) {
            var heroConf = conf_mgr.getHeroCombatConf(robotHeroIds[i]);
            if (heroConf && heroConf.quality > maxQuality) {
    ...
    for (var i = 0; i < heroCount; i++) {
        ...
        var leftff = heroff - calcFightForce(attrs, conf_mgr.gConfAttPrCoEff[1], 1);
        ...
        for (var attr in weights) {
            ...
            if (conf_mgr.gConfAttribute[attr].factor) {
                ...
                attrs[attr] = Math.floor(attff / (conf_mgr.gConfAttribute[attr].factor * conf_mgr.gConfAttPrCoEff[1]['aPrC' + attr]));
            }
        }

        // heroff = calcFightForce(attrs, gConfAttPrCoEff[1], 1);
        heroff = calcFightForce(attrs, conf_mgr.gConfAttPrCoEff[1], 1);
        posObj[i + 1] = {
            hid: hids[i],
            slot: slots[i],
            level: 1,
...
global.isModuleOpen = function (player, name) {
    ...
    var conf = conf_mgr.gConfModuleOpen[name];
    if (!conf) {
        return false;
    }
...
exports.isModuleOpen_new = function (player, name) {
    // var conf = gConfModuleOpen_new[name];
    var conf = conf_mgr.gConfModuleOpen_new[name];
    if (!conf) {
        return false;
    }
...
global.isModulePrivilegeOpen = function (player, name) {
    ...
    var tavernHotPrivilegeId = conf_mgr.gConfNobiltyTitleKey[name].id;
...
global.generateEquip = function (equipArr, level) {
    ...
    if (level > conf_mgr.gMaxUserLevel - 10) {
        nextLevel = level;
    }
    var selectedEquips = [];
    // var levelEquipConf = gConfEquipClassified[equipArr[2] == 0 ? level : nextLevel];
    var levelEquipConf = conf_mgr.gConfEquipClassified[equipArr[2] == 0 ? level : nextLevel];
    if (!levelEquipConf) {
        return [];
    }
...
// 解析一次道具组奖励
global.parseGroupAwards = function (award) {
    var awards = [];
    if (award[0] != 'group') {
        return awards;
    }
...
// 解析奖励/消耗配置
exports.parseAwardsConfig = function (str) {
    var awards = [];
...
// 解析奖励/消耗配置
exports.parseAwardsConfigByCount = function (str, count) {
    var awards = [];
    if (!count) {
        return awards;
    }
...
global.calcMagicRate = function (vip_level, long_card, month_card, week_card) {
    ...
    for (var id in conf_mgr.gConfexploreMagiConvert) {
        // var data = gConfexploreMagiConvert[id];
        var data = conf_mgr.gConfexploreMagiConvert[id];
    ...
    return conf_mgr.gConfExploreBase.magicCircleConvertRatio.value + rate;
};
...
global.calcMagicRateExtra = function (k, magic) {
    ...
    if (magic.msg[k]) {
        ...
        if (lv) {
            ...
            var conf = conf_mgr.gConfExploreMagicAwardUp[k][lv];
            if (conf) {
...
global.getRefineLevelByExp = function (quality, exp) {
    ...
    var refineConf = conf_mgr.gConfEquipRefine[quality];
    if (!refineConf) {
        return 0;
    }
...
global.getLegionBossTime = function () {
    // var bossTimeStr = conf_mgr.gConfGlobal.legionBossTime;
...
global.getTerritoryWarBossTime = function () {
    ...
    var strBeginTime = conf_mgr.gConfTerritoryWarBase.bossActivityTime.value[0];
...
global.getTerritoryBossEndTime = function () {
    ...
    var strEndTime = conf_mgr.gConfTerritoryWarBase.bossActivityTime.value[1];
...
global.randRuneSpecialAtt = function (id, execpt) {
    ...
    var conf = conf_mgr.gConfRuneSpecialAttConf[id];
    if (!conf) {
        return [];
    }
...
global.isArenaOpen = function (type, player) {
    ...
    var arenaConf = conf_mgr.gConfArenaBase[type];
