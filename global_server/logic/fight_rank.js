...
exports.fight_update = function (req, res, resp, onReqHandled) {
...
exports.get_list_by_snapshot = function (req, res, resp, onReqHandled) {
...
function get_octopus_list(uid, snapshot_dict) {
    var tResultList = {};
    var tKeyList = [];
    for (var tKey in conf_mgr.gConfOctopus) {
        tKeyList.push(tKey);
    }
    ...
    for (var i = 0; i < tKeyList.length; i++) {
        var tKey = tKeyList[i];
        var tConfInfo = conf_mgr.gConfOctopus[tKey];
        ...
        if (target_snapshot) {
            ...
            tTargetLv = tTargetLv > conf_mgr.gMaxUserLevel ? conf_mgr.gMaxUserLevel : tTargetLv;
