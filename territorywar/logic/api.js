...
const { TERRITORYWAR_ErrorCode, TERRITORYWAR_EventType } = require('../../common/error.js');
...
// 移动
exports.move_to = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        ...
        if (!userInfo) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_USER_NOT_FOUND; resp.desc = 'user not found'; break;
        }

        var curLid = userInfo.territoryWar.pos.lid;
        if (!gTerritoryWar.canWalk(curLid, cellId)) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_EMPTY_CELL; resp.desc = 'empty cell'; break;
        }

        // 检查自身是否在战斗状态
        if (gTerritoryWar.isPlayerInFight(uid, curLid)) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_YOU_ARE_IN_FIGHT_STATE; resp.desc = 'fight state'; break;
        }

        // 检查目标格子是否与当前格子相邻
        var curCellId = userInfo.territoryWar.pos.cellId;
        if (!gUserInfo.isNeighborCell(curCellId, cellId)) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_TARGET_NOT_IN_NEIGHBOR_CELL; resp.desc = 'element is not in neighbor cell'; break;
        }

        // 检查目标格子是否有敌人
        var playerList = gTerritoryWar.getCellPlayerList(userInfo.territoryWar.lid, curLid, cellId);
        if (Object.keys(playerList).length > 0) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_PATH_HAS_ENEMY; resp.desc = 'target cell has enemy'; break;
        }
        ...
        if (cellInfo != null) {
            ...
            if (curLid == userInfo.territoryWar.lid) {
                ...
                if (enemyIndex < 0) {
                    resp.code = TERRITORYWAR_ErrorCode.ERROR_NOT_CONTAIN_THIS_LID; resp.desc = 'enemy not contain cur lid'; break;
                }
            ...
            if (cellInfo.resType == TerriToryWarResourceType.CREATURE) {
                var monsterInfo = conf_mgr.gConfTerritoryWarMapMonster[cellInfo.resId];
                if (monsterInfo) {
                    if (monsterInfo.isShare) {
                        if (cellInfo.resParam != 0) {
                            resp.code = TERRITORYWAR_ErrorCode.ERROR_CELL_NOT_VISITED; resp.desc = 'cell has not visited'; break;
                        }
                    } else {
        ...
        if (curLid == userInfo.territoryWar.lid && parseInt(conf_mgr.gConfTerritoryWarBase.initialPos.value) == cellId) {
...
// 传送（传送到目标军团的领地）
exports.transfer = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        ...
        if (enemies.indexOf(targetLid) < 0) {
            if (enemies.indexOf(targetLid.toString()) < 0) {
                resp.code = TERRITORYWAR_ErrorCode.ERROR_TRANSFER_TARGET_ERROR; resp.desc = 'target is not enemy legion'; break;
            }
        }

        //if (gTerritoryWar.getVisitSteleCount(userInfo.territoryWar.lid) < 3) {
        //    resp.code = TERRITORYWAR_ErrorCode.ERROR_TRANSFER_NOT_OPEN; resp.desc = 'visit stele count not enough'; break;
        //}
...
// 回城（回到自己军团的领地，指定的关隘）
exports.go_back = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var cellId = +req.args.cellId;
        ...
        if (!gUserInfo.isCityVisited(uid, cellId)) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_CITY_NOT_VISIT; resp.desc = 'city not visited'; break;
        }

        // 检查目标格子上是否有地方玩家（传送回军团城不判断）
        if (cellId != parseInt(conf_mgr.gConfTerritoryWarBase.initialPos.value) && gTerritoryWar.hasEnemyPlayer(userInfo.territoryWar.lid, cellId, userInfo.territoryWar.lid)) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_TARGET_CELL_HAS_ENEMY; resp.desc = 'target cell has enemy player'; break;
        }

        // 检查是否处于战斗状态
        if (gTerritoryWar.isPlayerInFight(uid, curLid)) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_YOU_ARE_IN_FIGHT_STATE; resp.desc = 'you are in fight state'; break;
        }
        ...
        if (cellId == parseInt(conf_mgr.gConfTerritoryWarBase.initialPos.value)) {
...
// 采集资源
exports.gather = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var cellId = +req.args.cellId;
        ...
        var cellInfo = gTerritoryWar.getCellInfo(curLid, cellId);
        if (cellInfo == null) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_CAN_NOT_FIND_CELL_INFO; resp.desc = 'empty cell or cellId error'; break;
        }
        ...
        enemyIndex += 1;
        if (visitedList[cellId]) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_CELL_HAS_GATHER; resp.desc = 'cell has gathered'; break;
        }

        var resId = cellInfo.resId;
        if (gTerritoryWar.hasOnceElement(cellId)) {
            var gridConf = conf_mgr.gConfTerritoryWarMapGrid[cellId];
        ...
        var eventType = conf_mgr.gConfTerritoryWarMapElement[resId].event;
        ...
        var awards = clone(conf_mgr.gConfTerritoryWarMapElement[resId].award);
        if (awards && awards[0] && awards[0][0] == 'level') {
            var awardStr = awards[0][1];
            var awardNum = awards[0][2];
            var levelAward = clone(conf_mgr.gConfLevel[joinLevel][awardStr]);
...
// 占领矿点
exports.occupy_mine = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
        ...
        var curLid = userInfo.territoryWar.pos.lid;
        if (myLid != curLid) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_CAN_NOT_OCCUPY_ENEMY_MINE; resp.desc = 'lid error'; break;
        }
        ...
        if (cellInfo.resType != TerriToryWarResourceType.MINE) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_TARGET_CELL_IS_NOT_MINE; resp.desc = 'cell not mine'; break;
        }

        if (gTerritoryWar.isMineOccupy(curLid, cellId)) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_TARGET_MINE_HAS_OCCUPY; resp.desc = 'mine has occupy'; break;
        }

        // 检查怪物所在格子与玩家所在格子是否相邻
        if (!gUserInfo.isNeighborCell(userInfo.territoryWar.pos.cellId, cellId)) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_TARGET_NOT_IN_NEIGHBOR_CELL; resp.desc = 'mine is not in neighbor cell'; break;
        }

        gTerritoryWar.onMineOccupy(uid, curLid, cellId);

        // 事件处理
        var mineName = conf_mgr.gConfTerritoryWarMapMine[cellInfo.resId].name;
        gTerritoryWar.addEvent(myLid, TERRITORYWAR_EventType.EVENT_OCCUPY_MINE, userInfo.info.un, mineName);
    } while (false);
...
// 掠夺
exports.rob_mine = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
        ...
        var curLid = userInfo.territoryWar.pos.lid;
        if (myLid == curLid) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_CAN_NOT_ROB_SELF_MINE; resp.desc = 'lid error'; break;
        }
        ...
        if (visitedList[cellId]) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_TARGET_MINE_HAS_ROB; resp.desc = 'mine has robed'; break;
        }
        ...
        var awards = clone(conf_mgr.gConfTerritoryWarMapMine[cellInfo.resId].award2);
        for (var i = 0; i < awards.length; i++) {
            awards[i][2] = Math.floor(awards[i][2] * (1 + param[0] / 100));
        }
        ...
        if (roberLegionInfo) {
            gTerritoryWar.addEvent(curLid, TERRITORYWAR_EventType.EVENT_MINE_WAS_ROBED, mineName, roberLegionInfo.name, userInfo.info.un);
        }

        var targetLegionInfo = gTerritoryWar.getLegionInfo(curLid);
        if (targetLegionInfo) {
            gTerritoryWar.addEvent(userInfo.territoryWar.lid, TERRITORYWAR_EventType.EVENT_ROB_MINE, userInfo.info.un, targetLegionInfo.name, mineName);
        }
    } while (false);
...
// 获取矿信息
exports.get_mine_Info = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
...
// 攻击怪物
exports.attack_monster = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
        ...
        var cellInfo = gTerritoryWar.getCellInfo(curLid, cellId);
        if (cellInfo == null) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_CAN_NOT_FIND_CELL_INFO; resp.desc = 'cell info is null'; break;
        }

        if (cellInfo.resType != TerriToryWarResourceType.CREATURE) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_TARGET_CELL_IS_NOT_CREATURE; resp.desc = 'cell res type is not creature'; break;
        }

        var monsterInfo = conf_mgr.gConfTerritoryWarMapMonster[cellInfo.resId];
        if (!monsterInfo) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_CAN_NOT_FIND_CREATURE_INFO; resp.desc = 'monster info not found'; break;
        }
        ...
        if (monsterInfo.isShare) {
            if (cellInfo.resParam == 0) {
                resp.code = TERRITORYWAR_ErrorCode.ERROR_SHARE_MONSTER_IS_DEAD; resp.desc = 'share monster is dead'; break;
            }
            ...
            if (visitedList[cellId] && visitedList[cellId].param == 0) {
                resp.code = TERRITORYWAR_ErrorCode.ERROR_PRIVATE_MONSTER_IS_DEAD; resp.desc = 'monster is dead'; break;
            }
...
// 战斗开始
exports.before_fight_monster = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var cellId = +req.args.cellId;
        var userInfo = gUserInfo.getUser(uid);
        ...
        // 检查怪物所在格子与玩家所在格子是否相邻
        if (!gUserInfo.isNeighborCell(userInfo.territoryWar.pos.cellId, cellId)) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_TARGET_NOT_IN_NEIGHBOR_CELL; resp.desc = 'monster is not in neighbor cell'; break;
        }
        ...
        var formationConf = conf_mgr.gConfFormation[monsterInfo.combatId];
        var bossId = formationConf['pos' + formationConf.boss];
        var monsterConf = conf_mgr.gConfMonster[bossId];
        if (userInfo.status.level + 5 < monsterConf.level) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_USER_LEVEL_NOT_ENOUGH; resp.desc = 'level not enough'; break;
        }
...
exports.fight_monster = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var cellId = +req.args.cellId;
...
// 秒杀怪物
exports.sec_kill_monster = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
        ...
        if (secKill.indexOf(cellInfo.resId) < 0) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_NOT_SEC_KILL;; resp.desc = 'can not kill'; break;
        }

        var formationId = monsterInfo.combatId;;
        if (formationId) {
            // 获取怪物等级
            var formationConf = conf_mgr.gConfFormation[formationId];
            if (!formationConf) {
                resp.code = TERRITORYWAR_ErrorCode.ERROR_CAN_NOT_FIND_CREATURE_INFO; resp.desc = 'formation info not found'; break;
            }

            var monsterId = formationConf['pos' + formationConf.boss];
            var monsterConf = conf_mgr.gConfMonster[monsterId];
            if (!monsterConf) {
                resp.code = TERRITORYWAR_ErrorCode.ERROR_CAN_NOT_FIND_CREATURE_INFO; resp.desc = 'monster info not found'; break;
            }
...
// 攻击boss
exports.attack_boss = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
        ...
        if (cellInfo.resType != TerriToryWarResourceType.BOSS) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_TARGET_CELL_IS_NOT_CREATURE; resp.desc = 'cell res type is not boss'; break;
        }
        ...
        if (curTime < beginTime || curTime >= endTime) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_NOT_BOSS_CHALLENGE_TIME; resp.desc = 'not open time'; break;
        }
...
exports.before_fight_boss = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var cellId = +req.args.cellId;
        var userInfo = gUserInfo.getUser(uid);
...
exports.fight_boss = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var cellId = +req.args.cellId;
        ...
        var bossInfo = conf_mgr.gConfTerritoryBossForce[cellInfo.resId];
        if (bossInfo) {
            var scaleCount = gUserInfo.getDragonScaleCount(uid);
            var factor = parseFloat(conf_mgr.gConfTerritoryWarBase.bossScoreAdd.value);
...
// 获取格子里面的玩家列表
exports.get_cell_player_list = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
        ...
        if (Object.keys(resp.data.playerList).length == 0) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_EMPTY_CELL;
...
// 攻击玩家
exports.attack_player = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
        ...
        if (!targetInfo) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_CAN_NOT_FIND_TARGET_PLAYER; resp.desc = 'target not found'; break;
        }

        if (targetInfo.territoryWar.lid == userInfo.territoryWar.lid) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_CAN_NOT_ATTACK_LEGION_MEMBER; resp.desc = 'can not attack same legion player'; break;
        }

        // 检查目标所在格子与玩家所在格子是否相邻
        if (!gUserInfo.isNeighborCell(userInfo.territoryWar.pos.cellId, targetInfo.territoryWar.pos.cellId)) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_TARGET_NOT_IN_NEIGHBOR_CELL; resp.desc = 'target is not in neighbor cell'; break;
        }

        // 检查自己及目标是否在战斗中
        if (gTerritoryWar.isPlayerInFight(uid, userInfo.territoryWar.pos.lid)) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_YOU_ARE_IN_FIGHT_STATE; resp.desc = 'you are in battle'; break;
        }

        if (gTerritoryWar.isPlayerInFight(targetUid, targetInfo.territoryWar.pos.lid)) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_TARGET_IS_IN_FIGHT_STATE; resp.desc = 'target is in battle'; break;
        }
...
// 攻击玩家结果
exports.fight_player = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
        ...
        if (!enemyInfo) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_CAN_NOT_FIND_TARGET_PLAYER; resp.desc = 'enemy not found'; break;
        }
        ...
        if (userInfo.territoryWar.lid == userInfo.territoryWar.pos.lid) {
            ...
            if (star > 0) {
                ...
                gTerritoryWar.addEvent(userInfo.territoryWar.lid, TERRITORYWAR_EventType.EVENT_DEFEAT_ENEMY_IN_OUR_CITY,
                ...
                gTerritoryWar.addEvent(enemyInfo.territoryWar.lid, TERRITORYWAR_EventType.EVENT_WAS_DEFEAT_BY_IN_OTHER_CITY,
                    enemyInfo.info.un, myLegionInfo.name, userInfo.info.un);
            } else {
                var enemyLegionInfo = gTerritoryWar.getLegionInfo(enemyInfo.territoryWar.lid);
                gTerritoryWar.addEvent(userInfo.territoryWar.lid, TERRITORYWAR_EventType.EVENT_WAS_DEFEAT_BY_IN_OUR_CITY,
...
// 获取战报
exports.get_reports = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
...
// 领取成就奖励
exports.get_achievement_awards = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        ...
        if (gUserInfo.isAchievementAwardGot(uid, achievementType, achievementId)) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_AWARD_HAS_GOT; resp.desc = 'award has got'; break;
        }

        if (!gUserInfo.isAchievementFinish(uid, achievementType, achievementId)) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_ACHIEVEMENT_NOT_FINISH; resp.desc = 'achievement not finish'; break;
        }
...
exports.challenge_before_fight = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
...
exports.challenge_fight = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
...
// 探索遗迹
exports.start_explore = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
        ...
        // 检查是否已经有已经正在探索
        if (gUserInfo.getExploreCount(uid) >= parseInt(conf_mgr.gConfVip[userInfo.status.vip].relicNum)) {
            resp.code = TERRITORYWAR_ErrorCode.ERROR_HAS_EXPLORE; resp.desc = 'explore count more than 1'; break;
        }
...
// 停止探索
exports.stop_explore = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
...
// 加速探索
exports.speed_explore = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
...
// 获取本军团事件列表
exports.get_events = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
...
// 获取敌方信息
exports.get_enemy = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
...
exports.offline = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
...
// 获取玩家领地战信息
exports.get_player_territory_war_info = function (req, res, resp, onReqHandled) {
    do {
        resp.data = {};
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
...
// 取消战斗
exports.cancel_fight = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
...
// 玩家离开军团
exports.on_player_leave_legion = function (req, res, resp, onReqHandled) {
    do {
...
// 军团解散
exports.on_legion_dismiss = function (req, res, resp, onReqHandled) {
    do {
        var lid = +req.args.lid;
...
// 获取关隘状态
exports.get_city_state = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
        ...
        var cities = {};
        for (var id in conf_mgr.gConfTerritoryWarTransfer) {
            var cellId = parseInt(conf_mgr.gConfTerritoryWarBase[conf_mgr.gConfTerritoryWarTransfer[id].target].value);
...
// 获取boss排行榜
exports.get_boss_rank = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
...
// 获取boss信息
exports.get_boss_info = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
...
// 获取boss列表
exports.get_boss_list = function (req, res, resp, onReqHandled) {
    do {
        var uid = +req.uid;
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
...
exports.legion_level_up = function (req, res, resp, onReqHandled) {
    do {
        var lid = req.args.lid;
        var level = req.args.level;
        var legionWarLevel = req.args.legionWarLevel;
...
exports.get_boss_notice_info = function (req, res, resp, onReqHandled) {
    do {
