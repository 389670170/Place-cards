...
UserInfo.prototype = {
    ...
    getUserFightInfo: function (uid, notUseDefPos, useWorldWarTeam) {
        ...
        if (user.territoryWar.pos.lid == user.territoryWar.lid) {
            ...
            if (cellId == parseInt(conf_mgr.gConfTerritoryWarBase.jianmenguanPos.value) ||
                cellId == parseInt(conf_mgr.gConfTerritoryWarBase.jiayuguanPos.value) ||
                cellId == parseInt(conf_mgr.gConfTerritoryWarBase.hulaoguanPos.value) ||
                cellId == parseInt(conf_mgr.gConfTerritoryWarBase.initialPos.value)) {
    ...
    update: function (uid, lid, updates, dragon, buildings) {
        ...
        if (!userInfo) {
            ...
            if (!this.users[uid].territoryWar) {
                ...
                this.users[uid].territoryWar.pos.cellId = parseInt(conf_mgr.gConfTerritoryWarBase.initialPos.value);
    ...
    onPlayerGetAchievementAward: function (uid, achievementType, achievementId, level) {
        ...
        var achievementInfo = conf_mgr.gConfTerritoryWarAchievement[achievementType][achievementId];
        if (!achievementInfo) {
            return null;
        }
        ...
        var awardIndex = 0;
        var levels = conf_mgr.gConfTerritoryWarAchievementType[achievementType].level;
        ...
        return conf_mgr.gConfTerritoryWarAchievementAwards[awardId].award;
    },
    ...
    onPlayerGoBack: function (uid, transferId) {
        ...
        if (conf_mgr.gConfTerritoryWarTransfer[transferId].cost)
    ...
    startExploreRelic: function (uid, relicId) {
        ...
        if (!this.users[uid].territoryWar.relic[relicId]) {
            return TERRITORYWAR_ErrorCode.ERROR_CAN_NOT_FIND_RELIC;
        }

        if (this.users[uid].territoryWar.relic[relicId].num <= 0) {
            return TERRITORYWAR_ErrorCode.ERROR_RELIC_NUM_NOT_ENOUGH;
        }

        if (this.users[uid].territoryWar.relic[relicId].endTime != 0) {
            return TERRITORYWAR_ErrorCode.ERROR_RELIC_IS_ALREADY_STARTED;
        }

        var param = this.getBuildingParam(uid, 'relicExploreTime');
        var remainTime = conf_mgr.gConfTerritoryWarRelic[relicId].time * 3600 - this.users[uid].territoryWar.relic[relicId].exploreTime;
    ...
    stopExploreRelic: function (uid, relicId) {
        ...
        var totalTime = conf_mgr.gConfTerritoryWarRelic[relicId].time * 3600;
    ...
    speedExploreRelic: function (uid, relicId) {
        if (!this.users[uid])
            return TERRITORYWAR_ErrorCode.ERROR_USER_NOT_FOUND;
        ...
        var leftTime = 0;
        if (endTime == 0) {
            leftTime = conf_mgr.gConfTerritoryWarRelic[relicId].time * 3600 - exploreTime;
        } else {
            leftTime = endTime - curTime;
        }

        var speedTime = parseInt(conf_mgr.gConfTerritoryWarBase.relicAccelerateTime.value) * 3600;
    ...
    getBuildingIdWithFunction: function (func_name) {
        for (var cityId in conf_mgr.gConfLegionCityBase) {
            var cityInfo = conf_mgr.gConfLegionCityBase[cityId];
            for (var i = 1; i <= 4; i++) {
    ...
    getBuildingParam: function (uid, func_name) {
        ...
        if (this.users[uid].territoryWar.buildings) {
            if (this.users[uid].territoryWar.buildings[buildingId]) {
                ...
                param = conf_mgr.gConfLegionCityConf[buildingId][buildingLevel].value;
            }
        }
    ...
    sendRelicAward: function (uid, relicId) {
        var relicInfo = conf_mgr.gConfTerritoryWarRelic[relicId];
        if (true || !relicId) {
            return;
        }
        ...
        var mail = {
            ...
            awards: awards,
            time: time,
            expire: time + conf_mgr.gConfMail[28].time * OneDayTime,
            attach: {
    ...
    isNeighborCell: function (cell1, cell2) {
        var cellInfo1 = conf_mgr.gConfTerritoryWarMapGrid[cell1];
        var cellInfo2 = conf_mgr.gConfTerritoryWarMapGrid[cell2];
