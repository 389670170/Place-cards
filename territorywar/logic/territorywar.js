...
const LegionWarCom = require('../../common/legionwar.js');
...
TerritoryWar.prototype = {
    ...
    _init: function (callback) {
        ...
        for (var lid in this.territorys) {
            ...
            if (!this.territorys[lid].visitedCells) {
                ...
                this.territorys[lid].visitedCells[lid].push(parseInt(conf_mgr.gConfTerritoryWarBase.initialPos.value));
            ...
            if (this.territorys[lid].enemies.length == 0 && !this.isInWaitMatchQueue(lid) &&
                Object.keys(this.territorys[lid].stele).length >= parseInt(conf_mgr.gConfTerritoryWarBase.invadeLimit.value)) {
    ...
    initInvadeCells: function () {
        ...
        var invadePos = +conf_mgr.gConfTerritoryWarBase.invadePos.value;
        var cellData = conf_mgr.gConfTerritoryWarMapGrid[invadePos];
        ...
        for (var cellId in conf_mgr.gConfTerritoryWarMapGrid) {
            var pos = conf_mgr.gConfTerritoryWarMapGrid[cellId].pos;
    ...
    initWaitMatchQueue: function () {
        ...
        for (var id in conf_mgr.gConfLegionWarRank) {
    ...
    processFightState: function () {
        ...
        for (var lid in this.territorysMemData) {
            ...
            for (var uid in legionPlayers.battle_palyers) {
                if (curTime > legionPlayers.battle_palyers[uid].battle_start_time + conf_mgr.gConfTerritoryWarBase.combatLockTime.value) {
    ...
    getNewLegionName: function () {
        var max = Object.keys(conf_mgr.gConfLegionName).length;
        var index = common.randRange(1, max);
        return conf_mgr.gConfLegionName[index].name;
    },
    ...
    initOnceElement: function () {
        this.onceElementCells = [];
        var cellCount = Object.keys(conf_mgr.gConfTerritoryWarMapGrid).length;
        for (var i = 1; i <= cellCount; i++) {
            if (conf_mgr.gConfTerritoryWarMapGrid[i].onceElement.length > 0) {
                // 有一次性掉落
    ...
    buildInitData: function () {
        ...
        var cellCount = Object.keys(conf_mgr.gConfTerritoryWarMapGrid).length;
        for (var i = 1; i <= cellCount; i++) {
            if (conf_mgr.gConfTerritoryWarMapGrid[i].immobilizationElement != '') {
                this.initData.cells[i] = {};
                this.initData.cells[i].x = conf_mgr.gConfTerritoryWarMapGrid[i].pos[0];
                this.initData.cells[i].y = conf_mgr.gConfTerritoryWarMapGrid[i].pos[1];

                var elementInfo = conf_mgr.gConfTerritoryWarMapGrid[i].immobilizationElement.split('.');
                ...
                if (parseInt(conf_mgr.gConfTerritoryWarMapGrid[i].isRandomElement) != 0) {
                    emptyCell.push(i);
                }
            }
        }
        ...
        // 初始化随机内容
        for (var i in conf_mgr.gConfTerritoryWarMapElement) {
            if (conf_mgr.gConfTerritoryWarMapElement[i].randomNum > 0) {
                for (var j = 0; j < conf_mgr.gConfTerritoryWarMapElement[i].randomNum; j++) {
                    var index = common.randRange(0, emptyCell.length - 1);
                    var cellId = emptyCell[index];

                    if (!conf_mgr.gConfTerritoryWarMapGrid[cellId]) {
                        DEBUG('cell is null id is ' + cellId);
                        var a = 0;
                    }
                    this.initData.cells[cellId] = {};
                    this.initData.cells[cellId].x = conf_mgr.gConfTerritoryWarMapGrid[cellId].pos[0];
                    this.initData.cells[cellId].y = conf_mgr.gConfTerritoryWarMapGrid[cellId].pos[1];
    ...
    createBoss: function (lid) {
        ...
        var maxBossLevel = conf_mgr.gConfTerritoryBossLegionConf[legionLevel].levelMax;
        var minBossLevel = conf_mgr.gConfTerritoryBossLegionConf[legionLevel].levelMin;
        ...
        var bossForceConf = conf_mgr.gConfTerritoryBossForce[bossLevel];
        var bossIndex = common.randRange(0, bossForceConf.battleId.length - 1);

        var bossPosArr = conf_mgr.gConfTerritoryWarBase.bossPos.value;
        ...
        this.territorys[lid].cells[bossCellId].x = conf_mgr.gConfTerritoryWarMapGrid[bossCellId].pos[0];
        this.territorys[lid].cells[bossCellId].y = conf_mgr.gConfTerritoryWarMapGrid[bossCellId].pos[1];
    ...
    insertWaitMatchLegion: function (boxId, lid, isBase, isRobot, createLid) {
        ...
        } else if (curCount >= parseInt(conf_mgr.gConfTerritoryWarBase.legionNumLimit.value)) {
            // 盒子满了，将这个盒子里的军团放在一组
            var removeIds = [];
            for (var legionId in this.waitMatchQueue[boxId].legions) {
                ...
                if (territory) {
                    for (var subLid in this.waitMatchQueue[boxId].legions) {
                        if (subLid != legionId) {
                            ...
                            if (territory.enemies.indexOf(enemyLid) < 0 &&
                                territory.enemies.length < parseInt(conf_mgr.gConfTerritoryWarBase.legionNumLimit.value) - 1) {
                    ...
                    // 事件处理
                    this.addEvent(legionId, TERRITORYWAR_EventType.EVENT_MATCH_ENEMY);
                }
            }
    ...
    getTerritoryData: function (uid, lid, lname, legionLevel, legionWarLevel, legionIcon, serverId, gatherRecord, leader) {
        ...
        if (curLid == lid) {
            ...
            if (!this.territorys[curLid]) {
                // 踢回自己的领地
                var transferData = conf_mgr.gConfTerritoryWarTransfer[1];
                var cellId = parseInt(conf_mgr.gConfTerritoryWarBase[transferData.target].value);
        ...
        if (enemies.length == 0 && !this.isInWaitMatchQueue(lid) &&
            legionLevel >= parseInt(conf_mgr.gConfTerritoryWarBase.portalOpenLevel.value)) {
        ...
        if (this.reports[uid]) {
            ...
            for (var i = 0, len = reports.length; i < len; i++) {
                ...
                if (user) {
                    ...
                    quality = conf_mgr.gConfHero[user.pos[1].hid].quality;
        ...
        for (var sid in this.servers) {
            this.broadcastToWorld(sid, worldReq, worldResp, function () {
                if (worldResp.code == 0) {
                    ...
                    if (legionInfo.level < parseInt(conf_mgr.gConfTerritoryWarBase.portalOpenLevel.value))
                        return;
    ...
    onPlayerVisitStoneStele: function (uid, lid, visit_lid, visit_cellId) {
        ...
        if (lid == visit_lid) {
            ...
            if (Object.keys(this.territorys[lid].stele).length >= parseInt(conf_mgr.gConfTerritoryWarBase.invadeLimit.value)) {
                // 改为去world取军团段位信息
                this.requestLegionInfo(lid);

                // 事件处理
                this.addEvent(lid, TERRITORYWAR_EventType.EVENT_TRANSFER_OPEN);
            }
        }
        ...
        if (userInfo) {
            if (lid == visit_lid) {
                this.addEvent(lid, TERRITORYWAR_EventType.EVENT_VISIT_OUR_STELE, userInfo.info.un);
            } else {
                var legionInfo = this.getLegionInfo(visit_lid);
                if (legionInfo)
                    this.addEvent(lid, TERRITORYWAR_EventType.EVENT_VISIT_OTHER_STELE, userInfo.info.un, legionInfo.name);
            }
        }
    ...
    onPlayerVisitWatchTower: function (uid, lid, visit_lid, visit_cellId) {
        ...
        if (this.territorys[lid].watchTower.indexOf(visit_cellId) < 0) {
            ...
            // 将瞭望塔开启的格子加入已访问
            var openCells = conf_mgr.gConfTerritoryWarBase.watchTowerSpecialOpen.value;
            ...
            if (userInfo) {
                ...
                var towerName = conf_mgr.gConfTerritoryWarMapElement[resId].name;
                this.addEvent(lid, TERRITORYWAR_EventType.EVENT_ACTIVE_WATCH_TOWER, userInfo.info.un, towerName);
            }
        }
    },
    ...
    onPlayerDeath: function (uid, killer) {
        ...
        if (userInfo.territoryWar.lid == userInfo.territoryWar.pos.lid) {
            // 在自己领地被杀死
            gUserInfo.setPos(uid, userInfo.territoryWar.lid, parseInt(conf_mgr.gConfTerritoryWarBase.initialPos.value));
            this.broadcastPlayerPosition(uid, userInfo.territoryWar.lid, parseInt(conf_mgr.gConfTerritoryWarBase.initialPos.value), true);
        } else {
            // 将玩家传送到出生地
            gTerritoryWar.onPlayerEnter(uid, userInfo.territoryWar.lid, parseInt(conf_mgr.gConfTerritoryWarBase.initialPos.value));
        }
    ...
    addEvent: function (lid, event_id, param1, param2, param3) {
        ...
        var maxCount = parseInt(conf_mgr.gConfTerritoryWarBase.eventRecordNum.value);
    ...
    calcMineAwards: function () {
        ...
        for (var lid in this.territorys) {
            ...
            for (var mineId in territory.mine) {
                ...
                // 掠夺损失
                var param = territory.mine[mineId].robCount * parseInt(conf_mgr.gConfTerritoryWarBase.pillageLose.value);
                if (param > 100) {
                    param = 100;
                }
                ...
                var param2 = 0;
                if (visitCount > 0) {
                    param2 = parseInt(conf_mgr.gConfTerritoryWarBase.stoneTabletIncrease.value[visitCount - 1]);
                }

                params[mineId].param2 = param2;
            }

            if (Object.keys(params).length > 0 && conf_mgr.gConfMail[26]) {
                var time = common.getTime();
                var mail = {
                    awards: 'territory_war',
                    time: time,
                    expire: time + conf_mgr.gConfMail[26].time * OneDayTime,
                    attach: {
                        lid: uLid,
                        params: params,
                    },
    ...
    calcBossAwards: function () {
        ...
        for (var lid in this.territorys) {
            ...
            if (territory.boss.curOwner != 0 && conf_mgr.gConfMail[29]) {
                // 发归属奖励
                var time = common.getTime();
                var mail = {
                    awards: 'territory_boss_owner',
                    time: time,
                    expire: time + conf_mgr.gConfMail[29].time * OneDayTime,
                    attach: {
            ...
            if (territory.boss.damage.damageRank.length > 0 && conf_mgr.gConfMail[30]) {
                var rank = [];
                var lids = [];
                ...
                var mail = {
                    ...
                    time: time,
                    expire: time + conf_mgr.gConfMail[30].time * OneDayTime,
                    attach: {
                        lids: lids,
                        rank: rank,
                    },
    ...
    updateLegionLevel: function (lid, level, legionWarLevel, serverId) {
        if (this.legions[lid]) {
            ...
            if (level >= parseInt(conf_mgr.gConfTerritoryWarBase.portalOpenLevel.value) &&
                Object.keys(this.territorys[lid].stele).length >= parseInt(conf_mgr.gConfTerritoryWarBase.invadeLimit.value)) {
    ...
    getBossSendAwardTime: function () {
        var strAwardTime = conf_mgr.gConfTerritoryWarBase.bossAwardTime.value[0].split(":");
