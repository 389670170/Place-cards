...
function saveServerRegData(serverRegData, callback) {
    gDBRegister.save({ _id: serverRegData.sid, legions: serverRegData.legions }, function (err) {
        if (err) {
            ERROR(util.format('%j', err));
            callback(TERRITORYWAR_ErrorCode.ERROR_DB_ERROR);
        } else {
            callback();
        }
    });
...
function getLegionMatchInfo(callback) {
    gDBLegion.find({}, { '_id': 1, 'sid': 1, 'lid': 1, 'score': 1, 'fight_force': 1 }).toArray(function (err, docs) {
        if (err) {
            callback(err);
        } else {
            callback(null, docs);
