...
const { loadConf } = require('../common/conf_mgr.js');
...
loadConf(config, ServerName.GATE_WAY)
...
function startWebServer() {
    ...
    http.createServer(function (req, res) {
        ...
        var reqObj;
        if (common.isBase64(tTempUrl)) {
            tTempUrl = (new Buffer(tTempUrl, 'base64')).toString('utf8');
            reqObj = url.parse("/?" + tTempUrl, true);
        }
        else {
            reqObj = url.parse(req.url, true);
        }
        ...
        req.on('end', function () {
            if (path.endWith('/crossdomain.xml')) {
                ...
                return;
            } else if (path.beginWith('/global/') && common.GLOBAL_SERVER_INFO_DICT[ServerName.GLOBAL]) {
                httpGet(common.GLOBAL_SERVER_INFO_DICT[ServerName.GLOBAL].host, common.GLOBAL_SERVER_INFO_DICT[ServerName.GLOBAL].port, path.substr(7), function (data) {
                    res.writeHead(200, common.defaultHeaders);
                    res.end(data);
                });
                return;
            } else if (path.beginWith('/legionwar/') && common.GLOBAL_SERVER_INFO_DICT[ServerName.LEGION_WAR]) {
                httpGet(common.GLOBAL_SERVER_INFO_DICT[ServerName.LEGION_WAR].host, common.GLOBAL_SERVER_INFO_DICT[ServerName.LEGION_WAR].port, path.substr(10), function (data) {
                    res.writeHead(200, common.defaultHeaders);
                    res.end(data);
                });
                return;
            } else if (path.beginWith('/worldwar/') && common.GLOBAL_SERVER_INFO_DICT[ServerName.WORLD_WAR]) {
                httpGet(common.GLOBAL_SERVER_INFO_DICT[ServerName.WORLD_WAR].host, common.GLOBAL_SERVER_INFO_DICT[ServerName.WORLD_WAR].port, path.substr(9), function (data) {
                    res.writeHead(200, common.defaultHeaders);
                    res.end(data);
                });
                return;
            } else if (path.beginWith('/territorywar/') && common.GLOBAL_SERVER_INFO_DICT[ServerName.TERRITORY_WAR]) {
                httpGet(common.GLOBAL_SERVER_INFO_DICT[ServerName.TERRITORY_WAR].host, common.GLOBAL_SERVER_INFO_DICT[ServerName.TERRITORY_WAR].port, path.substr(13), function (data) {
                    res.writeHead(200, common.defaultHeaders);
                    res.end(data);
                });
                return;
    ...
    console.log(` web server listen on ${config.GatewayListen} `);

    var pidFile = `pid_gateway_${config.ServerId}.pid`;
...
var logic = {};
// 登录,如果是新用户完成注册
logic.login = function (req, res, resp, onReqHandled) {
    var openid = req.openid;
    if (!openid) {
        openid = 0;
    }
...
// GM管理接口
logic.gm = function (req, res, resp, onReqHandled) {
    var openid = req.openid;
    ...
    if (req.method == 'reload') {
        for (var sid in config.Games) {
            var tGameServer = config.Games[sid];
            var path = "/" + '?mod=gm&act=reload&uid=10000&args=' + encodeURI(JSON.stringify({}));
            httpGet(tGameServer[0], tGameServer[1], path, function (data) {
                res.writeHead(200, common.defaultHeaders);
                res.end(data);
            }, null, true);
        }
    ...
    var tGameServer = getGameServer(uid, true);
...
logic.pay = function (req, res) {
    ...
    gDBPay.find({ _id: billno }, { _id: 1 }).limit(1).next(function (err, doc1) {
        ...
        gDBPay.insertOne(record, function (err, result) {
            ...
            // 请求game数据
            var tGameServer = getGameServer(existUid, true);
