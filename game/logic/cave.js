...
exports.get = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        if (user.cave.cave_arr && user.cave.cave_arr.length == 0) { // 重置 没有格子数组
            var maxDiceNum = conf_mgr.gConfGlobal.customCaveDiceNumLimit;// 最大骰子上限
...
exports.shake_dice = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        if (getawards.chip) {// 如果是超级宝箱碎片
            ...
            if (index >= conf_mgr.gConfGlobal.customCaveBoxNeedChipNum) {// 积满碎片
        ...
        logic_event_mgr.emit(logic_event_mgr.EVENT.DO_DAILY_TASK, player, 'cave', 1);
...
exports.reply_dice = function (player, req, resp, onHandled) {
    var user = player.user;
    do {
        if (common.getTime() - user.cave.start_reply_time < conf_mgr.gConfGlobal.customCaveDiceInterval * 60) {
...
exports.get_awards = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        var awards = conf_mgr.gConfCustomCaveAward[user.cave.level].superAward;
        resp.data.awards = player.addAwards(awards, req.mod, req.act);
    } while (false);
...
exports.open_box = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        var costs = conf_mgr.gConfItem[id].cost.slice(0);
        ...
        var awards = [];
        if (specialAward) {
            awards = conf_mgr.gConfSpecialReward['first_open_treasure'].reward;
        } else {
            awards = generateDrop(parseAwardsConfig(conf_mgr.gConfItem[id].useEffect)[0][1], player.user.level);
        }
...
function createCaveArr(superCave) {
    ...
    var arr = [];
    for (var i in conf_mgr.gConfCustomCaveEvent) {
        var customCaveEvent = conf_mgr.gConfCustomCaveEvent[i];
...
function caveNum(dice_num) {
    ...
    function getWeight(diceNumArr) {
        ...
        for (var i in conf_mgr.gConfCustomCaveDiceWeight) {
            initWeight += conf_mgr.gConfCustomCaveDiceWeight[i].weight;
        }
        var totalWeight = initWeight + conf_mgr.gConfCustomCaveDiceWeight[diceNumArr[0]].addweight
            + conf_mgr.gConfCustomCaveDiceWeight[diceNumArr[1]].addweight; // 总权重

        for (var i in conf_mgr.gConfCustomCaveDiceWeight) {
            var weight = conf_mgr.gConfCustomCaveDiceWeight[i].weight;
            var addWeight = 0;
            if (diceNumArr[0] == i || diceNumArr[1] == i) {
                addWeight = conf_mgr.gConfCustomCaveDiceWeight[i].addweight;
            }
...
function getAwards(player, arr, curStay, level, chip) {
    ...
    if (type == 'gold') {
        ...
        if (odds <= conf_mgr.gConfGlobal.customCaveGoldAward) {
            var lootId = conf_mgr.gConfCustomCaveAward[level]['goldAdditionalLootId'];
    ...
    if (conf_mgr.gConfCustomCaveAward[level] && conf_mgr.gConfCustomCaveAward[level][type]) {
        awards.push(conf_mgr.gConfCustomCaveAward[level][type][0]);
    }
    return {
        awards: awards,
        chip: chipNum, // 碎片位置
