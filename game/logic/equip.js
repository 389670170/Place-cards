...
function equitInfoToWorld(user) {
    ...
    requestWorld(req, {}, function () {
        // client_send_msg(ServerName.WORLD, req.act, req.mod, req.args, {}, null, function () {
...
exports.dismantling = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        if (!equip || equip.god || equip.pos || !conf_mgr.gConfEquip[equip.id].isAncient) {
            sidOk = false;
        ...
        var conf = conf_mgr.gConfEquip[equip.id];
        if (conf.dismantlingAward.length == 0) {
            resp.code = 1; resp.desc = 'no dismantling'; break;
        }

        var costs = [['user', 'gold', -conf_mgr.gConfEquip[equip.id].dismantlingCost]];
...
exports.merge_dress = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        var conf = conf_mgr.gConfSoldierDress[did];
        if (!conf) {
...
exports.intensify = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        var equipConf = conf_mgr.gConfEquip[equip.id];
        if (!equipConf) {
            resp.code = 1; resp.desc = 'no the equip conf'; break;
        }

        var equipBaseConf = conf_mgr.gConfEquipBase[equipConf.type][equipConf.quality];
        if (!equipBaseConf) {
            resp.code = 1; resp.desc = 'no the equip base conf'; break;
        }

        // 是否已强化到顶级
        var maxLevel = Object.keys(conf_mgr.gConfEquipUpgrade).max();
        ...
        for (var level = toLevel; level > curLevel; level--) {
            var nextIntensifyConf = conf_mgr.gConfEquipUpgrade[level];
        ...
        logic_event_mgr.emit(logic_event_mgr.EVENT.DO_DAILY_TASK, player, 'equipUpgrade', 1);
        // 引导任务
...
exports.intensify_all = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        while (!allMaxLevel && !goldLack) {
            ...
            var toLevel = min + 1;
            var conf = conf_mgr.gConfEquipUpgrade[toLevel];
            ...
            var equipConf = conf_mgr.gConfEquip[equipArr[minIndex].id];
            if (!equipConf) {
                confError = true;
                break;
            }
...
exports.refine = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        // 是否已精炼到顶级
        var refineConf = conf_mgr.gConfEquipRefine[equipConf.quality];
        if (!refineConf) {
        ...
        for (var costsId in item) {
            if (!conf_mgr.gConfItem[costsId]) {
                resp.code = 1; resp.desc = 'invaild args'; connfigValid = false; break;
            }

            if (conf_mgr.gConfItem[costsId].useType != 'partexp') {
            ...
            allCosts.push(costs);
            addExp += costsNum * conf_mgr.gConfItem[costsId].useEffect;
        }
...
exports.refine_all = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        for (var i = 0; i < conf_mgr.gEquipRefineItemsLowToHigh.length; i++) {
            costNum[conf_mgr.gEquipRefineItemsLowToHigh[i].id] = 0;
            hasNum[conf_mgr.gEquipRefineItemsLowToHigh[i].id] = getMaterialCount(player, conf_mgr.gEquipRefineItemsLowToHigh[i].id);
        }
        ...
        while (!allMaxLevel && !itemLack) {
            ...
            var minIndex = 0;
            for (var k in equipArr) {
                var equipConf = conf_mgr.gConfEquip[equipArr[k].id];
            ...
            for (var i = 0; i < conf_mgr.gEquipRefineItemsLowToHigh.length; i++) {
                if (needExp > 0) {
                    var id = conf_mgr.gEquipRefineItemsLowToHigh[i].id;
                    var cnt = Math.ceil(needExp / conf_mgr.gEquipRefineItemsLowToHigh[i].exp);
                    ...
                    if (realCostNum > 0) {
                        cost.push(['material', conf_mgr.gEquipRefineItemsLowToHigh[i].id, -realCostNum]);
                        needExp = needExp - realCostNum * conf_mgr.gEquipRefineItemsLowToHigh[i].exp;
                        addExp += realCostNum * conf_mgr.gEquipRefineItemsLowToHigh[i].exp;
...
exports.improve = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        var equipConf = conf_mgr.gConfEquip[equipObj.id];
        var costEquipConf = conf_mgr.gConfEquip[costEquipObj.id];
        ...
        // 检查是否可以改良
        var equipImproveConf = conf_mgr.gConfEquipImprove[equipConf.quality][equipObj.grade];
        if (!equipImproveConf) {
        ...
        // todo fish
        // equitInfoToWorld(user);
            logic_event_mgr.emit(logic_event_mgr.EVENT.DO_DAILY_TASK, player, 'statueImprove');
    } while (false);
