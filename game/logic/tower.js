...
const { pubKey, ServerName, TEAM_NAME } = require('../../common/enum.js');
...
exports.before_fight = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        if (floor == conf_mgr.gMaxFloorInTower) {
...
exports.fight = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        // 是否已经通关
        if (nextFloor > conf_mgr.gMaxFloorInTower) {
            resp.code = 1;
        ...
        // 等级判断
        if (user.status.level < conf_mgr.gConfTower[nextFloor].level) {
            resp.code = 1;
        ...
        if (star > 0) {
            ...
            if (tower.cur_floor > tower.top_floor) {
                ...
                req.args.max_floor = conf_mgr.gMaxFloorInTower;
                requestWorld(req, resp);
                // client_send_msg(ServerName.WORLD, req.act, req.mod, req.args, resp)
            }

            var conf = conf_mgr.gConfTower[tower.cur_floor];
...
exports.reset_num = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        // 判断是否还是重置次数
        var allResetNum = conf_mgr.gConfVip[user.status.vip].towerAgainTimes;
        ...
        var cost = conf_mgr.gConfBuy[resetNum].towerReset;
        ...
        logic_event_mgr.emit(logic_event_mgr.EVENT.DO_DAILY_TASK, player, 'tower', 1);
    } while (false);
...
exports.get_effort = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        var effort = conf_mgr.gConfTowerresultaward[id] || 0;
        // 奖励不存在
        if (!effort) {
...
function calcFloor(player, start_floor, start_time, max_floor) {
    ...
    do {
        ...
        for (var i = targetFloor; i <= Object.keys(conf_mgr.gConfTower).max(); i++) {
            if (targetFloor > max_floor) break;
            passTime -= conf_mgr.gConfTower[i].costTime;
            if (passTime < 0) {
                targetFloor = i - 1;
                break;
            }// 如果小于0当前这一层不算
            ...
            targetFloor = i;
            awards.push(conf_mgr.gConfTower[i].award[0]);
            awards.push(conf_mgr.gConfTower[i].award[1]);
            if (conf_mgr.gConfTower[i].box > 0) {
                var runeAwards = getRuneDropAward(player);
                awards = awards.concat(runeAwards);
            }
        }
    } while (false);
...
/** 获取当前掉落符文奖励 */
function getRuneDropAward(player) {
    var user = player.user;

    var awards = [];
    if (user.tower_server.rune_new_drop_count < user.tower_server.rune_new_drop_tab.length) {        // 新手掉落还没掉完
        ...
        player.markDirty('tower_server.rune_drop_count');

        if (user.tower_server.rune_drop_count >= user.tower_server.rune_drop_tab.length) {            // 这一轮周期已经走完，需要重置
            player.resetRuneDropTab();
        }
    }
...
function findDecomposeRunesByQuality(user, qualities, essence) {
    ...
    for (var rid in bag.rune) {
        ...
        var runeConf = conf_mgr.gConfRuneConf[rune.id];
        if (!runeConf) {
            continue;
        }
...
function decomposeOneRune(user, rid) {
    ...
    for (var i = 1; i <= curLevel; i++) {
        consumeExp += Math.abs(conf_mgr.gConfRuneUpgradeConf[i]['cost' + runeQuality][0][2]);
    }
...
exports.upgrade_rune = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        // 是否到顶级
        if (runeObj.level >= Object.keys(conf_mgr.gConfRuneUpgradeConf).max()) {
            resp.code = 1; resp.desc = 'already max level'; break;
        }

        // 不能超过最高等级
        if (toLevel > Object.keys(conf_mgr.gConfRuneUpgradeConf).max()) {
            toLevel = Object.keys(conf_mgr.gConfRuneUpgradeConf).max();
        }
        ...
        for (var i = curLevel + 1; i <= toLevel; i++) {
            costs = costs.concat(conf_mgr.gConfRuneUpgradeConf[i]['cost' + runeQuality])
        }
