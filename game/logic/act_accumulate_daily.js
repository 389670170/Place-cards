...
function get_data(player) {
    ...
    var tPassDay = common.getDateDiff(getGameDate(), common.getDate(conf_mgr.gConfActivities[act_name].startTime)) + 1;                  // 本期第几天
    if (!tUserActInfo.open_day || tUserActInfo.open_day != conf_mgr.gConfActivities[act_name].startTime || user.activity[act_name].pass_day != tPassDay) {
...
exports.get_award = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        var config = conf_mgr.gConfAvAccumulateDaily[id];
        if (!config) {
            resp.code = 1;
