...
function get_time_info() {
    if (!open_list) {
        var tOpenList = conf_mgr.gConfGlobal["avOctopusOpen"].split("|");
        ...
        for (var i = 0; i < tOpenList.length; i++) {
            ...
            open_list.push({
                ...
                time: conf_mgr.gConfGlobal["avOctopusDay"]
            })
        }
    }
    ...
    var tWeekDayOffet = (tNowHour >= conf_mgr.gConfGlobal.resetHour) ? 0 : -1;                                                            // 重置时间前 按照前一天来计算 否则按当日来计算
    ...
    for (var i = 0; i < open_list.length; i++) {
        ...
        if (tWeekDayOffet < 0 && (tEndTime == 1)) {
            ...
            start_time = common.getTime(common.getDate((Date.zeroWeek() / 1000) - (7 * 24 * 60 * 60) - (1 * 24 * 60 * 60) + (tTimeInfo.week * 24 * 60 * 60))) + (conf_mgr.gConfGlobal.resetHour * 3600);     // 真实的开始时间
        }
        else {
            start_time = common.getTime(common.getDate((Date.zeroWeek() / 1000) - (1 * 24 * 60 * 60) + (tTimeInfo.week * 24 * 60 * 60))) + (conf_mgr.gConfGlobal.resetHour * 3600);     // 真实的开始时间
        }
        return {
    ...
    start_time = conf_mgr.gConfActivities[act_name].startTime;
    return {
        start_time: start_time,
        end_time: start_time + (conf_mgr.gConfGlobal["avOctopusDay"] * 24 * 60 * 60),
        start_week: 0,
        end_week: 0,
    }
}
...
function get_info(player, gen_over_call) {
    ...
    var onGetFightForce = function (fight_force) {
        ...
        if (fight_force > 0) {
            ...
            if (!tActInfo || tActInfo.start_time != start_time || !tActInfo.target_list || !tActInfo.target_list[1]) {
                ...
                tActInfo.stage = conf_mgr.gConfActivities[act_name].stage;   // 当前的configID
    ...
    if (!player.user.temp || !player.user.temp.max_fight_force) {
        player_team.getFightForce(player, TEAM_NAME.DEFAULT, 1, true, onGetFightForce);
    }
    else {
    ...
    var func = (info) => {
        ...
        do {
            ...
            if (player.memData.fight_start_time && info.start_time && player.memData.fight_start_time != info.start_time) {                                       // 上一个周期的一局  保留奖励，标记当前周期信息
                resp.data.awards = player.addAwards(conf_mgr.gConfOctopusAward[target].award, req.mod, req.act);
            ...
            if (star == 0) {  // 战斗失败
                ...
                switch (conf_mgr.gConfOctopus[target].isRecover) {
                    case 1:                                                         // 重置 参战者状态
