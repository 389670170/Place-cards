...
function get_data(player) {
    ...
    if (!tUserActInfo.open_day || tUserActInfo.open_day != conf_mgr.gConfActivities[act_name].startTime) {
        user.activity[act_name] = get_default_act_data();
        user.activity[act_name].open_day = conf_mgr.gConfActivities[act_name].startTime;                    // 活动开启时间
    }
...
function on_pay(player, cash, chargeId, gift_key, gift_id, amount) {
    LOG(`${player.uid} onpay:${act_name} cash:${cash} chargeId:${chargeId}`);
    ...
    LOG(`${player.uid} onpay:${act_name} cash: ${cash} chargeId:${chargeId} now paid:${day_recharge.paid}`)
...
exports.get_info = function (player, req, resp, onHandled) {
    do {
        ...
        resp.data.accumulate_pay = tUserActInfo;
    } while (false);
...
exports.get_award = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        var config = conf_mgr.gConfAvAccumulatePay[id];
        if (!config) {
            resp.code = 1;
        ...
        resp.data.accumulate_pay = tActData;
    } while (false);
