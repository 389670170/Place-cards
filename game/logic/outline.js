...
function condi_check(player, cond_id) {
    ...
    var cond = conf_mgr.gConfOutlineCondition[cond_id];
...
function outline_check(player) {
    ...
    for (var id in conf_mgr.gConfOutlineTheme) {

        var theme = conf_mgr.gConfOutlineTheme[id];
        let k1 = theme.id;
    ...
    for (var id in conf_mgr.gConfOutlineDayBoss) {

        var boss = conf_mgr.gConfOutlineDayBoss[id];
        let k1 = boss.id;
...
function outline_make_rand(outline_rec) {
    ...
    for (var id in conf_mgr.gConfOutlineBeadChange) {
        var change = conf_mgr.gConfOutlineBeadChange[id];
...
exports.boss_fight_check = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        var dayboss = conf_mgr.gConfOutlineDayBoss[id];
        if (!dayboss) {
        ...
        if (boss.fight >= conf_mgr.gConfVip[vip].dragonBossDayLimit) {
...
exports.boss_fight = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        // 今日次数是否还有剩余
        var vip = player.user.status.vip;
        if (boss.fight_num >= conf_mgr.gConfVip[vip].dragonBossDayLimit) {
            resp.code = 1; resp.desc = 'fight times exceed'; break;
        }
...
exports.exchange_bead = function (player, req, resp, onHandled) {
    do {
        ...
        // 检测道具够不够
        var costs = parseAwardsConfig(conf_mgr.gConfGlobal.dragonBeadChangeNeed);
        ...
        var list = conf_mgr.gConfGlobal.dragonBeadLootSpecialList.toString().split('|');
        ...
        if (id == 0) {
            var keys = Object.keys(conf_mgr.gConfOutlineBeadChange);
            var idx = common.randRange(0, keys.length - 1);
            id = keys[idx];
        }

        var conf = conf_mgr.gConfOutlineBeadChange[id];
