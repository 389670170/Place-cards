...
var upgrade_list = null;

function get_upgrades() {
    if (upgrade_list) { return upgrade_list; }
    upgrade_list = [
        ...
        function limitExchangeFix(player) {
            if (!player.user.activity.limit_exchange.time) {
                player.user.activity.limit_exchange.time = conf_mgr.gConfActivities['limit_exchange'].time;
            }
    ...
    for (var i = 0; i < logic.mod_name_list.length; i++) {
        var tModName = logic.mod_name_list[i];
        if (!tModName) { continue; }
        if (!logic[tModName].upgrade) { continue; }
        upgrade_list.push(logic[tModName].upgrade);
    }
