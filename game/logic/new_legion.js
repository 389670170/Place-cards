...
const { pubKey, priKey, ServerName } = require('../../common/enum.js');
...
exports.create = function (player, req, resp, onHandled) {
    do {
        ...
        if (common.getTime() - user.new_legion.join_time < conf_mgr.gConfGlobal.legionJoinTimeLimit * 60 * 60) {
            resp.code = LEGION_ErrorCode.ERROR_LEGION_JOIN_CD; resp.desc = 'join_time error'; break;
        }

        // 验证name
        if (!name) {
            resp.code = LEGION_ErrorCode.ERROR_NAME_INVALID; resp.desc = 'name is error'; break;
        }
        ...
        var gSide = conf_mgr.gConfLegionFlag[1][side];
        var gPattern = conf_mgr.gConfLegionFlag[2][pattern];

        // 验证icons
        if (!gSide || !gPattern) {
            resp.code = LEGION_ErrorCode.ERROR_INVALID_ARGS; resp.desc = 'icons is error'; break;
        } else if (gSide.condition == 'legion_level' && gSide.value) {
            resp.code = LEGION_ErrorCode.ERROR_FLAG_SET_CONDITION; resp.desc = 'side legion_level is error'; break;
        } else if (gPattern.condition == 'legion_level' && gPattern.value) {
            resp.code = LEGION_ErrorCode.ERROR_TEXTURE_SET_CONDITION; resp.desc = 'pattern legion_level is error'; break;
        } else if (gSide.condition == 'vip_level' && gSide.value > user.status.vip) {
            resp.code = LEGION_ErrorCode.ERROR_FLAG_SET_CONDITION; resp.desc = 'side vip_level is error'; break;
        } else if (gPattern.condition == 'vip_level' && gPattern.value > user.status.vip) {
            resp.code = LEGION_ErrorCode.ERROR_TEXTURE_SET_CONDITION; resp.desc = 'pattern vip_level is error'; break;
        }

        var costs = parseAwardsConfig(conf_mgr.gConfGlobal.legionCreatCost);
        if (!player.checkCosts(costs)) {
            resp.code = LEGION_ErrorCode.ERROR_COST_NOT_ENOUGH; resp.desc = 'lack of resources'; break;
        }
...
exports.modify_name = function (player, req, resp, onHandled) {
    do {
        ...
        var costs = parseAwardsConfig(conf_mgr.gConfGlobal.legionNameChageCost);
        if (!player.checkCosts(costs)) {
            resp.code = LEGION_ErrorCode.ERROR_COST_NOT_ENOUGH; resp.desc = 'lack of resources'; break;
        }
...
exports.modify_notice = function (player, req, resp, onHandled) {
    do {
        ...
        if (!notice) {
            resp.code = LEGION_ErrorCode.ERROR_NOTICE_INVALID; resp.desc = 'notice is error'; break;
        }
...
exports.get_legions_list = function (player, req, resp, onHandled) {
    do {
        ...
        if (!isModuleOpen_level(player.user.status.level, 'legion')) {
            resp.code = LEGION_ErrorCode.ERROR_CAN_NOT_JOIN; resp.desc = 'level is not enough'; break;
        }
...
exports.build = function (player, req, resp, onHandled) {
    do {
        ...
        if (!conf_mgr.gConfLegionBuild[type]) {
            resp.code = LEGION_ErrorCode.ERROR_INVALID_ARGS; resp.desc = 'args is error'; break;
        }

        if (user.new_legion.build >= conf_mgr.gConfVip[user.status.vip].legionConstruct) {
            resp.code = LEGION_ErrorCode.ERROR_BUILD_COUNT_MAX; resp.desc = 'num is error'; break;
        }

        var costs = conf_mgr.gConfLegionBuild[type].cost;
        if (!player.checkCosts(costs)) {
            resp.code = LEGION_ErrorCode.ERROR_COST_NOT_ENOUGH; resp.desc = 'lack of resources'; break;
        }
...
exports.get_build_awards = function (player, req, resp, onHandled) {
    do {
        ...
        if (user.new_legion.build_awards[type]) {
            resp.code = LEGION_ErrorCode.ERROR_BUILD_AWARD_HAS_GOT; resp.desc = 'already awards'; break;
        }
...
exports.join_bonfire = function (player, req, resp, onHandled) {
    do {
        requestWorld(req, resp, function () {
            // client_send_msg(ServerName.WORLD, req.act, req.mod, req.args, resp, null, function () {
            if (resp.code == 0) {
                logic_event_mgr.emit(logic_event_mgr.EVENT.DO_DAILY_TASK, player, 'legionFire', 1);
                logic_event_mgr.emit(logic_event_mgr.EVENT.DO_TASK, player, 'legionFire', 1);
            }
            onHandled();
        })
        return;
...
exports.add_material = function (player, req, resp, onHandled) {
    do {
        ...
        var woodFire = '';
        var costs = parseAwardsConfig(conf_mgr.gConfGlobal['legionBonfire_operateCost' + type]);
        var confOpt = conf_mgr.gConfGlobal['legionBonfire_operate' + type].split('|');
        ...
        if (woodFire.num >= confOpt[0]) {
            if (type == 1) {
                resp.code = LEGION_ErrorCode.ERROR_ADD_WOOD_COUNT_MAX;
            } else {
                resp.code = LEGION_ErrorCode.ERROR_ADD_FIRE_COUNT_MAX;
            }
            resp.desc = 'num error'; break;
        } else if (common.getTime() - woodFire.time < confOpt[3] * 60) {
            if (type == 1) {
                resp.code = LEGION_ErrorCode.ERROR_ADD_WOOD_CD_TIME;
            } else {
                resp.code = LEGION_ErrorCode.ERROR_ADD_FIRE_CD_TIME;
            }
        ...
        requestWorld(req, resp, function () {
            // client_send_msg(ServerName.WORLD, req.act, req.mod, req.args, resp, null, function () {
            if (resp.code == 0) {
                var ConfBonFireLevel = conf_mgr.gConfLegionLevel[resp.data.level];
...
exports.grab_red_awards = function (player, req, resp, onHandled) {
    do {
        ...
        if (legion.red_awards[type] >= conf_mgr.gConfLegioBonfirerRedpaper[type].limit) {
            resp.code = LEGION_ErrorCode.ERROR_GRAB_RED_GIT_MAX; resp.desc = 'already up'; break;
        }
        ...
        requestWorld(req, resp, function () {
            ...
            if (resp.code == 0) {
                ...
                if (type.indexOf('level') > -1) {// 等级红包
                    awards = conf_mgr.gConfLegionLevel[level]['bonfireLevelRedPaper' + typeNum];
                } else {// 添柴红包
                    awards = conf_mgr.gConfLegionLevel[level]['bonfireOperateRedPaper' + typeNum];
                }
...
exports.boss_challenge = function (player, req, resp, onHandled) {
    ...
    do {
        // 检查挑战次数
        if (user.new_legion.boss.fight_count >= parseInt(conf_mgr.gConfGlobal.legionBossChallengeTimes)) {
            resp.code = LEGION_ErrorCode.ERROR_BOSS_FIGHT_COUNT_MAX; resp.desc = 'max count'; break;
        }
        ...
        if (curTime < bossTime[0] || curTime > bossTime[1]) {
            resp.code = LEGION_ErrorCode.ERROR_BOSS_NOT_FIGHT_TIME; resp.desc = 'not in fight time'; break;
        }

        var cost = conf_mgr.gConfBuy[user.new_legion.boss.fight_count + 1].legionBossC;
        if (!player.checkCosts(cost)) {
            resp.code = LEGION_ErrorCode.ERROR_COST_NOT_ENOUGH; resp.desc = 'cost not enough'; break;
        }
...
exports.boss_inspire = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        // 检查是否超过鼓舞次数
        if (user.new_legion.boss.inspire_count >= parseInt(conf_mgr.gConfGlobal.legionBossAakbTimes)) {
            resp.code = LEGION_ErrorCode.ERROR_INSPIRE_COUNT_MAX; resp.desc = 'max count'; break;
        }
        ...
        if (curTime < dailyResetTime || curTime >= endTime) {
            resp.code = LEGION_ErrorCode.ERROR_NOT_INSPIRE_TIME; resp.desc = 'not time'; break;
        }

        // 检查消耗够不够
        var cost = conf_mgr.gConfLegionBossAakb[type].cost;
        if (!player.checkCosts(cost)) {
            resp.code = LEGION_ErrorCode.ERROR_COST_NOT_ENOUGH; resp.desc = 'cost not enough'; break;
        }
        ...
        requestWorld(req, resp, function () {
            ...
            if (resp.code == 0) {
                ...
                var awards = conf_mgr.gConfLegionBossAakb[type].award;
                resp.data.awards = player.addAwards(awards, req.mod, req.act);
