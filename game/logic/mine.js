...
exports.get = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        var jadeId = conf_mgr.gConfJadeSeal.mine[0];
        var addPer = conf_mgr.gConfJadeSeal.mine[1];
        ...
        requestWorld(req, resp, function () {
            ...
            if (resp.code == 0) {
                ...
                if (resp.data.status == 'occupy_mine') {
                    var logConf = conf_mgr.gConfPlayLog['mine']['occupy'];
                    player.recordPlay(logConf.logType, logConf.logName);
                    logic_event_mgr.emit(logic_event_mgr.EVENT.DO_DAILY_TASK, player, 'mine', 1);
                }
            }
            onHandled();
        });
        ...
        requestWorld(req, resp, function () {
            ...
            if (resp.code == 0) {
                ...
                if (star > 0) {
                    ...
                    var logConf = conf_mgr.gConfPlayLog['mine']['rob'];
                    player.recordPlay(logConf.logType, logConf.logName);
                    logic_event_mgr.emit(logic_event_mgr.EVENT.DO_TASK, player, 'mineRob', 1);
