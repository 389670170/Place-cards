...
function get_data(player) {
    ...
    var tPassDay = common.getDateDiff(getGameDate(), getGameDate(common.GLOBAL_SERVER_INFO_DICT.serverStartTime)) + 1;
    for (var tKey in user.activity.assets_feed) {
        tKey = tKey - 0;
        var tActData = conf_mgr.gConfAvAssetsFeed[tKey]
        ...
        if (!conf_mgr.gConfAvAssetsFeedAward[tAssetsFeedData.get_stage]) {                               // 如果记录的id不在配置表中 代表配置表有修改，需要修订
...
function is_valid_gift(player, id) {
    var tActData = conf_mgr.gConfAvAssetsFeed[id]
...
function get_award_info(player, id, start_day) {
    ...
    var tArardInfoDict = conf_mgr.gConfAvAssetsFeedAward[start_day];
...
function get_buy_stage(passedDay) {
    passedDay = passedDay || common.getDateDiff(getGameDate(), getGameDate(common.GLOBAL_SERVER_INFO_DICT.serverStartTime)) + 1;

    if (!act_day_list) {
        act_day_list = [];
        for (var tKey in conf_mgr.gConfAvAssetsFeedAward) {
...
function on_pay(player, cash, chargeId, gift_key, gift_id) {
    ...
    var tGiftInfo = null;
    for (var tKey in conf_mgr.gConfAvAssetsFeed) {
        tKey = tKey - 0;
        if (conf_mgr.gConfAvAssetsFeed[tKey].rechargeid != tGiftName) { continue; }
        tGiftInfo = conf_mgr.gConfAvAssetsFeed[tKey]
        break;
    }
...
exports.get_award = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        tUserActInfo[id].last_get_day = common.getDateDiff(getGameDate(), getGameDate(common.GLOBAL_SERVER_INFO_DICT.serverStartTime)) + 1;
        player.markDirty('activity.assets_feed');
