...
function total_can_get_times(player) {
    var user = player.user;
    var times = common.getDateDiff(common.getDate(common.getTime() - (conf_mgr.gConfGlobal.resetHour * 3600)), common.getDate(user.info.create - (conf_mgr.gConfGlobal.resetHour * 3600))) + 1;
...
exports.get_award = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        var tMaxKey = 1;
        for (var tKey in conf_mgr.gConfAVSignNew) {
        ...
        var config = conf_mgr.gConfAVSignNew[tRound][tID];
        if (!config) {
            resp.code = 1;
