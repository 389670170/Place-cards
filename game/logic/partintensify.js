...
function getPartLevelByExp(part, exp) {
    var partConf = conf_mgr.gConfEquipRefine[part];
    if (!partConf) {
        return 0;
    }

    var maxCount = Object.keys(conf_mgr.gConfEquipRefine[part]).length;
...
exports.addPartExp = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        // 检查是否已经到顶级
        if (curPartLevel != 0 && !conf_mgr.gConfEquipRefine[partPos][curPartLevel]) {
        ...
        var itemConf = conf_mgr.gConfItem[itemId];
        if (!itemConf) {
...
exports.addPartExp_all = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        if (newExp > conf_mgr.gConfEquipRefine[partPos][curPartLevel + 1].exp) {
            if (exp != newExp - conf_mgr.gConfEquipRefine[partPos][curPartLevel + 1].exp) {
