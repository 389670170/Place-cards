...
exports.get = function (player, req, resp, onHandled) {
    ...
    do {
        var openTime = common.GLOBAL_SERVER_INFO_DICT.worldWarOpenTime;
        ...
        var fightForce = player_team.getFightForce(player, TEAM_NAME);
        ...
        requestWorldWar(worldWarRequest, resp, function () {
            if (resp.code == 0) {
                ...
                logic_event_mgr.emit(logic_event_mgr.EVENT.DO_DAILY_TASK, player, 'hegemony', 1);
            }
            onHandled();
        });
        return;
...
exports.challenge = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        if (!isCash && worldwar.battle >= +conf_mgr.gConfGlobal['worldwarBattleLimitPerDay']) {
        ...
        if (isCash) {
            var costs = parseAwardsConfig(conf_mgr.gConfGlobal['worldwarBuyBattle']);
...
exports.fight = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        if (player.memData.use_cash) {
            costs = parseAwardsConfig(conf_mgr.gConfGlobal['worldwarBuyBattle']);
            if (!player.checkCosts(costs)) {
                resp.code = 1; resp.desc = 'not enough cash'; break;
            }
        }
        ...
        requestWorldWar(req, resp, function () {
            if (resp.code == 0) {
                ...
                var id = star > 0 ? 2 : 1;
                resp.data.awards = player.addAwards(generateDrop(conf_mgr.gConfWorldWarBattle[id].dropId, user.status.level), req.mod, req.act);
            }
            onHandled();
        });
...
exports.support = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        var worldWarRequest = {
            ...
            args: {
                id: supportUid,
                gold: conf_mgr.gConfLevel[user.status.level].goldAward,
            }
        };
        requestWorldWar(worldWarRequest, resp, function () {
            if (resp.code == 0) {
                var awards = parseAwardsConfig(conf_mgr.gConfGlobal.worldwarSupportAward);
...
exports.get_exploit_award = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        if (!conf_mgr.gConfExploitWall[type] || !conf_mgr.gConfExploitWall[type][target]) {
            resp.code = 1; resp.desc = 'type or target error'; break;
        }

        requestWorldWar(req, resp, function () {
            if (resp.code == 0) {
                resp.data.awards = player.addAwards(conf_mgr.gConfExploitWall[type][target].awards, req.mod, req.act);
            }
...
exports.worship = function (player, req, resp, onHandled) {
    ...
    do {
        requestWorldWar(req, resp, function () {
            if (resp.code == 0) {
                var awards = parseAwardsConfig(conf_mgr.gConfGlobal.worldwarWorshipAward);
                resp.data.awards = player.addAwards(awards, req.mod, req.act);
