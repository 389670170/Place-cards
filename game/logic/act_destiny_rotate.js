...
const { parseAwardsConfig, parseAwardsConfigByCount } = require('../../common/global.js');
...
function get_data(player) {
    ...
    if (!tUserActInfo || tUserActInfo.open_day != conf_mgr.gConfActivities[act_name].startTime) {
        user.activity[act_name] = get_default_act_data();
        user.activity[act_name].open_day = conf_mgr.gConfActivities[act_name].startTime;
    }
...
function player_rotate(player, act_info) {
    ...
    var tSmallRotate = get_rotate_result(conf_mgr.gConfAvDestinyRotateNormal[act_info.small_stage], act_info.small_got, tLargeCost);       // 小转盘
    if (tSmallRotate) {
    ...
    if (tSmallRotate && tSmallRotate.has_special) {
        tLargeRotate = get_rotate_result(conf_mgr.gConfAvDestinyRotateHigh[act_info.large_stage], act_info.large_got);                // 大转盘
    }
    if (tLargeRotate) {
...
exports.buy_token = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        var cost = parseAwardsConfigByCount(conf_mgr.gConfGlobal["fatewheelBuyHero_expNumCost"], tBuyTimes);
        ...
        var tAddItemList = [];
        tAddItemList = tAddItemList.concat(parseAwardsConfigByCount(conf_mgr.gConfGlobal["fatewheelBuyHero_expNum"], tBuyTimes));
        tAddItemList = tAddItemList.concat(parseAwardsConfigByCount(conf_mgr.gConfGlobal["fatewheelBuyFortuneCoinNum"], tBuyTimes));
...
exports.rotate = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        if (!conf_mgr.gConfAvDestinyRotateHigh[tUserActInfo.large_stage]) {
            resp.code = 1;
            resp.desc = 'invalid type';
            break;
        }

        var cost = conf_mgr.gConfAvDestinyRotateCost[tUserActInfo.small_stage];
        if (!cost) {
            resp.code = 1;
            resp.desc = 'invalid type';
            break;
