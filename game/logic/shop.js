...
const { OneDayTime, OneWeekTime } = require('../../common/enum.js');
...
function getShopRefreshTime(id, tabId, last_refresh_time) {
    ...
    var tabConf = conf_mgr.gConfShopTab[tabId];
    if (!tabConf) {
        return 0;
    }
    ...
    if (tabConf.type == 'serverReset') {
        // 服务器重启重置
        resetTime = common.SERVER_START_TIME;
    } else if (tabConf.type == 'dayReset') {
        // 每天重置
        var dailyResetHour = conf_mgr.gConfGlobal.resetHour;
...
exports.refresh = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        var shopConf = conf_mgr.gConfShopNew[shopId];
        if (!shopConf) {
        ...
        if (typeof tabConf.freeResetTime == 'number') {
            ...
            freeResetTime = conf_mgr.gConfVip[user.status.vip][val];
        }

        // 英雄商店
        if (tabId == 201) {
            var tequanAdd = task_mod.getPrivilegeVal(player, 'godShopRefresh');
        ...
        if (typeof +tabConf.resetLimit == 'number') {
            ...
            limitCount = conf_mgr.gConfVip[user.status.vip][key];
        }
    ...
    do {
        ...
        if (goodsConf.timesLimit != 0) {
            ...
            if (typeof timesLimit == 'number') {
                ...
                if (arr[0] == 'vip') {
                    upNum = conf_mgr.gConfVip[user.status.vip][arr[1]];
                } else {
                    upNum = conf_mgr.gConfLevel[user.status.level][arr[1]];
                }
            }
        ...
        if (goodsConf.showLimit != 0) {
            ...
            if (limitType == 'vip') {
                ...
                // 开服天数
                var openDay = common.getDateDiff(getGameDate(), getGameDate(common.GLOBAL_SERVER_INFO_DICT.serverStartTime));
                openDay++;
        ...
        if (goodsConf.buyCost) {
            //costs = conf_mgr.gConfBuy[buyCount][goodsConf.buyCost]
            var hasBuyCount = shopTab.goods[goodsIndex].buy;
            for (var i = 0; i < buyCount; i++) {
                var buyTime = hasBuyCount + i + 1;
                var tempCosts = clone(conf_mgr.gConfBuy[buyTime][goodsConf.buyCost]);
                if (tempCosts) {
        ...
        if (tabId == 101) {
            logic_event_mgr.emit(logic_event_mgr.EVENT.DO_DAILY_TASK, player, 'shopBuy', buyCount);
