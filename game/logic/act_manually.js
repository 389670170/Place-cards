...
const act_name = 'manually';

// 获取数据
function get_data(player) {
    var user = player.user;
    if (user.activity.manually && !user.activity.manually.startTime) {
        user.activity.manually.startTime = conf_mgr.gConfActivities["manually"].startTime
    }

    if (
        !user.activity.manually                                                     // 没有参与过
        ||
        user.activity.manually.stage != conf_mgr.gConfActivities["manually"].stage           // 参与的不是本期
        ||
        user.activity.manually.startTime != conf_mgr.gConfActivities["manually"].startTime           // 参与的不是本期
    ) {
        user.activity.manually = {                            // 龙纹手册
            "now_exp": 0,                       // 当前经验值
            "is_unlock": 0,                     // 0-未解锁额外奖励 1-已解锁额外奖励
            "stage": conf_mgr.gConfActivities["manually"].stage,
            "startTime": conf_mgr.gConfActivities["manually"].startTime,
            "rewards": {                        // 基础奖励信息
...
function on_get_mail(player, mail) {
    if (common.getTime() < conf_mgr.gConfActivities["manually"].startTime) {
    ...
    var passedDay = common.getDateDiff(getGameDate(mail.time), getGameDate(common.GLOBAL_SERVER_INFO_DICT.serverStartTime)) + 1;
...
function on_pay(player, cash, chargeId, gift_key, gift_id) {
    ...
    switch (chargeId) {
        case conf_mgr.gConfGlobal.avmanuallyRechargeId:
            var data = get_data(player);
            data.is_unlock = 1;
...
on_task_event = function (player, event, value, compar_mod) {
    ...
    do {
        ...
        for (var tKey in conf_mgr.gConfAvmanuallyTask) {
            var tTaskInfo = conf_mgr.gConfAvmanuallyTask[tKey];
...
exports.get_manually_reward = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        var id = req.args.id;
        var conf = conf_mgr.gConfAvmanuallyAward[id];
        if (!conf) {
            resp.code = 1;
        ...
        if (conf.limitLevel > ~~(user.activity.manually.now_exp / conf_mgr.gConfGlobal.avmanuallyGradeExp)) {
            resp.code = 1;
...
exports.buy_manually_lv = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        var id = req.args.id;
        var conf = conf_mgr.gConfAvmanuallyLevel[id];
        ...
        var data = get_data(player);
        data.now_exp = (data.now_exp || 0) + (conf.manualLevel * conf_mgr.gConfGlobal.avmanuallyGradeExp);
...
exports.has_manually_reward = function (player) {
    ...
    do {
        ...
        var tManuallyLv = ~~(user.activity.manually.now_exp % conf_mgr.gConfGlobal.avmanuallyGradeExp);
        var tPlayerManuallyInfo = user.activity.manually;
        for (var tKey in conf_mgr.gConfAvmanuallyAward) {
            if (!conf_mgr.gConfAvmanuallyAward[tKey]) { continue; }
...
//     // function hero_additional_attribute(player, hero, team) {
//     return {};
// }
/**---------------------------------------------------------------------------------------------------------**/