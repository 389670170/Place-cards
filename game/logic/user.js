...
const sign_high = require('./sign_high.js');
...
const { ServerName, OneDayTime, TEAM_NAME } = require('../../common/enum.js');
const { client_send_msg } = require('../../common/net/ws_client.js');
const { requestWorld } = require('../../common/server.js');
const { parseAwardsConfig, getLoginData, isModuleOpen_new } = require('../../common/global.js');
...
exports.login = function (player, req, resp, onHandled) {
    ...
    //         // client_send_msg(ServerName.WORLD, "getWorldInfo", req.mod, reqget.args, respget, null, function () {
    //         if (respget.code == 0) {
    //             //更新world数据
...
function login(player, req, resp, onHandled) {
    ...
    if (isActivityStart(player, 'head_reset')) {
        ...
        if (headReset.time != conf_mgr.gConfActivities['head_reset'].startTime) {
            headReset.time = conf_mgr.gConfActivities['head_reset'].startTime;
    ...
    if (!user.tips['daily_task']) {
        ...
        for (var id in dailyTask) {
            ...
            if (dailyTask[id] >= conf_mgr.gConfDailyTask[id].target && !user.task.daily_reward[id]) {
                DEBUG('daily_task id = ' + id + ', dailyTask[id] = ' + dailyTask[id] + ', target = ' + conf_mgr.gConfDailyTask[id].target);
    ...
    if (!user.tips['world_reward']) {
        ...
        for (var id in conf_mgr.gConfWorldSituation) {
            if (user.battle.progress >= conf_mgr.gConfWorldSituation[id].target && !reward[id]) {
    ...
    if (isActivityStart(player, 'exchange_points')) {
        ...
        var startTime = conf_mgr.gConfAvExchangePointsTime[1].startTime;
        var endTime = conf_mgr.gConfAvExchangePointsTime[1].endTime;
        ...
        for (var id in conf_mgr.gConfAvExchangePointsTime) {
            if (nowTime >= conf_mgr.gConfAvExchangePointsTime[id].startTime && nowTime <= conf_mgr.gConfAvExchangePointsTime[id].endTime) {
                var round = id;
            }
        }
        ...
        if (!user.tips['exchange_points']) {
            ...
            for (var key in exchangePoint) {
                var confPoints = conf_mgr.gConfAvExchangePointsRound[round][key];
    ...
    if (user.mark.guide) {
        ...
        var customConf = conf_mgr.gConfCustomSet[nextProgress];
    ...
    player_team.getFightForce(player, TEAM_NAME.DEFAULT, 0, true);
    ...
    resp.data.server_open_time = common.GLOBAL_SERVER_INFO_DICT.serverStartTime;
    resp.data.worldwar_open_time = common.GLOBAL_SERVER_INFO_DICT.worldWarOpenTime;
    ...
    for (var tKey in logic) {
        ...
        if (tLogic.get_login_data) {                                 // 获取功能相关的数据
            var tLoginData = tLogic.get_login_data(player);
            if (tLoginData) {
                resp.data.tips[tLoginData.key] = tLoginData.login_data;
            }
        }

        if (tLogic.get_tips) {                                 // 获取功能相关的提示内容
            var tTipInfo = tLogic.get_tips(player);
            if (tTipInfo) {
                resp.data.tips[tTipInfo.key] = tTipInfo.tips;
            }
        }
    }
    ...
    var num = +id;
    for (var id in conf_mgr.gConfAvFirstPay) {
    ...
    //         if (!user.tips['legion']) {
    //             if (user.new_legion.build < conf_mgr.gConfVip[user.status.vip].legionConstruct) {
    //                 user.tips['legion'] = 1;
    //             }
    //         }
    ...
    //         // 活动配置
    //         resp.data.avconf = conf_mgr.gConfActivitiesClient;
...
exports.set_name = function (player, req, resp, onHandled) {
    do {
        ...
        if (req.args.cash && !user.mark.rename && user.status.cash < Math.abs(conf_mgr.gConfGlobal.renameCashCost[0][2])) {
        ...
        requestWorld(req, resp, function () {
            // client_send_msg(ServerName.WORLD, req.act, req.mod, req.args, msg_resp, null, function () {
            if (msg_resp.code == 0) {
                if (!info.un) {
                    // 进入主场景
                    var phpReq = {
                ...
                if (req.args.cash && !user.mark.rename) {
                    var costs = parseAwardsConfig(conf_mgr.gConfGlobal.renameCashCost);
...
exports.set_headpic = function (player, req, resp, onHandled) {
    ...
    do {
        var id = +req.args.id;
        var conf = conf_mgr.gConfHeadpic[id]
        if (!conf) {
...
exports.get_head_frame_status = function (player, req, resp, onHandled) {
    ...
    resp.data.progress = getHeadFrameProgress(player);

    onHandled();
};

function getHeadFrameProgress(player) {
    var progress = {};
    for (var i = 1; i <= Object.keys(conf_mgr.gConfHeadFrame).length; i++) {
        var conf = conf_mgr.gConfHeadFrame[i];
        if (conf) {
            if (conf.condition == 'user_level') {
                progress[conf.id] = player.user.status.level;
            } else if (conf.condition == 'player') {
                progress[conf.id] = player.getQuality();
            } else if (conf.condition == 'vip_level') {
                progress[conf.id] = player.user.status.vip;
            }
        }
    }
...
exports.offline = function (player, req, resp, onHandled) {
    onOffline(player, req.args.online_time);
...
exports.sign = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        // } else if (conf_mgr.gConfSignReward[sign.continuous + 1]) {
        //     sign.continuous++;
        //     player.markDirty('sign.continuous');
        // }
        // var conf = conf_mgr.gConfSign[sign.count];
        // var awards = [];
        // if (conf.monthly) {
        //     var curMonth = Math.floor(today / 100) % 100;
        //     awards = conf_mgr.gConfSignMonth[curMonth]['award' + conf.monthly];
        // } else {
        //     awards = conf.award;
        // }
        //
...
exports.resign = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        // 检测vip等级是否达到
        if (user.status.vip < conf_mgr.gConfGlobal.resignVipRequire) {
        ...
        //var costs = [['user', 'cash', -conf_mgr.gConfGlobal.resignCashCost]];
        // 检测补签花费
        var costs = parseAwardsConfig(conf_mgr.gConfGlobal.resignCashCost);
...
exports.get_vip_reward = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        var conf = conf_mgr.gConfVip[vip];
        if (!conf) {
        ...
        var costs = conf_mgr.gConfVip[vip].curPrice;
        if (!player.checkCosts(costs)) {
...
exports.get_enemy = function (player, req, resp, onHandled) {
    ...
    if (!from || from == ServerName.WORLD) {
        requestWorld(req, resp, onHandled);
        // client_send_msg(ServerName.WORLD, req.act, req.mod, req.args, resp, null, onHandled);
    } else if (from == 'universe' || from == ServerName.WORLD_WAR) {
        requestWorldWar(req, resp, onHandled);
    } else if (from == ServerName.ARENA) {
        requestArenaServer(req, resp, onHandled);
    } else if (from == ServerName.TEAM_ZONE) {
        requestTeamZone(req, resp, onHandled);
    } else if (from == ServerName.LAND_GRABBER) {
        requestLandGrabber(req, resp, onHandled);
    } else if (from == ServerName.COUNTRY_WAR) {
        requestCountryWar(req, resp, onHandled);
    } else if (from == ServerName.LEGION_WAR) {
...
exports.mark_guide = function (player, req, resp, onHandled) {
    ...
    if (guide[request] == 0) {
        ...
        // resp.data.awards = player.addAwards(conf_mgr.gConfSpecialReward[request].reward,req.mod,req.act);

        var retAwards = {};
        retAwards.awards = conf_mgr.gConfSpecialReward[request].reward;
...
exports.test_sys_mail = function (player, req, resp, onHandled) {
    requestWorld(req, resp, function () {
        // client_send_msg(ServerName.WORLD, req.act, req.mod, req.args, resp, function () {
        onHandled();
    });
};
...
exports.get_statue_awards = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        var cdTime = conf_mgr.gConfGlobal.teamManorStatueAwardCD * 3600;
        ...
        var dropId = conf_mgr.gConfLevel[level].teamStatueAward;
        var awards = generateDrop(dropId);

        resp.data.time = time;
        resp.data.awards = player.addAwards(awards, 'user', 'get_awards_statue');

        logic_event_mgr.emit(logic_event_mgr.EVENT.DO_DAILY_TASK, player, 'statueAward');
    } while (false);
...
exports.get_world_chat_count = function (player, req, resp, onHandled) {
    var user = player.user;
    do {
        var dayWorldChatMaxTime = conf_mgr.gConfLevel[user.status.level].worldChatCount;
...
exports.update_fight_force = function (player, req, resp, onHandled) {
    ...
    player_team.getFightForce(player, TEAM_NAME.DEFAULT);

    onHandled();
};

function onOffline(player, online_time) {
    var user = player.user;
    var now = common.getTime();
    ...
    player.markDirty('mark.total_online_time');
};