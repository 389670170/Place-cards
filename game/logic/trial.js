...
const TrialExploreMaxCount = 27;   // 探索最大次数
...
exports.init_user_data = function (initUser) {
    // 重新生成随机数据
    var indexArr = [];
    ...
    var maxCnt = Object.keys(conf_mgr.gConfLegionTrialAdventure).length;
    for (var i = 1; i <= maxCnt; i++) {
        var conf = conf_mgr.gConfLegionTrialAdventure[i];
...
// 重置试炼探索随机数据
exports.resetAdventureRand = function (player) {
    player.user.adventure_rand = {};

    // 重新生成随机数据
    var indexArr = [];
    for (var i = 1; i <= TrialExploreMaxCount; i++) {
        indexArr[i] = i;
        player.user.adventure_rand[i] = 0;
    }
    ...
    for (var i = 1; i <= maxCnt; i++) {
        ...
        for (var j = 1; j <= count; j++) {
            var newIndex = common.randRange(1, indexArr.length - 1);
            var realIndex = indexArr[newIndex];
            player.user.adventure_rand[realIndex] = i;
            indexArr.remove(realIndex);
        }
    }

    player.markDirty('adventure_rand');
},
...
function explore_once(user, trialData, resp, explore_count) {
    ...
    if (adventure_type > 0) {
        ...
        if (adventure_type == AdventureType.monster) {
            ...
            var maxIndex = Object.keys(conf_mgr.gConfLegionTrialGoods[param1]).length;
        ...
        adventureObj.time = common.getTime() + conf_mgr.gConfLegionTrialAdventure[adventure_type].duration * 3600;
...
exports.reset_explore_coin = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        var maxCount = Object.keys(conf_mgr.gConfBuy).length;
        var times = trialData.round[round].reset_count + 1;
        if (times >= maxCount) {
            times = maxCount;
        }

        var costs = conf_mgr.gConfBuy[times].trialCoinReset;
...
function getAwardFactor(round, trialData) {
    ...
    for (var i = 0; i < TrialExploreCondition.length; i++) {
        ...
        if (reach) {
            var addFactor = conf_mgr.gConfLegionTrialCoinIncreaseType[TrialExploreCondition[i][3]].awardIncrease;
    ...
    if (allDiff) {
        var addFactor = conf_mgr.gConfLegionTrialCoinIncreaseType[4].awardIncrease;
        if (!addFactor || addFactor < 0) {
            addFactor = 0;
        }
        factor += addFactor;
...
exports.get_explore_award = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        logic_event_mgr.emit(logic_event_mgr.EVENT.DO_DAILY_TASK, player, 'uncharted', 1);
...
exports.get_achievement_award = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        if (!conf_mgr.gConfLegionTrialAchievement[achievementId] ||
            !conf_mgr.gConfLegionTrialAchievement[achievementId][index]) {
            resp.code = 1; resp.desc = 'args error'; break;
        }
        ...
        // 检查成就是否已达成
        var needNum = conf_mgr.gConfLegionTrialAchievement[achievementId][index].needIntegral;
        ...
        // 发奖
        var awards = conf_mgr.gConfLegionTrialAchievement[achievementId][index].award;
...
exports.buy_shop_item = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        var conf = conf_mgr.gConfLegionTrialGoods[trialData.adventure[index].param1][trialData.adventure[index].param2];
        if (!conf) {
...
exports.reset_monster_star = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        var costs = conf_mgr.gConfBuy[times].trialCrackpotReset;
        if (!player.checkCosts(costs)) {
            resp.code = 1; resp.desc = 'cash not enough'; break;
        }
