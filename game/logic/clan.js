...
exports.create = function (player, req, resp, onHandled) {
    ...
    do {
        if (!isModuleOpen_new(player, 'team')) {
            resp.code = Team_ErrorCode.ERROR_TEAM_NOT_OPEN; resp.desc = 'not open'; break;
        }
        ...
        // 检查战队名字是否为空
        if (!name || nameLen == 0) {
            resp.code = Team_ErrorCode.ERROR_TEAM_NAME_EMPTY; resp.desc = 'name is empty'; break;
        }

        if (nameLen > 12) {
            resp.code = Team_ErrorCode.ERROR_TEAM_NAME_TOO_LONE; resp.desc = 'name too long'; break;
        }

        var cost = parseAwardsConfig(conf_mgr.gConfGlobal.teamCreatCost);
        if (!player.checkCosts(cost)) {
            resp.code = Team_ErrorCode.ERROR_CREATE_COSTS_NOT_ENOUGH; resp.desc = 'lack of resources'; break;
        }

        //var un = user.info.un;
        //if (!un){
        //    resp.code = Team_ErrorCode.ERROR_NICKNAME_IS_EMPTY; resp.desc = 'un is null';break;
        //}
...
exports.modify_bulletin = function (player, req, resp, onHandled) {
    do {
        ...
        if (!bulletinName || nameLen == 0) {
            resp.code = Team_ErrorCode.ERROR_BULLETIN_IS_EMPTY; resp.desc = 'not null'; break;
        }

        if (nameLen > 48) {
            resp.code = Team_ErrorCode.ERROR_BULLETIN_TOO_LONE; resp.desc = 'bulletinName too long'; break;
        }
...
exports.modify_name = function (player, req, resp, onHandled) {
    do {
        ...
        var cost = parseAwardsConfig(conf_mgr.gConfGlobal.teamNameChageCost);
        if (!player.checkCosts(cost)) {
            resp.code = Team_ErrorCode.ERROR_MODIFY_NAME_COSTS_NOT_ENOUGH; resp.desc = 'lack of resources'; break;
        }
...
exports.join_team = function (player, req, resp, onHandled) {
    do {
        ...
        //if (!un){
        //    resp.code = Team_ErrorCode.ERROR_NICKNAME_IS_EMPTY; resp.desc = 'not un';break;
        //}
    ...
    do {
        requestWorld(req, resp, function () {
            ...
            if (resp.data && resp.data.team) {
                ...
                logic_event_mgr.emit(logic_event_mgr.EVENT.DO_TASK, player, 'teamLevel', 1);
...
exports.get_daily_awards = function (player, req, resp, onHandled) {
    do {
        ...
        var clan = user.clan;
        if (clan.get_award) {
            resp.code = Team_ErrorCode.ERROR_ALREADY_GOT_AWARD; resp.desc = 'already get'; break;
        }
    ...
    do {
        ...
        for (var i in refreshTask) {
            ...
            if (taskBoard.length == 0) {
                ...
                if (taskBoard[1] > 0 && (curTime - taskBoard[1] >= conf_mgr.gConfGlobal.teamTaskRefreshTime)) {
...
exports.is_refresh_task = function (player, req, resp, onHandled) {
    do {
        ...
        if (!taskArr[1] || common.getTime() - taskArr[1] < conf_mgr.gConfGlobal.teamTaskRefreshTime) {
...
exports.refresh_task = function (player, req, resp, onHandled) {
    do {
        ...
        // 达到当日激活任务上限
        // if (clan.task >= conf_mgr.gConfGlobal.teamTaskTimeLimit){
        //     resp.code = Team_ErrorCode.ERROR_DAILY_TASK_REACH_MAX; resp.desc = 'Daily limit has been reached';break;
        // }

        if (!taskArr[1]) {
            resp.code = 106; resp.desc = 'not refresh task'; break;
        }
        var cost = parseAwardsConfig(conf_mgr.gConfGlobal.teamTaskResetCost);
        if (!player.checkCosts(cost)) {
...
exports.reset_task = function (player, req, resp, onHandled) {
    do {
        ...
        if (!index) {
            resp.code = Team_ErrorCode.ERROR_NO_TASK_NEED_RESET; resp.desc = 'not reset'; break;
        }

        var cost = parseAwardsConfig(conf_mgr.gConfGlobal.teamTaskRefreshCost);
        if (!player.checkCosts(cost)) {
            resp.code = Team_ErrorCode.ERROR_TASK_RESET_COST_NOT_ENOUGH; resp.desc = 'lack of resources'; break;
        }
...
exports.active_task = function (player, req, resp, onHandled) {
    do {
        ...
        if (taskArr[1] > 0) {
            resp.code = Team_ErrorCode.ERROR_TASK_ALREADY_ACTIVE; resp.desc = 'already active'; break;
        }
        ...
        requestWorld(req, resp, function () {
            ...
            if (resp.code == 0) {
                ...
                logic_event_mgr.emit(logic_event_mgr.EVENT.DO_DAILY_TASK, player, 'teamTask', 1);
            }
            onHandled();
        });
        return;
...
exports.get_awards_box = function (player, req, resp, onHandled) {
    do {
        ...
        if (clan.task_award >= conf_mgr.gConfGlobal.teamTaskAwardLimit) {
            resp.code = Team_ErrorCode.ERROR_DAILY_AWARD_COUNT_MAX; resp.desc = 'award_box is capped'; break;
        }
