...
const { COUNTRYWAR_ErrorCode } = require('../../common/error.js');
...
function canEnterCountryWar(player, req, resp) {
    ...
    if (!isModuleOpen_new(player, 'countrywar')) {
        resp.code = COUNTRYWAR_ErrorCode.ERROR_COUNTRY_WAR_NOT_OPEN;
        resp.desc = 'not open';
        return false;
    }

    // 判断玩家等级
    //var needLevel = parseInt(conf_mgr.gConfCountryWarBase.joinWarLevelLimit.value);
    //if (player.user.status.level < needLevel) {
    //    resp.code = COUNTRYWAR_ErrorCode.ERROR_USER_LEVEL_NOT_ENOUGH;
    ...
    if (dayNum < parseInt(conf_mgr.gConfCountryWarBase.serverOpenDayLimit.value)) {
        resp.code = COUNTRYWAR_ErrorCode.ERROR_SERVER_OPEN_DAYS_NOT_ENOUGH;
    ...
    if (!isCountryWarOpenTime()) {
        resp.code = COUNTRYWAR_ErrorCode.ERROR_NOT_COUNTRY_WAR_TIME;
        resp.desc = 'not country war time';
        return false;
    }
...
function setCountryScore(player, score) {
    var user = player.user;
    user.status.country_score = score;
    player.markDirty('status.country_score');
};
...
exports.broadcast_call_of_duty = function (player, req, resp, onHandled) {
    ...
    // 检查玩家官职是否满足要求
    user = player.user;
    var userPos = conf_mgr.gConfPosition[user.info.position].position;
    if (userPos > parseInt(conf_mgr.gConfCountryWarBase.callNeedPosition.value)) {
        resp.code = COUNTRYWAR_ErrorCode.ERROR_COUNTRY_POSITION_NOT_ENOUGH;
    ...
    requestCountryWar(countryReq, countryResp, function () {
        if (countryResp.code == 0) {
            ...
            var costCashArray = conf_mgr.gConfCountryWarBase.callConsumeCash.value;
            ...
            if (!player.checkCosts(costs)) {
                resp.code = COUNTRYWAR_ErrorCode.ERROR_CASH_NOT_ENOUGH;
...
exports.back_to_city = function (player, req, resp, onHandled) {
    ...
    // 检查集结令是否满足
    var moneyStr = 'goods';
    var moneyNum = parseInt(conf_mgr.gConfCountryWarBase.backToCityConsume.value);
    if (moneyType == 1) {
        moneyStr = 'cash';
        moneyNum = parseInt(conf_mgr.gConfCountryWarBase.backToCityConsumeCash.value);
    }

    // 检查物资是否满足
    var costs = [['user', moneyStr, -moneyNum]];
    if (!player.checkCosts(costs)) {
        resp.code = COUNTRYWAR_ErrorCode.ERROR_GOODS_NOT_ENOUGH;
        if (moneyType == 1) {
            resp.code = COUNTRYWAR_ErrorCode.ERROR_CASH_NOT_ENOUGH;
        }
...
exports.fast_relive = function (player, req, resp, onHandled) {
    ...
    // 检查集结令是否满足
    var moneyStr = 'goods';
    var moneyNum = parseInt(conf_mgr.gConfCountryWarBase.reliveConsume.value);
    if (moneyType == 1) {
        moneyStr = 'cash';
        moneyNum = parseInt(conf_mgr.gConfCountryWarBase.reliveConsumeCash.value);
    }
...
exports.clear_fight_cool_time = function (player, req, resp, onHandled) {
    var cashNum = parseInt(conf_mgr.gConfCountryWarBase.battleDiffTimeCash.value);
    var costs = [['user', 'cash', -cashNum]];
