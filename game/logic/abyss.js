...
const { pubKey, priKey, TEAM_NAME } = require('../../common/enum.js');
...
/** 深渊世界玩家信息 */
function get_abyss_info(player) {
    var user = player.user;
    if (!user.abyss) {
        user.abyss = {};
    }
    if (!user.abyss.progress) {
        user.abyss.progress = {};
    }
    if (!user.abyss.reward_box) {
        user.abyss.reward_box = {};
    }
    return user.abyss;
};
...
function is_valid_progress(player, progress) {
    var abyss_info = get_abyss_info(player)

    if (abyss_info.progress[progress] == 3) {                                       // 已经三星通关
        return false;
    }

    if (is_special_progress(progress) && abyss_info.progress[progress]) {           // 旋涡关卡 已经通关
        return false
    }

    var region = ~~(progress / 100);
    var little_progress = ~~(progress % 100);
    if (100101 == progress) {                                         // 第一关 只要功能开启就可以打
        return isModuleOpen_new(player, 'abyss');
    }
    else if (little_progress == 0) {
        var tTotalStar = 0;
        for (var i = 0; i < MAX_LITTLE_PROGRESS; i++) {
            tTotalStar += abyss_info.progress[region * 100 + i + 1];
        }

        return (tTotalStar == (MAX_LITTLE_PROGRESS * 3));                                    // 旋涡关卡 直接判断上一个关卡是否全3星
    }
    else if (little_progress == 1) {
        return abyss_info.progress[((region - 1) * 100) + MAX_LITTLE_PROGRESS];              // 一般关卡 判断上一区域的最后一关是否通关
    }
    else {
        return (abyss_info.progress[(region * 100) + (little_progress - 1)] > 0);            // 不是旋涡关卡 直接判断上一个关卡是否已经通过
    }
    return false;
};
...
exports.get_info = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        resp.data.progress = abyss_info.progress;
        resp.data.reward_box = abyss_info.reward_box;
        player.markDirty('abyss');
    } while (false);
...
exports.before_fight = function (player, req, resp, onHandled) {
    var user = player.user;
    do {
        if (!isModuleOpen_new(player, 'abyss')) {
            resp.code = 1;
            resp.desc = 'not open';
            break;
        }
...
exports.fight = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        var serverSign = getBattleFightSign('abyss', req.uid, time, star, player.memData.rand_origin);
        DEBUG(`abyss fight: ${dec_sign} ${serverSign} ${sign}`)
        ...
        var abyss_info = get_abyss_info(player);
        if (star <= 0 && !abyss_info.progress[progress]) {
            abyss_info.progress[progress] = star;
            resp.data.progress = abyss_info.progress;
            player.markDirty("abyss");
            break;
        }
        ...
        if (abyss_info.progress[progress]) {                // 之前通关过
            abyss_info.progress[progress] = (abyss_info.progress[progress] > star) ? abyss_info.progress[progress] : star;
            player.markDirty("abyss");
            resp.data.progress = abyss_info.progress;
        ...
        abyss_info.progress[progress] = (abyss_info.progress[progress] > star) ? abyss_info.progress[progress] : star;
        player.markDirty("abyss");
        resp.data.awards = player.addAwards(conf_mgr.gConfAbyssCustom[progress].award, req.mod, req.act);
...
exports.get_award = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        var effort = conf_mgr.gConfAbyssBox[id];
        ...
        var abyss_info = get_abyss_info(player);
        if (abyss_info.reward_box[id]) {        // 已经领取
        ...
        for (var i = start_progress; i < (start_progress + MAX_LITTLE_PROGRESS); i++) {
            tTotalStarCount = tTotalStarCount + abyss_info.progress[start_progress];
        }
        ...
        abyss_info.reward_box[id] = 1;
        player.markDirty('abyss.reward_box');
        resp.data.reward_box = abyss_info.reward_box;
