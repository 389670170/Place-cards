...
function get_data(player) {
    ...
    tUserActInfo.target_time = tUserActInfo.target_time || (common.getTime(common.getDate(user.info.create - (conf_mgr.gConfGlobal.resetHour * 3600))) + conf_mgr.gConfGlobal.avLevelupReceiveTime * 3600 + conf_mgr.gConfGlobal.resetHour * 3600);
    if (tUserActInfo.can_get != 2 && (common.getTime() >= tUserActInfo.target_time)) {
        tUserActInfo.can_get = 1;
    }

    tUserActInfo.open_day = getGameDate();//conf_mgr.gConfActivities[act_name].startTime
...
function on_pay(player, cash, chargeId, gift_key, gift_id) {
    ...
    if (tGiftName != conf_mgr.gConfGlobal.avLevelupGmRechargeid) { return; }

    var tUserActInfo = get_data(player);
    tUserActInfo.can_get = 2;

    var awards = player.addAwards(parseAwardsConfig(conf_mgr.gConfGlobal.avLevelupRechargeReceiveaward));
...
exports.get_award = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        var tAwardList = parseAwardsConfig(conf_mgr.gConfGlobal.avLevelupReceiveaward);
        tUserActInfo.can_get = 2;
        player.markDirty('activity.level_up');
