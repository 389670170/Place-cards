...
exports.village_occupy = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        if (player.memData.village_id > 0) {
            resp.code = LANDGRABBER_ErrorCode.ERROR_HAS_OCCUPY_VILLAGE; resp.desc = 'already has village'; break;
        }
        ...
        // 检查资源是否足够
        var costs = parseAwardsConfig(conf_mgr.gConfGlobal.territoryWarTeamChallengeCost);
        if (!player.checkCosts(costs)) {
            resp.code = LANDGRABBER_ErrorCode.ERROR_OCCUPY_VILLAGE_COST_NOT_ENOUGH; resp.desc = 'cost not enough'; break;
        }
        ...
        if (player.memData.village_o_ts > 0) {
            if (now - player.memData.village_o_ts < 10) {
                resp.code = LANDGRABBER_ErrorCode.ERROR_OCCUPY_CONFLICT; resp.desc = 'time limited'; break;
            }
        }
...
exports.land_occupy = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        if (!player.isVillageReleased(village_id)) {
            resp.code = LANDGRABBER_ErrorCode.ERROR_VILLAGE_NOT_RELEASE; resp.desc = 'village not release'; break;
        }
        ...
        // 检查资源是否足够
        var costs = parseAwardsConfig(conf_mgr.gConfGlobal.territoryWarPersonalChallengeCost);
        if (!player.checkCosts(costs)) {
            resp.code = LANDGRABBER_ErrorCode.ERROR_OCCUPY_LAND_COST_NOT_ENOUGH; resp.desc = 'cost not enough'; break;
        }
        ...
        function callback(player, resp) {
            if (resp.code == 0) {
                ...
                if (!enemy || enemy == undefined) {
                    ...
                    // 直接占领
                    logic_event_mgr.emit(logic_event_mgr.EVENT.DO_DAILY_TASK, player, 'territorywar', 1);
