...
module.exports = function (player, mod, act, args, name, num, update_before, update_later) {
    ...
    if (conf_mgr.gConfLogIndex[modAct]) {
        logIndexConf = conf_mgr.gConfLogIndex[modAct];
    ...
    if (conf_mgr.gConfLog[modAct] && conf_mgr.gConfLog[modAct][index]) {
        logConf = conf_mgr.gConfLog[modAct][index];
    }

    // 特殊处理部分内容
    // 英雄商店
    if (modAct == 'shop_buy' && !conf_mgr.gConfLog[modAct][index]) {
        logConf = conf_mgr.gConfLog[modAct]['0'];
    }
    ...
    if (costName) {
        ...
        args = {
            ...
            type: name,
            typeName: conf_mgr.gConfGeneralText[conf_mgr.gConfUser[name].logName].text,
            costName: costName,
            costValue: num,
            number: number,
