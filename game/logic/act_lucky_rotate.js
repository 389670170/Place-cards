...
function update_svr_open_days() {
    ...
    var t2 = common.GLOBAL_SERVER_INFO_DICT.serverStartTime;
...
function get_activity_key(name) {
    return conf_mgr.gConfActivities[name].startTime;
}
...
function grid_refresh(conf, data, idx) {
    ...
    var items = conf_mgr.gConfAvLuckyRotateItem[gid];
    var filtered = [];
...
function get_data(player) {
    ...
    // fresh data
    for (let id in conf_mgr.gConfAvLuckyRotateBase) {

        let conf = conf_mgr.gConfAvLuckyRotateBase[id];
...
// 每周重置
exports.reset_by_week = function (player) {
    update_svr_open_days();
...
exports.get_info = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        let conf = conf_mgr.gConfAvLuckyRotateBase[type];
        if (!conf) {
...
exports.rotate = function (player, req, resp, onHandled) {
    ...
    do {
        ...
        requestArenaServer(req, resp, function () {
            ...
            for (var i = 0; i < time; i++) {
                ...
                if (data.power >= conf.pPower[1]) {
                    ...
                    for (var j = 0; j < GRID_NUM; j++) {
                        ...
                        var item = conf_mgr.gConfAvLuckyRotateItem[grid[0]][grid[1]];
            ...
            logic_event_mgr.emit(logic_event_mgr.EVENT.DO_TASK, player, 'doWheel', time);
