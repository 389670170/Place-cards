...
const { LEGIONWAR_ErrorCode, LEGIONWAR_StageType } = require('../../common/error.js');
...
LegionWar.prototype = {
        ...
        async.series([function (cb) { // 加载机器人
            ...
            gDBWorld.find({ '_id': 'legionwar' }).limit(1).next(function (err, doc) {
                if (doc) {
                    ...
                    _me.roundBeginTime = LegionWarCom.genStageBeginTime(LEGIONWAR_StageType.PREPARE);
                    _me.updateCount = 0;
    ...
    update: function () {
        ...
        if (this.updateCount > 2) {
            ...
            if (stageInfo.type == LEGIONWAR_StageType.PREPARE) {
                this.roundBeginTime = stageInfo.stageBegin;
            }

            if (this.updateCount > 3) {
                if (this.stage == LEGIONWAR_StageType.PREPARE && this.lastSyncTime < this.roundBeginTime) {
                    // 重新拉取军团数据
    ...
    switchStage: function (curStage, curId) {
        ...
        switch (this.stage) {
            case LEGIONWAR_StageType.INVALID:
                curStage = LEGIONWAR_StageType.IDLE;
                break;
            case LEGIONWAR_StageType.PREPARE:
                if (curStage != LEGIONWAR_StageType.FIGHT) {
                    curStage = LEGIONWAR_StageType.PREPARE;
                }
                break;
            case LEGIONWAR_StageType.FIGHT:
                if (curStage != LEGIONWAR_StageType.IDLE) {
                    curStage = LEGIONWAR_StageType.FIGHT;
                }
                break;
            case LEGIONWAR_StageType.IDLE:
                if (curStage != LEGIONWAR_StageType.PREPARE && curStage != LEGIONWAR_StageType.NOT_JOIN) {
                    curStage = LEGIONWAR_StageType.IDLE;
                }
                break;
            default:
                if (curStage != LEGIONWAR_StageType.PREPARE) {
                    curStage = LEGIONWAR_StageType.NOT_JOIN;
                }
        }
        ...
        async.series([
            function (cb) { // 切出当前阶段
                if (_me.stage == LEGIONWAR_StageType.FIGHT) {
                    // 切出战斗阶段
                    if (_me.step != 1) {
                        _me.round += 1;
                        _me.markDirty('round');
            ...
            function (cb) { // 切入新阶段
                ...
                } else if (curStage == LEGIONWAR_StageType.FIGHT) {
                    // 切入战斗时间
                    _me.endUpgradeCitiesBeforWar();
                    _me.checkRoundData(function (err) {
                        cb(err);
                    });
                } else if (curStage == LEGIONWAR_StageType.IDLE) {
                    // 切入休息时间
                    async.series([
    ...
    buildLegion: function (origin, enmey) {
        ...
        for (var uid in origin.members) {
            var member = {};
            member.city_buff_speed = conf_mgr.gConfGlobal.legionWarBuildVelocity;
        ...
        for (var cityIdx = 1; cityIdx <= 16; ++cityIdx) {
            var cityConf = conf_mgr.gConfLegionWarCity[cityIdx];
        ...
        async.series([function (cb) {
            ...
            for (var cid in legion.cities) {
                ...
                if (city.aliveArm > 0) {
                    ...
                    enemyScore += conf_mgr.gConfLegionWarCity[+cid].score;
                }
    ...
    autoAssignCityForce: function (lid, callback) {
        this.getLegion(lid, function (err, res) {
            if (err) callback(err);
                ...
                var dispatchNum = +(conf_mgr.gConfGlobal.legionWarDispatchNum);
            ...
            Storage.getRankList(function (err, ranklist) {
                if (err) cb(err);
                    ...
                    for (var lid in info) {
                        ...
                        if (legionInf.win) {
                            rankLegion.score = (rankLegion.score || 0) + (+conf_mgr.gConfGlobal.legionWarWinScore);
                        } else {
                            rankLegion.score = Math.max(0, (rankLegion.score || 0) - (+conf_mgr.gConfGlobal.legionWarLoseScore));
                        }
        ...
        Storage.getLegion(lid, function (err, conf) {
            if (err) callback(err);
                ...
                if (!_me.legion.hasOwnProperty(lid)) {
                    callback(LEGIONWAR_ErrorCode.ERROR_NO_SUCH_LEGION, 'NO SUCH LEGION');
                } else {
                    callback(null, {
                        conf: conf,
    ...
    registerServer: function (args, callback) {
        ...
        if ((!serverId || isNaN(serverId)) && serverId != 0) {
            callback(LEGIONWAR_ErrorCode.ERROR_INVALID_ARGS);
            return;
        }
    ...
    getMainPageInfo: function (lid, joined, callback) {
        if (this.round == 1 && (this.stage != 1 && this.stage != 2)) {
            // 第一轮还没开始，没有军团匹配数据
            callback(null, {
                code: LEGIONWAR_ErrorCode.ERROR_FIRST_ROUND_NOT_YET_START,
            });
        } else {
            if (this.stage == LEGIONWAR_StageType.NOT_JOIN) {
                // 休战时间
                callback(null, {
                    code: LEGIONWAR_ErrorCode.ERROR_STAGE_NOT_FIT,
                });
            } else {
                var _me = this;
    ...
    getBattlePageInfo: function (uid, lid, needGarrion, callback) {
        if (this.stage == LEGIONWAR_StageType.IDLE) {
            callback(LEGIONWAR_ErrorCode.ERROR_IDLE_CANNOT_ENTER_BATTLE);
            return;
        }

        // 只有在准备时间才会需要这个数据(军团长/副军团长调整玩家驻守位置)
        if (this.stage != LEGIONWAR_StageType.PREPARE) {
            needGarrion = false;
        }

        var _me = this, ownData = null, enmeyData = null;
        async.series([function (cb) { // 取本军团数据
            if (_me.stage == LEGIONWAR_StageType.PREPARE) {
    ...
    addLegionWarScoreByCityBuff: function (uid, buffExp) {
        var score = Math.floor(buffExp * conf_mgr.gConfGlobal.legionWarScoreAwardRatio);
        if (!score) {
            score = 0;
            return false;
        }
    ...
    getAccumulateLegionwar: function (uid, callback) {
        ...
        if (this.accumulate_award[uid].legionwar_score <= 0) {
            callback(LEGIONWAR_ErrorCode.ERROR_PLAYER_ALREADY_GOT_LEGIONWAR_SCORE);
            return;
        }
        var data = {};
        var awards = parseAwardsConfig(conf_mgr.gConfGlobal.legionWarBuildAward);
    ...
    calculateCityBuff: function (lid, city, force) {
        var cityConf = conf_mgr.gConfLegionWarCity[city];
        var legion = this.legion[lid];
    ...
    upgradeCityBuff: function (lid, city, uid, callback) {
        // 只有第一阶段才能增筑
        if (this.stage != LEGIONWAR_StageType.PREPARE) {
            callback(LEGIONWAR_ErrorCode.ERROR_STAGE_NOT_FIT);
            return;
        }
        ...
        // 增驻已满
        if (legionCity.buffLevel >= cityConf.maxBufLevel) {
            callback(LEGIONWAR_ErrorCode.ERROR_UPGRADE_CITYBUF_MAX);
            return;
        }

        var buffingCity = this.getUserBuffingCity(uid, lid);
        if (city == buffingCity) {
            callback(LEGIONWAR_ErrorCode.ERROR_PLAYER_ALREADY_UPGRADE_THIS_CITY);
            return;
        }
    ...
    cancelCityBuf: function (lid, uid, callback) {
        var buffingCity = this.getUserBuffingCity(uid, lid);
        if (buffingCity == -1) {
            callback(LEGIONWAR_ErrorCode.ERROR_PLAYER_NO_BUFF_CITY);
            return;
        }
        ...
        this.getLegion(lid, function (err, res) {
            if (err) callback(err);
                ...
                if (!res.conf.members.hasOwnProperty(uid)) {
                    callback(LEGIONWAR_ErrorCode.ERROR_PLAYER_NOT_IN_OWN_LEGION);
                    return;
                }
                ...
                if (legionGarrion.hasOwnProperty(uid)) {
                    ...
                    // 该玩家驻守次数已经用尽
                    if (garrionNum >= +(conf_mgr.gConfGlobal.legionWarDispatchNum)) {
                        callback(LEGIONWAR_ErrorCode.ERROR_PLAYER_GARRION_TIME_USE_UP);
                        return;
                    }
                }
                ...
                if (alreadyGarrionThisCity) {
                    callback(LEGIONWAR_ErrorCode.ERROR_PLAYER_ALREADY_GARRION_THIS_CITY);
                    return;
                }
                ...
                // 已经被驻守
                if (legionArm.uid != 0) {
                    callback(LEGIONWAR_ErrorCode.ERROR_CITY_ARM_ALREADY_GASSION);
                    return;
                }
    ...
    removeCityForce: function (lid, city, uid, callback) {
        ...
        this.getLegion(lid, function (err, res) {
            ...
            if (!playerRemoved) {
                callback(LEGIONWAR_ErrorCode.ERROR_PLAYER_NOT_GARRION_THIS_CITY);
                return;
            }
    ...
    useCard: function (lid, cardId, target, uid, callback) {
        ...
        if (cardConf['key'] == 'build') {
            ...
            var cityConf = conf_mgr.gConfLegionWarCity[targetCid];
                    ...
                    _me.getLegion(enemyLid, function (err, eres) {
                        if (err) callback(err);
                            ...
                            if (darkSlots.length == 0) {
                                callback(LEGIONWAR_ErrorCode.ERROR_ARM_IS_DEAD);
                                return;
                            }
                            ...
                            // 检查目标是否暗格
                            if (targetArm.type != 1) {
                                callback(LEGIONWAR_ErrorCode.ERROR_ARM_IS_NOT_DARK);
                                return;
                            }
                            ...
                            Storage.getLegionUsers(enemyLid, [targetArm.uid], function (err, users) {
                                if (err) callback(err);
                                    ...
                                    if (users.length == 0) {
                                        callback(LEGIONWAR_ErrorCode.ERROR_LEGION_DONT_HAVE_USER);
                                    } else {
                    ...
                    _me.getLegion(enemyLid, function (err, eres) {
                        if (err) callback(err);
                            ...
                            if (targetCity.canAttack) {
                                callback(LEGIONWAR_ErrorCode.ERROR_CITY_ALREADY_CAN_ATTACK);
                            }
    ...
    attackArm: function (lid, city, arm, attacker, cardId, callback) {
        ...
        if (this.stage != LEGIONWAR_StageType.FIGHT) {
            callback(LEGIONWAR_ErrorCode.ERROR_STAGE_NOT_FIT);
            return;
        }
            ...
            if (err) callback(err);
                ...
                _me.getLegion(enemyLid, function (err, eres) {
                    if (err) callback(err);
                        ...
                        if (battle) {
                            callback(LEGIONWAR_ErrorCode.ERROR_ARM_IS_UNDERATTACK);
                            return;
                        }
                        ...
                        if (!targetCity.canAttack) {
                            callback(LEGIONWAR_ErrorCode.ERROR_CITY_CAN_NOT_ATTACK);
                            return;
                        }
                        ...
                        // 城池效果
                        var attrs = conf_mgr.gConfLegionWarCity[city]['atts' + targetCity.buffLevel];
                        var values = conf_mgr.gConfLegionWarCity[city]['value' + targetCity.buffLevel];
    ...
    fight: function (lid, city, arm, attacker, star, power, callback) {
        ...
        _me.getLegion(lid, function (err, res) {
            if (err) callback(err);
                ...
                var battle = _me.getBattle(enemyLid, city, arm);
                if (!battle) {
                    callback(LEGIONWAR_ErrorCode.ERROR_BATTLE_TIMEOUT);
                    return;
                }

                // 检查是否攻击者
                if (battle.attacker != attacker) {
                    callback(LEGIONWAR_ErrorCode.ERROR_NOT_THE_ATTACKER);
                    return;
                }
                    ...
                    if (err) callback(err);
                        ...
                        if (report.win) {
                            ...
                            if (targetCity.aliveArm == 0) {
                                ...
                                // 给敌方加积分
                                attackerLegion.score += conf_mgr.gConfLegionWarCity[city].score;
            ...
            if (err) callback(err);
                ...
                _me.getLegion(enemyLid, function (err, eres) {
                    if (err) callback(err);
                        ...
                        // 判断城池是否已经挂了
                        if (targetCity.aliveArm > 0) {
                            callback(LEGIONWAR_ErrorCode.ERROR_CITY_IS_ALIVE);
                            return;
                        }
    ...
    checkTips: function (lid, uid, callback) {
            ...
            this.getLegion(lid, function (err, res) {
                if (err) callback(err);
                    ...
                    for (var uid in legionConf.members) {
                        ...
                        if (legionRuntime.garrison[uid] != +(conf_mgr.gConfGlobal.legionWarDispatchNum)) {
        ...
        } else if (this.stage == LEGIONWAR_StageType.FIGHT || this.stage == LEGIONWAR_StageType.IDLE) {
            // 战斗时间 或者 休息时间
            this.getLegion(lid, function (err, res) {
                if (err) callback(err);
                else {
