...
Legion.prototype = {
            ...
            if (doc) {
                ...
                for (var uid in doc.kicks) {
                    if (uid != 'wish') {
                        ...
                        if (leaveTime >= conf_mgr.gConfLegion.joinTimeLimit) {
    ...
    create: function (leaderId, name, icon) {
        ...
        var legion = {
            ...
            'notice': '',
            'type': 0,
            'level_limit': conf_mgr.gConfLegion.legionOpenLevel,
            'mark_day': 0,
            'members': {},
            'applicant_list': {},
            'gold_tree': {
        ...
        if (retainWish && (legion.level >= conf_mgr.gConfLegion.leigionWishOpenLevel)) {
    ...
    approve: function (lid, uid) {
        ...
        member.quality = conf_mgr.gConfHero[user.pos[1].hid].quality;
    ...
    kick: function (lid, uid) {
        ...
        var iconConf = conf_mgr.gConfLegionIcon[legion.icon];
    ...
    addXp: function (lid, xp) {
        ...
        var level = 1;
        while (conf_mgr.gConfLegionLevel[level + 1]) {
            if (legion.xp < conf_mgr.gConfLegionLevel[level].xp) {
                break;
            }
            level++;
        }
    ...
    addLog: function (lid, type, args1, args2, args3) {
        var legion = this.legions[lid];
        var log = [conf_mgr.gConfLegionLog[type].id, common.getTime()];
        ...
        legion.log.push(log);
        while (legion.log.length > conf_mgr.gConfLegion.legionLogLimit) {
            legion.log.shift();
        }
    ...
    resetLegionByDay: function (lid) {
        ...
        dayDiff = dayDiff > conf_mgr.gConfLegion.activeDayLimit ? conf_mgr.gConfLegion.activeDayLimit : dayDiff;
    ...
    sendKicksWishMail: function () {
        ...
        for (var uid in retainWish) {
            ...
            for (var id in retainWish[uid]) {
                ...
                var fidQuality = conf_mgr.gConfHero[fid].quality;
                var targetNum = conf_mgr.gConfLegionWishConf[fidQuality].wishFragmentMax;
                ...
                if (!count && hasGot != targetNum) {
                    count = 1;
                    var award = conf_mgr.gConfLegionWishAchievementKey['mail'].award;
    ...
    makeRetMembers: function (lid) {
        ...
        for (var uid in legion.members) {
            var user = gUserInfo.getUser(uid);
            var member = legion.members[uid];
            var heroCombatConf = conf_mgr.getHeroCombatConf(user.pos[1].hid);
            members[uid] = {
                'un': user.info.un,
    ...
    resetGoldTree: function (lid) {
        ...
        var now = common.getTime();
        var interval = 3600 * conf_mgr.gConfLegion.goldTreeResetInterval;
        var resetTime = Math.floor(now / interval) * interval;
        if (lastTime < resetTime) {
            goldTree.outcome = conf_mgr.gConfLegionLevel[legion.level].goldOutcome;
            goldTree.time = now;
    ...
    updateCopy: function (lid) {
        ...
        if (copy.time < getWeekResetTime()) {
            ...
            var formationConf = conf_mgr.gConfFormation[conf_mgr.gConfLegionCopy[1].formation1];
            for (var i = 1; i <= 9; i++) {
    ...
    do {
        ...
        if (lid) {
            ...
            if (legion.impeachmentTime && legion.members[legion.master]) {
                var timeLimit = conf_mgr.gConfLegion['legionHeadNotOnlineTimeLimit'] * 3600;
            ...
            var len = lids.length;
            var limit = conf_mgr.gConfLegion.legionListLimit;
...
// 弹劾
exports.impeachment = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
        ...
        if (common.getTime() - gUserInfo.getUser(legion.master).mark.active_time < conf_mgr.gConfLegion.legionHeadLeaveTimeLimit * 3600) {
...
exports.join = function (req, res, resp, onReqHandled) {
    do {
        ...
        if (userAppList.length > conf_mgr.gConfLegion.memberApplicantLimit) {
        ...
        var maxCount = conf_mgr.gConfLegionLevel[legion.level].memberMax;
        ...
        if (legion.type == 0) {
            ...
            if (appListCount >= conf_mgr.gConfLegion.legionApplicantLimit) {
                resp.code = 105; resp.desc = 'member full'; break;
            }
...
exports.approve = function (req, res, resp, onReqHandled) {
    do {
...
exports.reject = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
        var tarUid = req.args.uid;
...
exports.revoke_request = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
        var lid = req.args.lid;
...
exports.exit = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
        var lid = gLegion.userLegion[uid];
        var legion = gLegion.get(lid);
        ...
        } else if (legion.members[uid].time + conf_mgr.gConfLegion['legionExitTimeLimit'] > common.getTime()) {
...
// 踢人
exports.kick = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
        var tarUid = req.args.uid;
        ...
        if (member.active.splice(conf_mgr.gConfLegion.activeDayLimit - 2).sum()) {
            if (legion.xp < conf_mgr.gConfLegion.legionKickPlayerCostDonateValue) {
                resp.code = 1; resp.desc = 'permission denied'; break;
            }

            legion.xp -= conf_mgr.gConfLegion.legionKickPlayerCostDonateValue;
...
// 任命
exports.appoint = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
        var tarUid = req.args.uid;
        ...
        if (tarDuty == LegionDuty.DEPUTY && legion.deputy_count >= conf_mgr.gConfLegion.deputyLimit) {
            resp.code = 110; resp.desc = 'deputy full'; break;
        }

        if (tarDuty == LegionDuty.ADVISER && legion.adviser_count >= conf_mgr.gConfLegion.adviserLimit) {
...
// 军团设置
exports.setting = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
        ...
        var iconConf = conf_mgr.gConfLegionIcon[icon];
        if (iconConf.condition == 'vip_level') {
            var valid = false;
...
// 军团公告设置
exports.set_notice = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
...
exports.get_log = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
...
exports.invite = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
        var legion = gLegion.get(gLegion.userLegion[uid]);
        if (!legion) {
        ...
        resp.data.content = util.format(conf_mgr.gConfLocalText[54].text, legion.level, legion.name, legion.level_limit);
        resp.data.info = {
            lid: legion.lid,
            type: legion.type,
            level_limit: legion.level_limit,
...
exports.rank_list = function (req, res, resp, onReqHandled) {
    var lids = Object.keys(gLegion.legions).sort(function (l1, l2) {
        ...
        return l2 - l1;
    }).slice(0, conf_mgr.gConfGlobal.legionRankListLimit);
...
exports.rank_list = function (req, res, resp, onReqHandled) {
    ...
    var rankCnt = conf_mgr.gConfGlobal.legionRankListLimit;
...
exports.update_active = function (req, res, resp, onReqHandled) {
    ...
    if (lid) {
        ...
        legion.members[uid].active[conf_mgr.gConfLegion.activeDayLimit - 1] += active;
        gLegion.markDirty(lid, 'members');
    }
...
exports.gm_change_legion = function (req, res, resp, onReqHandled) {
    ...
    if (!lid) {
        ...
        if (toLevel != legion.level) {
            var levelConf = conf_mgr.gConfLegionLevel[toLevel - 1];
            if (levelConf) {
                legion.xp = 0;
                legion.level = 0;
...
exports.use_boon = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
        var legion = gLegion.get(gLegion.userLegion[uid]);
        if (!legion) {
        ...
        if (legion.level < conf_mgr.gConfLegion.legionBoonOpenLevel) {
...
exports.grab_boon = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
        var lid = gLegion.userLegion[uid];
        var legion = gLegion.get(lid);
...
exports.get_gold_tree = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
        var lid = gLegion.userLegion[uid];
        var legion = gLegion.get(lid);
        ...
        if (legion.level < conf_mgr.gConfLegion.legionGoldTreeOpenLevel) {
            resp.code = 1; resp.desc = 'low level'; break;
        }
...
exports.shake_tree = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
        var lid = gLegion.userLegion[uid];
        var legion = gLegion.get(lid);
...
exports.mercenary = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
        var lid = gLegion.userLegion[uid];
        var legion = gLegion.get(lid);
        ...
        if (legion.level < conf_mgr.gConfLegion.legionMercenaryOpenLevel) {
            resp.code = 1; resp.desc = 'low level'; break;
        }
...
exports.send_mercenary = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
...
exports.recall_mercenary = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
        var hid = req.args.hid;
        var lid = gLegion.userLegion[uid];
...
exports.get_mercenaries = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
        var lid = gLegion.userLegion[uid];
        var legion = gLegion.get(lid);
        ...
        if (!noLevel && legion.level < conf_mgr.gConfLegion.legionMercenaryOpenLevel) {
            resp.code = 1; resp.desc = 'low level'; break;
        }
        ...
        for (var owner in legion.mercenary) {
            ...
            var userLevel = userInfo.status.level;
            if (owner == uid || (gUserInfo.getUser(owner).status.level - userLevel >= conf_mgr.gConfLegion.legionmercenaryopenlevellimit)) {
                continue;
            }
...
exports.hire_mercenary = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
        var lid = gLegion.userLegion[uid];
        var legion = gLegion.get(lid);
        ...
        var noLevel = req.args.no_level;
        if (!noLevel && legion.level < conf_mgr.gConfLegion.legionTrialOpenLevel) {
            resp.code = 1; resp.desc = 'low level'; break;
        }
            ...
            for (var pos in ownerInfo.pos) {
                ...
                if (hid == posObj.hid) {
                    ...
                    mercenaries.push({
                        ...
                        attr: attrNoFate,
                        fight_force: calcFightForce(attrNoFate, conf_mgr.gConfAttPrCoEff[posObj.level]),
...
exports.trial_fight = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
        var lid = gLegion.userLegion[uid];
        var legion = gLegion.get(lid);
            ...
            for (var pos in ownerInfo.pos) {
                ...
                if (hid == posObj.hid) {
                    ...
                    if (mercenary[owner] && (hid in mercenary[owner])) {
                        mercenary[owner][hid] += conf_mgr.gConfLevel[posObj.level].mercenaryHireGold;
                        gLegion.markDirty(lid, 'mercenary');
                    }
                    break;
                }
...
exports.get_copy = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
        var lid = gLegion.userLegion[uid];
        if (!lid) {
        ...
        var legion = gLegion.get(lid);
        if (legion.level < conf_mgr.gConfLegion.legionCopyOpenLevel) {
...
exports.get_enemy = function (req, res, resp, onReqHandled) {
    do {
        ...
        if (!conf_mgr.gConfLegionCopy[gLegion.get(lid).copy.chapter]) {
...
exports.fight = function (req, res, resp, onReqHandled) {
    do {
        ...
        if (req.args.chapter == copy.chapter && req.args.progress == copy.progress) {
            var formation = conf_mgr.gConfLegionCopy[copy.chapter]['formation' + copy.progress];
            var formationConf = conf_mgr.gConfFormation[formation];
            var idx = 0;
            var isPass = true;
            ...
            var posIndex = 0;
            for (var i = 1; i <= 9; i++) {
                var monsterConf = conf_mgr.gConfMonster[formationConf['pos' + i]];
                if (monsterConf) {
                    var posDamage = 0;
            ...
            if (isPass) {
                ...
                if (!conf_mgr.gConfLegionCopy[copy.chapter]['formation' + (copy.progress + 1)]) {
                ...
                var chapterConf = conf_mgr.gConfLegionCopy[copy.chapter];
                if (chapterConf) {
                    var formationConf = conf_mgr.gConfFormation[chapterConf['formation' + copy.progress]];
                    copy.healths = [];
                    for (var i = 1; i <= 9; i++) {
                        var monsterId = formationConf['pos' + i];
                        if (monsterId) {
...
exports.get_damage_reward = function (req, res, resp, onReqHandled) {
    do {
        var lid = gLegion.userLegion[req.uid];
        if (!lid) {
            resp.code = 104; resp.desc = 'no legion'; break;
...
exports.get_castle = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var lid = req.args.lid;
        if (!lid) {
...
// 获取可侵略的城池列表
exports.get_legion_list = function (req, res, resp, onReqHandled) {
    ...
    do {
        ...
        var lids = common.randArrayWithNum(weights, conf_mgr.gConfGlobal.invadeLegionCount);
        var legions = [];
        for (var i = 0, len = lids.length; i < len; i++) {
...
exports.invade_castle = function (req, res, resp, onReqHandled) {
    do {
        var lid = req.args.lid;
...
// 开始战斗
exports.castle_before_fight = function (req, res, resp, onReqHandled) {
    do {
        var enemy = +req.args.enemy;
        var lid = +req.args.lid;
...
exports.castle_fight = function (req, res, resp, onReqHandled) {
    do {
...
exports.get_construct = function (req, res, resp, onReqHandled) {
    var lid = gLegion.userLegion[req.uid];
    if (!lid) {
...
exports.build_construct = function (req, res, resp, onReqHandled) {
    var lid = gLegion.userLegion[req.uid];
    if (!lid) {
        resp.code = 1; resp.desc = 'no legion'; onReqHandled(res, resp, 1); return;
    }
...
exports.gm_promote = function (req, res, resp, onReqHandled) {
    var uid = req.uid;
    var lid = gLegion.userLegion[uid];
    if (lid) {
        var legion = gLegion.get(lid);
...
exports.get_wish_list = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
        var lid = gLegion.userLegion[uid];
        if (!lid) {
        ...
        var legion = gLegion.get(lid);
        if (legion.level < conf_mgr.gConfLegion.leigionWishOpenLevel) {
            resp.code = 1; resp.desc = 'low level'; break;
        }
...
exports.wish = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
...
exports.wish_give = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
        ...
        if (givenLegion.level < conf_mgr.gConfLegion.leigionWishOpenLevel) {
            resp.code = 1; resp.desc = 'low level'; break;
        }
        ...
        var confGiven = conf_mgr.gConfHero[givenFragmentId];
        if (!confGiven) {
            resp.code = 103; resp.desc = 'args id error'; break;
        }

        var givenHeroQuality = confGiven.quality;
        var givenWishTarget = conf_mgr.gConfLegionWishConf[givenHeroQuality].wishFragmentMax;
        ...
        resp.data.awards = conf_mgr.gConfLegionWishConf[givenHeroQuality].award;
    } while (false);

    onReqHandled(res, resp, 1);
};

exports.get_wish_fragment = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
...
exports.get_wish_awards_message = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
        var lid = gLegion.userLegion[uid];
        if (!lid) {
...
exports.get_wish_log = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
        var lid = gLegion.userLegion[uid];
        if (!lid) {
