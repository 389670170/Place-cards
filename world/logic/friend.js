...
Friend.prototype = {
        ...
        for (var uid in this.messages) {
            for (var friendId in this.messages[uid]) {
                this.messages[uid][friendId].forEach(function (value, index, array) {
                    if (value[0] + conf_mgr.gConfGlobal.friendOverTime * 3600 < common.getTime()) {
    ...
    generateBaseDate: function (uid) {
        ...
        var combatConf = conf_mgr.getHeroCombatConf(userInfo.pos[1].rid);
    ...
    generateRankData: function (uid) {
        ...
        var combatConf = conf_mgr.getHeroCombatConf(userInfo.pos[1].hid);
        var lid = gNewLegion.userLegion[uid];
        var data = {
            'uid': uid,
            'vip': userInfo.status.vip,
...
exports.apply = function (req, res, resp, onReqHandled) {
    ...
    do {
        ...
        // 自己好友列表已满
        if (userCount >= conf_mgr.gConfGlobal.friendNumLimit) {
            status = 3; break;
        }

        // 对方好友申请已满
        var friendApplyCount = Object.keys(friendInfo.applied).length;
        if (friendApplyCount >= conf_mgr.gConfGlobal.friendApplyNumLimit) {
            status = 9; break;
        }

        var friendCount = Object.keys(friendInfo.friends).length;
        // 对方好友列表已满
        if (friendCount >= conf_mgr.gConfGlobal.friendNumLimit) {
            status = 4; break;
        }
...
// 一键申请
exports.apply_all = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
...
exports.applyByName = function (req, res, resp, onReqHandled) {
    // status: 0 正常 1 已经是好友 2 已经发送过申请 3 自己好友列表已满 4 对方好友列表已满 5 对方已经发送过申请
    var uid = +req.uid;
...
exports.handle_apply = function (req, res, resp, onReqHandled) {
    var uid = + req.uid;
    do {
...
// 一键同意所有申请
exports.agree_all_apply = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        resp.data.basedata = {};
        var friendIds = req.args.ids;
...
exports.remove_apply = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var userInfo = gFriend.getUser(uid);
...
// 一键删除所有申请
exports.remove_all_apply = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
...
// 删除好友
// 支持批量删除
exports.remove = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
...
exports.give_gift = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        ...
        var giftConf = parseAwardsConfig(conf_mgr.gConfGlobal.friendGift);
...
exports.giveall_gift = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var userInfo = gFriend.getUser(uid);
...
exports.deal_gift = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var userInfo = gFriend.getUser(uid);
        ...
        var userInfo = gFriend.getUser(uid);
        if (userInfo.dailygift_getnum >= conf_mgr.gConfGlobal.friendDailyGiftGetLimit) {
...
exports.get_gift = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var friendId = Math.floor(req.args.id);
        var userInfo = gFriend.getUser(uid);
...
exports.getall_gift = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var userInfo = gFriend.getUser(uid);
        if (userInfo.dailygift_getnum >= conf_mgr.gConfGlobal.friendDailyGiftGetLimit) {
        ...
        for (var friendId in userInfo.friends) {
            ...
            if (userGiftInfo.gift_receiveid) {
                ...
                getNum++;
                uids.push(parseInt(friendId));
                if (userInfo.dailyGiftGetNum >= conf_mgr.gConfGlobal.friendDailyGiftGetLimit) {
                    break;
                }
            }
        }
...
exports.get_user = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        ...
        for (var id in users) {
            ...
            if (userInfo.info.un == name) {
                ...
                resp.data.quality = conf_mgr.gConfHero[userInfo.pos[1].hid].quality;
...
exports.recommend = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var candidateIds = [];
        var users = gUserInfo.users;
        ...
        if (candidateIds.length < conf_mgr.gConfGlobal.friendRecommendCount) {
            for (var i = candidateIds.length; i < conf_mgr.gConfGlobal.friendRecommendCount; i++) {
        ...
        var recommend = [];
        var count = 0;
        for (var i = 0, len = candidateIds.length; i < len && count < conf_mgr.gConfGlobal.friendRecommendCount; i++) {
...
exports.friend_chat = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        ...
        if (gFriend.messages.mailnum >= conf_mgr.gConfGlobal.friendMessageMax) {
...
exports.get_message = function (req, res, resp, onReqHandled) {
    var uid = req.uid;
    do {
        var friendId = Math.floor(req.args.friendId);
        var userInfo = gFriend.getUser(uid);
...
exports.get_boss = function (req, res, resp, onReqHandled) {
    var uid = req.uid;
    var bossOwnerId = req.args.bossOwnerId;
    do {
        if (uid != bossOwnerId && !gFriend.checkFriend(uid, bossOwnerId)) {
