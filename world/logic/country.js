...
Country.create = function (callback) {
    ...
    var allCount = 0;
    for (var id in conf_mgr.gConfPosition) {
        allCount += conf_mgr.gConfPosition[id].count;
    }
    ...
    for (var i = 1; i <= CountryNum; i++) {
        for (var pos in conf_mgr.gConfPosition) {
            countryData.country[i].position[pos] = {};

            var count = conf_mgr.gConfPosition[pos].count;
...
Country.prototype = {
    ...
    getRobot: function (id, pos) {
        if (this.robots[id]) {
            ...
            var positionConf = conf_mgr.gConfPosition[pos];
    ...
    checkChallenge: function (cid, uid, enemy) {
        ...
        if (conf_mgr.gConfPosition[userPos].position - conf_mgr.gConfPosition[enemyPos].position > 1) {
            return 103;
        }
    ...
    addReport: function (uid, enemy, success, userPos, enemyPos, replay) {
        ...
        if (userReport.length > conf_mgr.gConfGlobal.countryReportCount) {
            userReport.shift();
        }
        if (isPlayer && enemyReport.length > conf_mgr.gConfGlobal.countryReportCount) {
            enemyReport.shift();
        }
    },
    ...
    getBalanceTime: function () {
        ...
        var hour = Math.floor(conf_mgr.gConfGlobal.countryBalanceTime);
        var mins = Math.floor((conf_mgr.gConfGlobal.countryBalanceTime % 1) * 60);
    ...
    setCountry: function (country, _uid) {
        if (!country) {
            ...
            for (var cid in this.country) {
                var sumFF = 0;
                for (var pos in conf_mgr.gConfPosition) {
                    if (conf_mgr.gConfPosition[pos].position > 3) {
                        continue;
                    }
    ...
    calcSalary: function () {
        ...
        for (var id in this.userPosition) {
            ...
            if (!isDroid(uid)) {
                ...
                var salary = conf_mgr.gConfPosition[position].salary;
...
exports.set_country = function (req, res, resp, onReqHandled) {
...
exports.get = function (req, res, resp, onReqHandled) {
    ...
    if (req.args.country) {
        ...
        var list = [];
        for (var pos in position) {
            if (conf_mgr.gConfPosition[pos].position > 2) {
                break;
            }
...
exports.get_city = function (req, res, resp, onReqHandled) {
    resp.data.position = gCountry.userPosition[req.uid];
...
exports.get_position = function (req, res, resp, onReqHandled) {
    do {
        var country = req.args.country;
    ...
    do {
        ...
        if (req.args.star > 0 && userPos > enemyPos) {
            ...
            if (enemyPos == 1) {
                ...
                array[0] = userName;
                array[1] = enemyName;
                array[2] = conf_mgr.gConfLocalText['STR_OFFICE_1'].Text;
    ...
    if (originReports) {
        for (var i = 0, len = originReports.length; i < len; i++) {
            ...
            if (isDroid(origin[2])) {
                ...
                var heroCombatConf = conf_mgr.getHeroCombatConf(enemy.pos[1].rid);
                if (!heroCombatConf) {
...
exports.rank_list = function (req, res, resp, onReqHandled) {
    ...
    for (var pos in position) {
        // 判断是否超出要求的数量
        if (ranks.length >= conf_mgr.gConfGlobal.rankListLimit_kingMe) break;
...
exports.fix_position = function (req, res, resp, onReqHandled) {
