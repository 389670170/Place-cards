...
function Tower() {
    this.ranks = new RankTree(                     // 层数排行榜
        ...
        function (c1, c2) {
            return judge(c1, c2);
        }, conf_mgr.gConfGlobal.towerRankListLimit);
    ...
    init: function (callback) {
        gDBWorld.find({ _id: 'tower' }).limit(1).next(function (err, doc) {
            if (doc) {
                ...
                /*doc.ranks = [];
                var rankCnt = conf_mgr.gConfGlobal.fightForceRankListLimit;
...
exports.rank_list = function (req, res, resp, onReqHandled) {
    do {
        ...
        gTower.ranks.each(function (data) {
            var floor = data[1];
            if (rankList.length < conf_mgr.gConfGlobal.rankListLimit_tower) {
