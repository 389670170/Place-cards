...
const { LEGIONWAR_StageType, LEGIONWAR_ErrorCode } = require('../../common/error.js');
        ...
        */},
        ...
        'user': {/*
            uid: {
                ...
                attackNum: +conf_mgr.gConfLegion.legionWarAttackNum,// 攻击次数
...
LegionWar.prototype = {
    init: function (callback) {
        gDBWorld.find({ '_id': 'legionwar' }).limit(1).next(function (err, doc) {
            if (doc) {
                ...
                // 本轮军团战开始时间
                this.roundBegin = LegionWarCom.genStageBeginTime(LEGIONWAR_StageType.PREPARE, true);
    ...
    getUserRuntime: function (uid) {
        if (!this.runtime.user.hasOwnProperty(uid)) {
            ...
            this.runtime.user[uid] = {
                card_use: {},
                cardEffects: [],
                attackNum: +conf_mgr.gConfLegion.legionWarAttackNum,
                cityAwardMark: {},
            };
    ...
    getMatchConditionLegions: function () {
        ...
        // 最小参战等级
        var minJoinLevel = +conf_mgr.gConfGlobal.legionWarMinJoinLevel;
        // 最少参战人数
        var minJoinMembers = +conf_mgr.gConfGlobal.legionWarMinJoinMembers;
        //服务器开发天数
        ...
        gNewLegion.enumationLegions(function (legion) {
            // 过滤不能参战的军团
            if (legion.level < minJoinLevel || Object.keys(legion.member_list).length < minJoinMembers || passedDay < conf_mgr.gConfGlobal.legionWarJoinDay) {
                return;
            }
            ...
            for (var uid in legion.member_list) {
                ...
                var heroCombatConf = conf_mgr.getHeroCombatConf(userInfo.pos[1].rid);
                members[uid] = {
                    uid: uid,
                    un: userInfo.info.un,
    ...
    registerServer: function (callback) {
        ...
        gNewLegion.enumationLegions(function (legion) {
            // 过滤不能参战的军团
            if (legion.level < minJoinLevel || legion.member_count < minJoinMembers || passedDay < conf_mgr.gConfGlobal.legionWarJoinDay) {
                return;
            }
    ...
    handleLegionResult: function (legionResult, oldRank, rankLegions, newRank, callback) {
        ...
        // 更新段位积分
        if (legionResult.win) {
            legionPersisit.score += +conf_mgr.gConfGlobal.legionWarWinScore;
            legionPersisit.winNum += 1;
        } else {
            legionPersisit.score = Math.max(0, legionPersisit.score - (+conf_mgr.gConfGlobal.legionWarLoseScore));
        }
        ...
        var checkLevel = (legionPersisit.level == 0)
            || (legionPersisit.score > conf_mgr.gConfLegionWarRank[legionPersisit.level].maxScore);
        if (checkLevel) {
            var nextLevel = legionPersisit.level + 1;
            for (; ;) {
                var nextConf = conf_mgr.gConfLegionWarRank[nextLevel];
                if (!nextConf) {
                    break;
                }
            ...
            if (newLevel != legionPersisit.level) {
                ...
                //     var awards = conf_mgr.gConfLegionWarRank[awardLevel].award;
        ...
        // 发送军团战结算奖励
        var rankConf = conf_mgr.gConfLegionWarRank[newLevel];
        ...
        async.series([function (cb) {
            ...
            if (err) {
                ...
                if (_me.stage == LEGIONWAR_StageType.PREPARE) {
                    data.stageEnd = LegionWarCom.genStageEndTime(LEGIONWAR_StageType.IDLE, true)
                }
                ...
                if (retObj.joined) {
                    ...
                    if (_me.stage == LEGIONWAR_StageType.PREPARE) {
                        retObj.stageEnd = LegionWarCom.genStageEndTime(LEGIONWAR_StageType.IDLE, true)
                    }
                }
    ...
    upgradeCityBuf: function (lid, uid, city, callback) {
        if (lid != gNewLegion.userLegion[uid]) {
            callback(LEGIONWAR_ErrorCode.ERROR_PLAYER_NOT_IN_OWN_LEGION, "not in legion");
            return;
        }
    ...
    useCard: function (lid, uid, cardId, target, callback) {
        ...
        if (!gNewLegion.joinBeforeTime(lid, uid, this.roundBegin)) {
            callback(LEGIONWAR_ErrorCode.ERROR_NEW_MEMBER_CAN_NOT_USE, 'New member can not do this');
            return;
        }

        var cardConf = conf_mgr.gConfLegionWarCityCard[cardId];
        if (cardConf['useTime'] != this.stage) {
            callback(LEGIONWAR_ErrorCode.ERROR_STAGE_NOT_FIT, 'Stage not fit');
            return;
        }
        ...
        if (cardConf['useNumLimit'] && usedCount >= cardConf['useNumLimit']) {
            callback(LEGIONWAR_ErrorCode.ERROR_CARD_USE_MAX, 'use count limit');
            return;
        }
    ...
    attackArm: function (lid, uid, city, arm, cardId, callback) {
        ...
        if (userRuntime.attackNum <= 0) {
            callback(LEGIONWAR_ErrorCode.ERROR_ATTACK_NUM_LIMIT, 'No attack num');
            return;
        }
    ...
    checkTips: function (uid, callback) {
        ...
        for (var fightN = 1; fightN <= fightNum; ++fightN) {
            if (!conf_mgr.gConfLegionWarAttackAward[fightN]) {
                continue;
            }
...
exports.get_main_page_info = function (req, res, resp, onReqHandled) {
...
exports.get_battle_page_info = function (req, res, resp, onReqHandled) {
    var lid = req.args.lid;
    var uid = req.uid;
...
exports.upgrade_citybuf = function (req, res, resp, onReqHandled) {
    var lid = req.args.lid;
    var city = req.args.city;
...
exports.cancel_citybuf = function (req, res, resp, onReqHandled) {
    var lid = req.args.lid;
...
exports.add_city_force = function (req, res, resp, onReqHandled) {
    var lid = req.args.lid;
    var city = req.args.city;
...
exports.remove_city_force = function (req, res, resp, onReqHandled) {
    var lid = req.args.lid;
    var city = req.args.city;
    var arm_uid = req.args.arm_uid;
...
exports.use_card = function (req, res, resp, onReqHandled) {
    var lid = req.args.lid;
    var cardId = req.args.card;
...
exports.attack_arm = function (req, res, resp, onReqHandled) {
    var lid = req.args.lid;
    var city = req.args.city;
    var arm = req.args.arm;
    var cardId = req.args.card;
...
exports.get_user_fightnum_info = function (req, res, resp, onReqHandled) {
    var lid = req.args.lid;
...
exports.get_user_fightnum_award = function (req, res, resp, onReqHandled) {
    var lid = req.args.lid;
...
exports.get_city_award = function (req, res, resp, onReqHandled) {
    var lid = req.args.lid;
    var city = req.args.city;
...
exports.get_history = function (req, res, resp, onReqHandled) {
    var lid = req.args.lid;
...
exports.get_server_ranklist = function (req, res, resp, onReqHandled) {
    var lid = req.args.lid;
...
exports.get_world_ranklist = function (req, res, resp, onReqHandled) {
    var lid = req.args.lid;
...
exports.get_rank_info = function (req, res, resp, onReqHandled) {
    var lid = req.args.lid;
...
// 获取符合要求的军团
exports.get_match_condition_legions = function (req, res, resp, onReqHandled) {
...
exports.handle_round_result = function (req, res, resp, onReqHandled) {
