...
function getOpenRankOverDate(name) {
    ...
    var tActivityInfo = conf_mgr.gConfActivitiesRaw[name];
    ...
    var tConfigDict = conf_mgr.gConfAvSchedule[name] || {};
    for (var round in tConfigDict) {
        var conf = tConfigDict[round];
        if (!conf) { continue; }

        if (passedDay >= conf.startDay && passedDay <= (conf.endDay + 1)) {
            return common.getDate(common.getTime(common.GLOBAL_SERVER_INFO_DICT.serverStartDate) + (conf.endDay + 1) * 24 * 60 * 60);
            // return (common.GLOBAL_SERVER_INFO_DICT.serverStartDate + conf.endDay + 1);
        }
    }
    ...
    return common.getDate(common.getTime(common.GLOBAL_SERVER_INFO_DICT.serverStartDate) + (conf_mgr.gConfActivitiesRaw[name].openDay + conf_mgr.gConfActivitiesRaw[name].duration) * 24 * 60 * 60);
}
...
Activity.prototype = {
    init: function (callback) {
        gDBWorld.find({ _id: 'activity' }).limit(1).next(function (err, doc) {
            if (doc) {
                ...
                if (!this.growFund) {
                    this.growFund = conf_mgr.gConfGrowFundBought[1].bought;
                    this.addUpdate('grow_fund', this.growFund);
                }

                function getOpenRankDate(name) {
                    return common.getDate(common.getTime(common.GLOBAL_SERVER_INFO_DICT.serverStartDate) + (conf_mgr.gConfActivitiesRaw[name].openDay + conf_mgr.gConfActivitiesRaw[name].duration) * 86400);
                }
    ...
    resetByDay: function () {
        ...
        if (this.open_rank) {
            if (this.open_rank.recharge_rank != conf_mgr.gConfActivities["open_rank_recharge"].endTime + conf_mgr.gConfActivities["open_rank_recharge"].delayDays) {
                this.open_rank.recharge_rank = conf_mgr.gConfActivities["open_rank_recharge"].endTime + conf_mgr.gConfActivities["open_rank_recharge"].delayDays;
                this.open_rank.final_recharge_rank = null;
            }
            if (this.open_rank.expense_rank != conf_mgr.gConfActivities["open_rank_expense"].endTime + conf_mgr.gConfActivities["open_rank_expense"].delayDays) {
                this.open_rank.expense_rank = conf_mgr.gConfActivities["open_rank_expense"].endTime + conf_mgr.gConfActivities["open_rank_expense"].delayDays;
                this.open_rank.final_expense_rank = null;
            }
            this.addUpdate('open_rank', this.open_rank);
        }
    ...
    addGrowFundBought: function () {
        ...
        var openDay = common.GLOBAL_SERVER_INFO_DICT.serverStartDate;
        var alreadyOpen = common.getDateDiff(openDay, today) + 1;
        if (!common.getDateDiff(openDay, today)) {
            return;
        }

        if (conf_mgr.gConfGrowFundBought[alreadyOpen]) {
            this.growFund += conf_mgr.gConfGrowFundBought[alreadyOpen].bought;
            this.addUpdate('grow_fund', this.growFund);
        }
    },
    ...
    addOpenRankMail: function (name) {
        ...
        if (mailId > 0) {
            ...
            var mail = {
                awards: name,
                time: time,
                expire: time + conf_mgr.gConfMail[mailId].time * OneDayTime,
                attach: {
                    ranks: ranks,
                    huge: 1,
                },
...
exports.get_openseven = function (req, res, resp, onReqHandled) {
    do {
...
exports.get_openseven_reward = function (req, res, resp, onReqHandled) {
    do {
        var id = req.args.id;
        if (!conf_mgr.gConfOpenSevenReward[id]) {
            resp.code = 101; resp.desc = 'id error'; break;
        }

        var limit = conf_mgr.gConfOpenSevenReward[id].target;
...
exports.get_buy_award = function (req, res, resp, onReqHandled) {
    do {
...
exports.buy_award = function (req, res, resp, onReqHandled) {
    do {
        var id = req.args.id;
...
exports.get_openholiday = function (req, res, resp, onReqHandled) {
    do {
...
exports.get_openholiday_reward = function (req, res, resp, onReqHandled) {
    do {
        var id = req.args.id;
        if (!conf_mgr.gConfOpenHolidayReward[id]) {
            resp.code = 101; resp.desc = 'id error'; break;
        }

        var limit = conf_mgr.gConfOpenHolidayReward[id].target;
...
exports.get_lucky_dragon = function (req, res, resp, onReqHandled) {
    var history = [];
...
exports.click_lucky_dragon = function (req, res, resp, onReqHandled) {
...
exports.get_grow_fund = function (req, res, resp, onReqHandled) {
    resp.data.count = gActivity.growFund;
    onReqHandled(res, resp, 1);
};

exports.get_grow_fund_award = function (req, res, resp, onReqHandled) {
    resp.data.count = gActivity.growFund;
    onReqHandled(res, resp, 1);
};

exports.buy_grow_fund = function (req, res, resp, onReqHandled) {
    gActivity.growFund += 1;
    gActivity.addUpdate('grow_fund', gActivity.growFund);
    onReqHandled(res, resp, 1);
};

exports.get_drops_dragon_award = function (req, res, resp, onReqHandled) {
    resp.data.get_count = gActivity.dropsDragon;
    onReqHandled(res, resp, 1);
};

exports.get_drops_dragon_award_aux = function (req, res, resp, onReqHandled) {
    onReqHandled(res, resp, 1);
};
...
exports.get_av_rank_level = function (req, res, resp, onReqHandled) {
    var rank_list = [];
    var self_rank = 0;
    var avRankConf = conf_mgr.gConfAvRankLevel;
...
exports.get_av_rank_fight_force = function (req, res, resp, onReqHandled) {
    var rank_list = [];
    var self_rank = 0;

    var avRankConf = conf_mgr.gConfAvRankFightForce;
...
exports.get_av_rank_fight_force_10 = function (req, res, resp, onReqHandled) {
    var rank_list = [];
    var self_rank = 0;

    var avRankConf = conf_mgr.gConfAvRankFightForce10;
...
exports.get_av_rank_fight_force_15 = function (req, res, resp, onReqHandled) {
    var rank_list = [];
    var self_rank = 0;

    var avRankConf = conf_mgr.gConfAvRankFightForce15;
...
function get_av_rank_recharge_config_list() {
    var avRankConf = conf_mgr.gConfAvRankRecharge;
    var rank_conf_list = [];
...
exports.get_av_rank_recharge_ranks = function (req, res, resp, onReqHandled) {
    var rank_list = [];
    var self_rank = 0;
...
function get_av_rank_expense_config_list() {
    var avRankConf = conf_mgr.gConfAvRankExpense;
    var rank_conf_list = [];
    for (var tKey in avRankConf) {
        rank_conf_list.push(avRankConf[tKey]);
    }
...
exports.get_av_rank_expense_ranks = function (req, res, resp, onReqHandled) {
    var rank_list = [];
    var self_rank = 0;
...
exports.reset_open_rank = function (req, res, resp, onReqHandled) {
