...
Mail.prototype = {
    ...
    add: function (uid, mail) {
        ...
        if (uid < 1000000) {
            ...
            // 删除多的邮件
            if (this.users[uid][2] > conf_mgr.gConfGlobal.mailMaxCount) {
...
SysMail.prototype = {
    ...
    getSysMailExtr: function (uid, mail, options) {
        ...
        if (mail.awards == 'arena_rank') {                   // 竞技场排名奖励
            ...
            function getArenaAwards(type, rank) {
                var fitIndex = null;
                if (arenaType >= 1 && arenaType <= 6) {
                    var max = Object.keys(conf_mgr.gConfArenaRank[arenaType]).max();
                    for (var i = 1; i < max; i++) {
                        if (arenaRank >= conf_mgr.gConfArenaRank[arenaType][i].rank && arenaRank < conf_mgr.gConfArenaRank[arenaType][i + 1].rank) {
                            fitIndex = i;
                            break;
                        }
                    }
                ...
                if (arenaType && fitIndex) {
                    award = conf_mgr.gConfArenaRank[arenaType][fitIndex].award;
                }
                return award;
            }
            ...
            var stages = Object.keys(conf_mgr.gConfWorldWarGlory);
            stages.sort(function (a, b) { return (+b) - (+a); });
            for (var i = 0, len = stages.length; i < len; i++) {
                if (rank >= +stages[i]) {
                    return { content: [12, rank], awards: conf_mgr.gConfWorldWarGlory[stages[i]].awards }
                }
            }
            ...
            var stages = Object.keys(conf_mgr.gConfWorldWarReward);
            stages.sort(function (a, b) { return (+b) - (+a) });
            for (var i = 0, len = stages.length; i < len; i++) {
                if (rank >= stages[i]) {
                    return { content: [13, rank], awards: conf_mgr.gConfWorldWarReward[stages[i]].awards };
                }
            }
            ...
            return { content: [14, chapter], awards: conf_mgr.gConfLegionCopy[chapter].reward };
        } else if (mail.awards == 'country') {                // 皇城每日结算奖励
            var position = mail.attach.position;
            ...
            var posConf = conf_mgr.gConfPosition[pos];
            if (!posConf)
                return null;
            ...
            var awards = conf_mgr.gConfLordCountrySalary[rank].awards;
            ...
            var award = null;
            var ranks = Object.keys(conf_mgr.gConfAvLuckyWheelRank).sort(function (a, b) { return a - b; });
            for (var i = 0, len = ranks.length; i < len; i++) {
                if (rank <= ranks[i]) {
                    award = conf_mgr.gConfAvLuckyWheelRank[ranks[i]].award;
                    break;
                }
            }
            ...
            var award = null;
            var ranks = Object.keys(conf_mgr.gConfPromoteRankRed).sort(function (a, b) { return a - b; });
            for (var i = 0, len = ranks.length; i < len; i++) {
                if (rank <= ranks[i]) {
                    award = conf_mgr.gConfPromoteRankRed[ranks[i]].award;
                    break;
                }
            }
            ...
            if (parseInt(mail.attach.lid) == lid) {
                ...
                for (var mineId in params) {
                    ...
                    var award = clone(conf_mgr.gConfTerritoryWarMapMine[mineId].award1);
            ...
            if (parseInt(mail.attach.lid) == lid) {
                ...
                var award = conf_mgr.gConfTerritoryBossLegionConf[legionLevel].award;
            ...
            if (mail_lids && mail_lids.indexOf(parseInt(lid)) >= 0) {
                ...
                var award = null;
                var rankMaxCount = Object.keys(conf_mgr.gConfTerritoryBossSelfAward).length;
                for (var i = 0; i < rankMaxCount; i++) {
                    var min = conf_mgr.gConfTerritoryBossSelfAward[i + 1].rank[0];
                    var max = conf_mgr.gConfTerritoryBossSelfAward[i + 1].rank[1];
                    if (!max && max != 0) {
                        max = min;
                    }
                    if (i + 1 == rankMaxCount) {
                    ...
                    if (rank >= min && rank <= max) {
                        award = conf_mgr.gConfTerritoryBossSelfAward[i + 1].award;
                        break;
                    }
                }
            ...
            var awardIndex = 1;
            for (var i = 1; i <= Object.keys(conf_mgr.gConfCountryWarPersonalRank).length; i++) {
                if (conf_mgr.gConfCountryWarPersonalRank[i.toString()].rank > rank) {
                    break;
                } else {
                    awardIndex = i;
                }
            ...
            var awards = conf_mgr.gConfCountryWarPersonalRank[awardIndex].awards;
            if (!awards)
                return null;
            ...
            var awardIndex = 1;
            var campAwardList = conf_mgr.gConfCountryWarCountryRank[campRank];
            ...
            var awards = conf_mgr.gConfCountryWarCountryRank[campRank][awardIndex].awards;
            if (!awards)
                return null;
            ...
            //var avConf = conf_mgr.gConfAvRank['fight_force'];
            var avConf = conf_mgr.gConfAvRankFightForce;
            var rank = mail.attach.ranks[uid];
            if (!rank) {
                return null;
            }
            ...
            //var avConf = conf_mgr.gConfAvRank['fight_force'];
            var avConf = conf_mgr.gConfAvRankFightForce10;
            var rank = mail.attach.ranks[uid];
            if (!rank) {
                return null;
            }
            ...
            //var avConf = conf_mgr.gConfAvRank['fight_force'];
            var avConf = conf_mgr.gConfAvRankFightForce15;
            var rank = mail.attach.ranks[uid];
            if (!rank) {
                return null;
            }
            ...
            //     var avConf = conf_mgr.gConfAvRank['arena'];
            //     var rank = mail.attach.ranks[uid];
            //     if (!rank) {
            //         return null;
            //     }
            ...
            var avConf = conf_mgr.gConfAvRankRecharge;
            var rank = mail.attach.ranks[uid];
            if (!rank) { return null; }
            ...
            var avConf = conf_mgr.gConfAvRankExpense;
            var rank = mail.attach.ranks[uid];
            if (!rank) { return null; }
            ...
            //var avConf = conf_mgr.gConfAvRank['level'];
            var avConf = conf_mgr.gConfAvRankLevel;
            var rank = mail.attach.ranks[uid];
            if (!rank) {
                return null;
            }
    ...
    addArenaAwardMail: function () {
        ...
        if (Object.keys(ranks).length > 0) {
            var mail = {
                awards: 'arena_rank',
                time: time,
                expire: time + conf_mgr.gConfMail[4].time * OneDayTime,
                attach: {
                    ranks: ranks,
                    levels: levels,
                    huge: 1,
    ...
    addBossScoreAwardMail: function () {
        ...
        var mail = {
            ...
            awards: 'boss_rank',
            time: time,
            expire: time + conf_mgr.gConfGlobal.awardMailExpireDay * 3600 * 24,
            attach: {
                ranks: ranks,
                huge: 1,
            },
    ...
    addWorldwarAwardMail: function (ranks) {
        if (conf_mgr.gConfMail[12]) {
            var time = common.getTime();
            var mail = {
                awards: 'ww_rank',
                time: time,
                expire: time + conf_mgr.gConfMail[12].time * OneDayTime,
                attach: {
                    ranks: ranks,
                    huge: 1,
                },
    ...
    addSupAwardMail: function (progress, supports) {
        ...
        var mail = {
            awards: 'ww_sup',
            time: time,
            expire: time + conf_mgr.gConfMail[11].time,
            attach: {
                progress: progress,
                supports: supports,
            },
    ...
    addPlayoffAwardMail: function (ranks) {
        ...
        var mail = {
            awards: 'ww_playoff',
            time: time,
            expire: time + conf_mgr.gConfMail[13].time * OneDayTime,
            attach: {
                ranks: ranks
            },
        };
    ...
    addLegionCopyAwardMail: function (lid) {
        ...
        var mail = {
            awards: 'legion_copy',
            time: time,
            expire: time + conf_mgr.gConfMail[14].time * OneDayTime,
            attach: {
                members: memberArr,
    ...
    addCountryAwardMail: function () {
        ...
        if (Object.keys(position).length > 0) {
            ...
            var mail = {
                awards: 'country',
                time: time,
                expire: time + conf_mgr.gConfMail[15].time * OneDayTime,
                attach: {
                    position: position,
                    salary: salary,
                    huge: 1,
    ...
    addLimitGroupAwardMail: function (goods, buyer) {
        ...
        var mail = {
            awards: 'limit_group',
            time: time,
            expire: time + conf_mgr.gConfMail[1004].time * OneDayTime,
            attach: {
                goods: goods,
                buyer: buyer,
                huge: 1,
    ...
    addLordAwardMail: function () {
        ...
        var mail = {
            awards: 'lord',
            time: time,
            expire: time + conf_mgr.gConfMail[9].time * OneDayTime,
            attach: {
                position: position,
                country: country,
                ranks: ranks,
    ...
    addLuckyWheelAwardMail: function (ranks) {
        ...
        var mail = {
            awards: 'lucky_wheel',
            time: time,
            expire: time + conf_mgr.gConfMail[1003].time * OneDayTime,
            attach: {
                ranks: attachRanks,
                huge: 1,
            },
    ...
    addPromoteWheelAwardMail: function (ranks) {
        ...
        var mail = {
            ...
            time: time,
            expire: time + conf_mgr.gConfMail[1009].time * OneDayTime,
            attach: {
                ranks: attachRanks,
                huge: 1,
            },
    ...
    addTaptapDailyAward: function (am) {
        ...
        var mail = {
            ...
            time: time,
            expire: time + conf_mgr.gConfMail[1011].time * OneDayTime,
            attach: {
                huge: 1,
            },
        };
...
exports.add_mail = function (req, res, resp, onReqHandled) {
    do {
...
exports.add_sys_mail = function (req, res, resp, onReqHandled) {
    do {
        var mail = req.args.mail;
...
exports.del_sys_mail = function (req, res, resp, onReqHandled) {
    do {
...
exports.read_mail = function (req, res, resp, onReqHandled) {
    do {
        var id = req.args.id;
...
// 一键领取
exports.read_all_mail = function (req, res, resp, onReqHandled) {
    var uid = req.uid;
    do {
        var user = gUserInfo.getUser(uid);
            ...
            if (mail.awards && typeof mail.awards != 'string') {
                ...
                if (result && result.awards.length) {
                    for (var i = 0, len = result.awards.length; i < len; i++) {
                        ...
                        if (user.status.food >= conf_mgr.gConfGlobal.foodMax && award[1] == 'food') {
                            DEBUG('food is full');
                            continue;
                        }
        ...
        for (var id in mails) {
            ...
            if (mail.awards && mail.awards.length) {
                for (var i = 0, max = mail.awards.length; i < max; i++) {
                    ...
                    if (!mail.title && !conf_mgr.gConfMail[mailId]) {
                        continue;
                    }
...
// 添加跨服战支持的奖励邮件
exports.add_sup_award_mails = function (req, res, resp, onReqHandled) {
    do {
...
// 添加跨服战积分赛每日的奖励邮件
exports.add_worldwar_award_mails = function (req, res, resp, onReqHandled) {
    do {
...
// 添加跨服战决赛的奖励邮件
exports.add_playoff_award_mails = function (req, res, resp, onReqHandled) {
    do {
        if (!req.args.ranks) {
            resp.code = 1; resp.desc = 'args error'; break;
        }
...
exports.add_limit_group_mails = function (req, res, resp, onReqHandled) {
    do {
...
exports.add_lucky_wheel_mails = function (req, res, resp, onReqHandled) {
    do {
        var ranks = req.args.ranks;
        if (!ranks) {
            resp.code = 1; resp.desc = 'args error'; break;
...
exports.add_promote_wheel_mails = function (req, res, resp, onReqHandled) {
    do {
        var ranks = req.args.ranks;
        if (!ranks) {
            resp.code = 1; resp.desc = 'args error'; break;
...
exports.add_upgrade_city_award_mail = function (req, res, resp, onReqHandled) {
    do {
        ...
        gMail.add(req.uid, {
            ...
            awards: awards,
            time: now,
            expire: now + conf_mgr.gConfMail[34].time * OneDayTime,
        });
    } while (false);
