...
LandGrabber.prototype = {
                ...
                for (var k in this.villages) {
                    ...
                    if (this.villages[k].team_id > 0) {
                        ...
                        if (teamData && this.teams[this.villages[k].team_id]) {
                            ...
                            this.villages[k].remain_res = conf_mgr.gConfTeamLand[k].resourceReserve[0][2];
    ...
    _init: function () {
        for (var k in conf_mgr.gConfTeamLand) {
            if (!this.villages[k]) {
                this.villages[k] = {};
                this.villages[k].team_id = 0;
                this.villages[k].time = 0;
                    ...
                    if (resEmpty) {
                        ...
                        for (var k in teamInfo.member_list) {
                            ...
                            if (awardsArr[memberUid] > 0) {
                                var awards = clone(conf_mgr.gConfTeamLand[villageId].resourceReserve);
                                awards[0][2] = Math.floor(awardsArr[memberUid]);
                                var time = common.getTime();
                                var mail = {
                                    awards: awards,
                                    time: time,
                                    expire: time + conf_mgr.gConfMail[42].time * OneDayTime,
    ...
    tickLand: function () {
        for (var k in this.lands) {
            ...
            for (var k1 in this.lands[villageId]) {
                if (!conf_mgr.gConfPersonalLand[villageId]) { continue; }
                if (!conf_mgr.gConfPersonalLand[villageId][landId]) { continue; }
                ...
                if (timeOver) {
                    ...
                    var mail = {
                        awards: awards,
                        time: time,
                        expire: time + conf_mgr.gConfMail[40].time * OneDayTime,
    ...
    resetByDay: function () {
        ...
        for (var k in this.villages) {
            ...
            if (villageId == 1) {
                for (var team_id in this.village1st) {
                        ...
                        for (var k1 in teamInfo.member_list) {
                            ...
                            if (awardsArr[memberUid] && awardsArr[memberUid] > 0) {
                                ...
                                var mail = {
                                    awards: awards,
                                    time: time,
                                    expire: time + conf_mgr.gConfMail[43].time * OneDayTime,
                    ...
                    // 重置产量
                    this.village1st[team_id].remain_res = conf_mgr.gConfTeamLand[villageId].resourceReserve[0][2];
                }
            ...
            this.villages[villageId].remain_res = conf_mgr.gConfTeamLand[villageId].resourceReserve[0][2];
        ...
        //             var remainTime = conf_mgr.gConfGlobal.territoryWarPersonalMaxHour * 3600 - holdTime - passTime;
        //             if (remainTime > 0) {
    ...
    getVillageSimpleInfo1st: function (team_id = 0) {
        ...
        if (vdata) {
            ...
            info = {
                team_id: 0,
                time: 0,
                remain_res: conf_mgr.gConfTeamLand[1].resourceReserve[0][2],
                calc_time: 0,
            }
        }
    ...
    getLandInfo: function (village_id, land_id, recalc_award) {
        ...
        if (this.lands[village_id] && this.lands[village_id][land_id]) {
            ...
            if (owner > 0) {
                ...
                var endTime = this.lands[village_id][land_id].time + conf_mgr.gConfGlobal.territoryWarPersonalMaxHour * 3600;// + notUseTime - holdTime;
    ...
    getBarrenLandByUid: function (uid) {
        ...
        if (this.barren_land[uid]) {
            ...
            if (user) {
                ...
                var endTime = this.barren_land[uid].time + conf_mgr.gConfGlobal.territoryWarPersonalMaxHour * 3600;// + notUseTime - holdTime;
                info.end_time = endTime; // 结束时间
            }
            info.village_id = 1;
        }
    ...
    calcLandAwards: function (village_id, land_id) {
        ...
        var owner = this.lands[village_id][land_id].owner;
        var landConf = conf_mgr.gConfPersonalLand[village_id][land_id];
        var curTime = common.getTime();
        var diffTime = curTime - lastCalcTime;

        // 检查是否超过每日可占领时长了
        var endTime = this.lands[village_id][land_id].time + conf_mgr.gConfGlobal.territoryWarPersonalMaxHour * 3600;
        if (curTime > endTime) {
            diffTime = endTime - lastCalcTime;
            timeOver = true;
        }

        if (!timeOver && diffTime < conf_mgr.gConfGlobal.territoryWarPersonalInterval * 60) {
            return false;
        }

        var dropCount = Math.floor(diffTime / (conf_mgr.gConfGlobal.territoryWarPersonalInterval * 60));
        ...
        if (dropCount > 0) {
            ...
            var passTime = dropCount * conf_mgr.gConfGlobal.territoryWarPersonalInterval * 60;
    ...
    calcBarrenLandAwards: function (uid) {
        ...
        var landConf = conf_mgr.gConfBarrenLand[barrenLandId];
        var curTime = common.getTime();
        var diffTime = curTime - lastCalcTime;
        ...
        if (oldOwner > 0) {
            ...
            if (user) {
                ...
                var mail = {
                    awards: awards,
                    time: time,
                    expire: time + conf_mgr.gConfMail[41].time * OneDayTime,
    ...
    calcVillageAward: function (village_id, awards_arr, force) {
        ...
        var villageConf = conf_mgr.gConfTeamLand[village_id];
        var curTime = common.getTime();
        var diffTime = curTime - lastCalcTime;
        if (diffTime < conf_mgr.gConfGlobal.territoryWarTeamInterval * 60 && !force) {
            return false;
        }
        ...
        if (oldTeam > 0) {
            ...
            for (var k in teamInfo.member_list) {
                ...
                if (arrAwards[memberUid] > 0) {
                    var awards = clone(conf_mgr.gConfTeamLand[village_id].resourceReserve);
                    awards[0][2] = Math.floor(arrAwards[memberUid]);
                    var time = common.getTime();
                    var mail = {
                        awards: awards,
                        time: time,
                        expire: time + conf_mgr.gConfMail[45].time * OneDayTime,
    ...
    occupyVillage1st: function (uid, team_data, team_id) {
        ...
        this.village1st[team_id] = {
            ...
            time: curTime,
            calc_time: curTime,
            remain_res: conf_mgr.gConfTeamLand[village_id].resourceReserve[0][2],
        }
    ...
    leaveVillage: function (uid, village_id, team_id) {
        if (this.villages[village_id] && this.villages[village_id].team_id) {
            ...
            if (!teamData) {
                ...
                return LANDGRABBER_ErrorCode.ERROR_NO_TEAM
            }

            if (team_id) {
                // 如果是解散队伍，就会传队伍id
            } else if (teamData.uid != uid) {
                return LANDGRABBER_ErrorCode.ERROR_NOT_VILLAGE_OWNER;
            }
            ...
            for (var k in teamData.member_list) {
                ...
                if (awardArr[memberUid] > 0) {
                    ...
                    var mail = {
                        awards: awards,
                        time: time,
                        expire: time + conf_mgr.gConfMail[44].time * OneDayTime,
                        content: contentArr,
                        attach: {
    ...
    leaveVillage1st: function (uid, team_id) {
        ...
        if (!teamData) {
            ...
            return [LANDGRABBER_ErrorCode.ERROR_NO_TEAM, 'no team'];
        }
    ...
    onTeamMemberLeave: function (team_id, member_uid) {
        ...
        if (this.calcVillageAward1st(team_id, awardsArr, true) && awardsArr[member_uid] > 0) {
            var awards = clone(conf_mgr.gConfTeamLand[1].resourceReserve);
    ...
    getLandDailyHoldTime: function (uid) {
        return this.dailyLandHoldTime[uid] || conf_mgr.gConfGlobal.territoryWarPersonalMaxHour * 3600;
    },

    getLandDailyRemainTime: function (uid) {
        //var notUseTime = this.notUseTime[uid] || 0;   // 昨日未用时间
        //var holdTime = this.dailyLandHoldTime[uid] || 0;
        var remainTime = conf_mgr.gConfGlobal.territoryWarPersonalMaxHour * 3600;
        return remainTime;
    },
...
// 获取村庄列表
exports.village_get_list = function (req, res, resp, onReqHandled) {
    do {
        // 向跨服服务器请求数据
        var crossReq = {
            uid: req.uid,
...
// 获取村庄信息
exports.village_get_info = function (req, res, resp, onReqHandled) {
    do {
...
// 前往抢夺
exports.village_occupy = function (req, res, resp, onReqHandled) {
    var uid = req.uid;
    do {
        var village_id = req.args.village_id;
        if (!village_id) {
        ...
        if (!teamData) {
            resp.code = LANDGRABBER_ErrorCode.ERROR_NO_TEAM; resp.desc = 'no team'; break;
        }
        ...
        if (!ownerInfo.team_id) {
            ...
            if (common.getTime() < villageInfo.time + conf_mgr.gConfGlobal.territoryWarTeamDefendTime * 60) {
                resp.data.village_info = gLandGrabber.getVillageDetailInfo(village_id);
                resp.code = LANDGRABBER_ErrorCode.ERROR_IN_PROTECT_TIME; resp.desc = 'protect time'; break;
            } else {
                // 有人占领，返回双方的战斗信息
                var infoArr = {};
                for (var k in teamData.member_list) {
...
exports.village_fight = function (req, res, resp, onReqHandled) {
    var uid = req.uid;
    do {
        var village_id = req.args.village_id;
        if (!village_id) {
...
// 放弃领主
exports.village_leave = function (req, res, resp, onReqHandled) {
    var uid = req.uid;
    do {
        var village_id = +req.args.village_id;
        if (!village_id) {
...
// 获取自身的地块占领信息
exports.land_get_info = function (req, res, resp, onReqHandled) {
    var uid = req.uid;
    do {
        var village_id = +req.args.village_id;
...
// 请求指定村庄的地块列表
exports.land_get_list = function (req, res, resp, onReqHandled) {
    var uid = req.uid;
    do {
        var village_id = +req.args.village_id;
        if (!village_id) {
        ...
        if (!conf_mgr.gConfPersonalLand[village_id]) {
        ...
        for (var k in conf_mgr.gConfPersonalLand[village_id]) {
...
// 占领地块
exports.land_occupy = function (req, res, resp, onReqHandled) {
    var uid = req.uid;
    do {
        var village_id = +req.args.village_id;
        var land_id = +req.args.land_id;
        ...
        if (barrenLand) {
            resp.code = LANDGRABBER_ErrorCode.ERROR_HAS_OCCUPY_LAND; resp.desc = 'you has occupy one land'; break;
        }
        ...
        if (remainTime <= 0) {
            resp.code = LANDGRABBER_ErrorCode.ERROR_LAND_REMAIN_TIME_ZERO; resp.desc = 'you has occupy one land'; break;
        }
        ...
        if (land_id > DEFAULT_BARREN_LAND_BEGIN_ID) {
            ...
            if (!landOwner || landOwner == 0) {
                ...
                if (common.getTime() < landInfo.time + conf_mgr.gConfGlobal.territoryWarPersonalDefendTime * 60) {
                    resp.data.land_info = gLandGrabber.getLandInfo(village_id, land_id);
                    resp.code = LANDGRABBER_ErrorCode.ERROR_IN_PROTECT_TIME;
                } else {
                    // 需要战斗
                    var replay = {
...
exports.land_fight = function (req, res, resp, onReqHandled) {
    var uid = req.uid;
    do {
        var village_id = +req.args.village_id;
        var land_id = +req.args.land_id;
...
// 地块撤离
exports.land_leave = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var village_id = +req.args.village_id;
        var land_id = +req.args.land_id;
...
// 小队解散
exports.village_team_dismiss = function (req, res, resp, onReqHandled) {
    var uid = req.uid;
    do {
...
// 成员加入
exports.village_member_join = function (req, res, resp, onReqHandled) {
    do {
...
// 成员离开
exports.village_member_leave = function (req, res, resp, onReqHandled) {
    do {
        var teamId = +req.args.team_id;
        var memberId = +req.args.member_id;
        if (!teamId || !memberId) {
...
// 成员村庄开启
exports.village_member_open = function (req, res, resp, onReqHandled) {
    do {
        var teamId = +req.args.team_id;
        var memberId = +req.args.member_id;
...
// 获取村庄的战报列表
exports.village_get_report_list_by_village = function (req, res, resp, onReqHandled) {
    var uid = req.uid;
    do {
        var village_id = req.args.village_id;
        if (!village_id) {
...
// 根据队伍id获取战报
exports.village_get_report_list_by_team = function (req, res, resp, onReqHandled) {
    var uid = req.uid;
    do {
        var team_id = req.args.team_id;
        if (!team_id) {
...
// 获取指定战斗的战斗列表
exports.get_battle_reports = function (req, res, resp, onReqHandled) {
    var uid = req.uid;
    do {
...
// 根据村庄获取地块战报列表
exports.get_reports_by_village = function (req, res, resp, onReqHandled) {
    var uid = req.uid;
    do {
        var village_id = req.args.village_id;
        if (!village_id) {
...
// 送花
exports.send_flower = function (req, res, resp, onReqHandled) {
    do {
        var villageId = req.args.village_id;
        if (!villageId) {
...
// 砸鸡蛋
exports.send_egg = function (req, res, resp, onReqHandled) {
    do {
        var villageId = req.args.village_id;
        if (!villageId) {
            resp.code = 1; resp.desc = 'invalid args'; break;
