...
Arena.prototype = {
    ...
    getRobot: function (id, rank) {
        if (this.robots[id] && this.robots[id].un && this.robots[id].max_hero) {
            ...
            var arenaType = Math.ceil(id / 10000);
    ...
    getPlayerInfo: function (id) {
        ...
        if (isDroid(id)) {
            ...
            //var heroCombatConf = conf_mgr.getHeroCombatConf(user.hero_bag.heros[1].rid);
            //if (heroCombatConf)
            info.quality = 1;
...
exports.get = function (req, res, resp, onReqHandled) {
    ...
    do {
        ...
        if (uid in gArena.users) {
            ...
            //curRank = ++gArena.playerCount[type];
            //curType = ArenaType.BRONZE;
            //gArena.ranks[type][curRank] = uid;
            //gArena.markDirty(type, curRank);
...
function challenge_rank(uid, type, oldType, oldRank, is_challenge) {// 要切换竞技场
    ...
    if (isValidArenaType(oldType)) {
        gArena.replaceOldPlayerWithRobot(uid, oldType, oldRank, type);
    }

    var newRank = is_challenge ? 0 : ++gArena.playerCount[type];
    ...
    gArena.users[uid].rank = newRank;
    gArena.markDirty(util.format('playerCount.%d', type));

    if (!is_challenge) {
        gArena.ranks[type][newRank] = uid;
        gArena.markDirty(util.format('ranks.%d.%d', type, newRank));
    }
};
...
exports.total_rank_list = function (req, res, resp, onReqHandled) {
    var uid = req.uid;
    do {
        var rankList = [];
        ...
        for (var rank = 1; rank <= conf_mgr.gConfGlobal.rankListLimit_arena && rank <= gArena.totalRanks.length; rank++) {
...
exports.rank_change_notify = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
        ...
        DEBUG('rank_change_notify uid = ' + uid + ', type = ' + rank_type + ', rank = ' + rank_val);

        var user = gUserInfo.getUser(uid);
        if (user) {
            if (!gArena.users[uid]) {
                gArena.users[uid] = {};
                //gArena.insertTotalRankUser(uid, rank_type, rank_val);
                gArena.updateUserInTotalRank(uid, rank_type, rank_val);
                DEBUG('world arena rankchange ----> updateUserInTotalRank uid:' + uid);
            } else {
                gArena.updateUserInTotalRank(uid, rank_type, rank_val);
                DEBUG('world arena rankchange else ----> updateUserInTotalRank uid:' + uid);
            }
            ...
            (function () {
                ...
                DEBUG(`-----------  ${uid}  -${old_rank}  ${old_type}  ${robotId}`)
                ...
                DEBUG(`set robot: ${old_type} ${old_rank}  = ${robotId}`)
            })();

            // delete gArena.ranks[gArena.users[uid].type][gArena.users[uid].rank];
            ...
            DEBUG(`11111111111111111 ${rank_type}  ${rank_val}`)
            ...
            if (isCrossArena(rank_type) && remove && rank_type == ArenaType.PLATINUM) {
                gArena.replaceExistUidWithRobot(uid, rank_type)
            }
        }
    } while (false);
