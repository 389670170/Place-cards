...
const { Team_ErrorCode } = require('../../common/error.js');
...
function Clan() {
    ...
    this.ranks = new RankTree(//
        ...
        function (c1, c2) {
            ...
            if (c1[1] < c2[1]) return 1;
            return c1[0] - c2[0];
        }, conf_mgr.gConfGlobal.rankListLimit_Team);
...
Clan.create = function (config, callback) {
    var clanData = {
        _id: 'clan',
...
Clan.prototype = {
    ...
    insertEvent: function (teamId, uid, time, type) {
        ...
        if (this.clan_log[teamId].length > conf_mgr.gConfGlobal.teamEventNumLimit) {
    ...
    refreshBadges: function (teamId) {
        ...
        if (memberCount < 5) {
            ...
            // 根据激活条件检查徽章激活情况
            for (var k in conf_mgr.gConfTeamEmblem) {
                var badgeInfo = conf_mgr.gConfTeamEmblem[k];
    ...
    calcTeamLevel: function (teamId) {
        ...
        var maxLv = Object.keys(conf_mgr.gConfTeamBase).length;
        for (var i = 1; i <= maxLv; i++) {
            clanLevel = i;
            var reqExp = conf_mgr.gConfTeamBase[i].exp;
        ...
        if (upExp > conf_mgr.gConfTeamBase[maxLv].exp) {
            upExp = conf_mgr.gConfTeamBase[maxLv].exp;
        }
    ...
    getQuality: function (custom_king) {
        ...
        if (custom_king && custom_king.chapter > 0 && custom_king.index > 0) {
            //quality = conf_mgr.gConfCustomKing[custom_king.chapter][custom_king.index].quality;
        }
    ...
    detect_badge: function (teamId) {
        ...
        var maxIndex = conf_mgr.gConfTeamBase[level].maxMember;
        if (memberNum >= maxIndex) {
            for (var i in conf_mgr.gConfTeamEmblem) {
                var teamEmblem = conf_mgr.gConfTeamEmblem[i];
    ...
    auto_dissolve_team: function () {
        do {
            ...
            var MaxDissolveTime = conf_mgr.gConfGlobal.teamAutoBreakTime * 24 * 60 * 60;
            for (var i in teams) {
                var tid = parseInt(i);
                if (teams[tid].dissolve_time != 0 && (common.getTime() - teams[tid].dissolve_time) > MaxDissolveTime) {
                    ...
                    var mail = {
                        awards: [],
                        time: time,
                        expire: time + conf_mgr.gConfMail[47].time * OneDayTime,
...
exports.get_team = function (req, res, resp, onReqHandled) {
    ...
    do {
        ...
        if (impeachUid && impeachTime) {
            var captainTime = conf_mgr.gConfGlobal.teamCaptainImpeachTime * 60 * 60;
            // var captainTime = 120;
...
// 获取队伍信息和成员信息
exports.get_team_and_member = function (req, res, resp, onReqHandled) {
    var uid = req.uid;
    do {
        var teamId = gClan.userTeam[uid];
        var team = gClan.getTeamMsg(uid);
...
exports.create = function (req, res, resp, onReqHandled) {
    do {
        ...
        if (gClan.isName(name)) {
            resp.code = Team_ErrorCode.ERROR_TEAM_NAME_EXIST; resp.desc = 'name is exist'; break;
        }
        ...
        if (oldTeamId) {
            if (!gClan.teams[oldTeamId]) {
                ...
                resp.code = Team_ErrorCode.ERROR_ALREADY_HAS_TEAM; resp.desc = 'teams exits'; break;
            }
        }

        var curTime = common.getTime();
        if (gClan.getLastJoinTime(uid) + conf_mgr.gConfGlobal.teamJoinTimeLimit * 3600 > curTime) {
            resp.code = Team_ErrorCode.ERROR_LAST_JOIN_TIME_TOO_NEAR; resp.desc = 'time limit'; break;
        }
...
// 修改战队公告
exports.modify_bulletin = function (req, res, resp, onReqHandled) {
    do {
        var bulletinName = req.args.bulletinName;
        var teamId = gClan.userTeam[req.uid];

        if (!gClan.isLeader(teamId, req.uid)) {
            resp.code = Team_ErrorCode.ERROR_NOT_TEAM_LEADER; resp.desc = 'not leader'; break;
        }
...
// 解散队伍
exports.dissolve_team = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var teamId = gClan.userTeam[uid];
        var team = gClan.teams[teamId];
        if (!team) {
            resp.code = Team_ErrorCode.ERROR_TEAM_NOT_EXIST; resp.desc = 'team not exist'; break;
        }
        ...
        var mail = {
            awards: [],
            time: time,
            expire: time + conf_mgr.gConfMail[46].time * OneDayTime,
...
exports.modify_name = function (req, res, resp, onReqHandled) {
    do {
        ...
        if (gClan.isName(name)) {
            resp.code = Team_ErrorCode.ERROR_TEAM_NAME_EXIST; resp.desc = 'name exits'; break;
        }
...
// 查询战队列表
exports.get_team_list = function (req, res, resp, onReqHandled) {
    do {
        var teamId = +req.args.teamId;
        if (teamId) {// 根据战队id查找
            if (!gClan.teams[teamId]) {
                resp.code = Team_ErrorCode.ERROR_TEAM_NOT_EXIST; resp.desc = 'not team'; break;
            }
...
// 申请加入战队
exports.join_team = function (req, res, resp, onReqHandled) {
    do {
        var teamId = +req.args.teamId;
        ...
        if (oldTeamId) {
            if (!gClan.teams[oldTeamId]) {
                ...
                resp.code = Team_ErrorCode.ERROR_ALREADY_HAS_TEAM; resp.desc = 'already join team'; break;
            }
        }

        if (gClan.applyList[req.uid] && gClan.applyList[req.uid].length >= conf_mgr.gConfGlobal.teamApplyLimit) {
            resp.code = Team_ErrorCode.ERROR_APPLY_LIST_FULL; resp.desc = 'join team capped'; break;
        }

        // 判断玩家距离上次加入队伍时间
        if (gClan.getLastJoinTime(req.uid) + conf_mgr.gConfGlobal.teamJoinTimeLimit * 3600 > common.getTime()) {
            resp.code = Team_ErrorCode.ERROR_LAST_JOIN_TIME_TOO_NEAR; resp.desc = 'time limit'; break;
        }
        ...
        if (applyCount >= conf_mgr.gConfGlobal.teamApplyMax) {
            resp.code = Team_ErrorCode.ERROR_APPLY_LIST_FULL; resp.desc = 'team apply count'; break;
        }

        var level = gClan.calcTeamLevel(teamId)[0];
        if (Object.keys(team.member_list).length >= conf_mgr.gConfTeamBase[level].maxMember) {
            resp.code = Team_ErrorCode.ERROR_MEMBER_LIST_FULL; resp.desc = 'team full'; break;
        }
...
// 撤销申请加入战队
exports.undo_join = function (req, res, resp, onReqHandled) {
    do {
        var teamId = +req.args.teamId;
        var team = gClan.teams[teamId];
        if (!team) {
        ...
        if (!gClan.applyList[req.uid] || gClan.applyList[req.uid].indexOf(teamId) <= -1) {
            resp.code = Team_ErrorCode.ERROR_NOT_APPLY_YET; resp.desc = 'not apply'; break;
        }
...
// 退出战队
exports.leave_team = function (req, res, resp, onReqHandled) {
    do {
        ...
        if (req.uid == team.uid) {
            resp.code = Team_ErrorCode.ERROR_LEADER_CAN_NOT_LEAVE; resp.desc = 'leader do sign out team'; break;
        }

        if (!memberList[req.uid]) {
            resp.code = Team_ErrorCode.ERROR_NOT_TEAM_MEMBER; resp.desc = 'do not member'; break;
        }
...
// 获取已经激活的徽章
exports.get_can_use_badge = function (req, res, resp, onReqHandled) {
    do {
        var teamId = gClan.userTeam[req.uid];
        var team = gClan.teams[teamId];
        var canUseBadge = [];
...
// 佩戴战队徽章
exports.use_badge = function (req, res, resp, onReqHandled) {
    do {
        ...
        if (!flag) {
            resp.code = Team_ErrorCode.ERROR_BADGE_CAN_NOT_USE; resp.desc = 'can not use badge'; break;
        }
...
// 获取入队条件
exports.get_join_term = function (req, res, resp, onReqHandled) {
    do {
        var teamId = gClan.userTeam[req.uid];
        if (!gClan.isLeader(teamId, req.uid)) {
            resp.code = Team_ErrorCode.ERROR_NOT_TEAM_LEADER; resp.desc = 'not leader'; break;
...
// 修改申请入队条件
exports.modify_join_term = function (req, res, resp, onReqHandled) {
    do {
        ...
        if (team.uid != req.uid) {
            resp.code = Team_ErrorCode.ERROR_NOT_TEAM_LEADER; resp.desc = 'do not leader'; break;
        }
...
// 任命队长
exports.nominate_leader = function (req, res, resp, onReqHandled) {
    do {
        ...
        var team = gClan.teams[teamId];
        if (!team) {
            resp.code = Team_ErrorCode.ERROR_TEAM_NOT_EXIST;
            resp.desc = 'not team';
            break;
        }
        ...
        if (uid == nominateUid) {
            resp.code = Team_ErrorCode.ERROR_ALREADY_TEAM_LEADER; resp.desc = 'already leader'; break;
        }
        ...
        if (!flag) {
            resp.code = Team_ErrorCode.ERROR_NOT_TEAM_MEMBER; resp.desc = 'not member'; break;
        }
...
// 踢出队伍
exports.kicked_out = function (req, res, resp, onReqHandled) {
    do {
        ...
        if (kickedUid == req.uid) {
            resp.code = Team_ErrorCode.ERROR_CAN_NOT_KICK_SELF; resp.desc = 'leader not kick'; break;
        }
...
exports.impeach = function (req, res, resp, onReqHandled) {
    do {
        ...
        if (leaderUid != team.uid) {
            resp.code = Team_ErrorCode.ERROR_CAN_NOT_IMPEACH_SELF; resp.desc = 'leader not kick'; break;
        }
        //已经存在弹劾
        if (team.impeach.uid && team.impeach.time) {
            resp.code = Team_ErrorCode.ERROR_IMPEACH_CONDITION_NOT_ENOUGH; resp.desc = 'do not impeach'; break;
        }
        var member = team.member_list[leaderUid];
        var impeachTiam = common.getTime();
        var CaptainTime = conf_mgr.gConfGlobal.teamCaptainImpeachLimit * 60 * 60;
        // var CaptainTime = 120;
...
// 是否同意加入队伍
exports.reply_join = function (req, res, resp, onReqHandled) {
    do {
        ...
        if (!applyList[applyUid]) {
            resp.code = Team_ErrorCode.ERROR_HAS_JOIN_OTHER_TEAM; resp.desc = 'not applicant'; break;
        }

        if (gClan.userTeam[applyUid]) {
            resp.code = Team_ErrorCode.ERROR_ALREADY_HAS_TEAM; resp.desc = 'already has team'; break;
        }

        // 判断玩家距离上次加入队伍时间
        if (isJoin == 1 && gClan.getLastJoinTime(applyUid) + conf_mgr.gConfGlobal.teamJoinTimeLimit * 3600 > common.getTime()) {
            resp.code = Team_ErrorCode.ERROR_LAST_JOIN_TIME_TOO_NEAR; resp.desc = 'time limit'; break;
        }
        ...
        if (isJoin == 1) {
            ...
            if (memberNum >= conf_mgr.gConfTeamBase[level].maxMember) {
                resp.code = Team_ErrorCode.ERROR_MEMBER_LIST_FULL; resp.desc = 'member is full'; break;
            }
...
// 判断战队成员离线时间，是否自动解散
exports.auto_dissolve_team = function (req, res, resp, onReqHandled) {
...
// 获取每日奖励 返回战队等级
exports.get_daily_awards = function (req, res, resp, onReqHandled) {
    do {
        var teamId = gClan.userTeam[req.uid];
        var team = gClan.teams[teamId];
        if (!team) {
        ...
        var level = gClan.calcTeamLevel(teamId)[0];
        resp.data.awards = conf_mgr.gConfTeamBase[level].award;
    } while (false);
...
// 获取战队排行榜
exports.get_ranks = function (req, res, resp, onReqHandled) {
    do {
        var rankList = [];
        gClan.ranks.each(function (data) {

            if (rankList.length >= conf_mgr.gConfGlobal.rankListLimit_team) return false;
...
// 进入战队任务界面获取信息
exports.get_task = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var teamId = gClan.userTeam[uid];
        var team = gClan.teams[teamId];
        if (!team) {
            resp.code = Team_ErrorCode.ERROR_TEAM_NOT_EXIST; resp.desc = 'no team'; break;
        }
...
// 激活任务 直接领取奖励
exports.active_task = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        ...
        var awards = conf_mgr.gConfTeamBase[teamLevel].teamTaskAward;
        var addExp = 0;
...
// 领取宝箱
exports.get_awards_box = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var teamId = gClan.userTeam[uid];
        ...
        var boxNum = gClan.task_msg[uid] || 0;
        if (boxNum <= 0) {
            resp.code = Team_ErrorCode.ERROR_NO_AWARD_CAN_GET; resp.desc = 'award_box is not num'; break;
        }
        ...
        var awards = conf_mgr.gConfTeamBase[oldLevel].teamTaskBoxAward;
        var addExp = 0;
        for (var i = 0; i < awards.length; i++) {
            if (awards[i][1] == 'team_exp') {
                gClan.addMemberTeamExp(teamId, uid, parseInt(awards[i][2]));
...
// 返回日志
exports.get_clan_log = function (req, res, resp, onReqHandled) {
    do {
        var teamId = gClan.userTeam[req.uid];
...
// 获取队长id
exports.get_leader_id = function (req, res, resp, onReqHandled) {
    do {
        var teamId = gClan.userTeam[req.uid];
        var team = gClan.teams[teamId];
        if (!team) {
...
// 获取小队信息
exports.get_team_info = function (req, res, resp, onReqHandled) {
    do {
...
exports.tidy_team_member = function (req, res, resp, onReqHandled) {
