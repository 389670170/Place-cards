...
const { LEGION_ErrorCode } = require('../../common/error.js');
const { OneDayTime } = require('../../common/enum.js');
const { parseAwardsConfig, reformAwards } = require('../../common/global.js');
...
function NewLegion() {
    ...
    this.ranks = new RankTree(// 军团排行
        ...
        function (c1, c2) {
            ...
            if (c1[1] < c2[1]) return 1;
            return c1[0] - c2[0];
        }, conf_mgr.gConfGlobal.rankListLimit_legion);
...
NewLegion.create = function (config, callback) {
    var newLegionData = {
        _id: 'new_legion',
        ai: config.ServerId * 1000000,
        legions: {},
        bonfireData: {},
        dailyContribution: {},
    };
...
NewLegion.prototype = {
        ...
        gDBWorld.find({ _id: 'new_legion' }).limit(1).next(function (err, doc) {
            if (doc) {
                ...
                for (var lid in legions) {
                    ...
                    if (legion.level >= parseInt(conf_mgr.gConfGlobal.legionBossOpenLimit)) {
                        if (!legion.boss.id) {
                            this.resetBoss(lid);
                        } else {
                            // 军团boss开启了才初始化
    ...
    initBonfire: function (lid, reset) {
        ...
        if (!legion.bonfire_time) {
            legion.bonfire_time = [conf_mgr.gConfGlobal.legionBonfire_openTimeDefault, 1];
            this.markDirty(util.format('legions.%d.bonfire_time', lid));
        }

        if (!legion.next_fire_time) {
            legion.next_fire_time = [conf_mgr.gConfGlobal.legionBonfire_openTimeDefault, 1];
        ...
        var hTime = conf_mgr.gConfGlobal.legionBonfire_openTime.split('|')[hIndex - 1];
        ...
        if (timeToStart > 0) {
            ...
            // 已经开始了，设置一个结束的定时器
            var totalTime = conf_mgr.gConfGlobal.legionBonfire_totalTime * 60; // 活动开启多久
            ...
            if (arr[i][0] == 'user') {
                ...
                switch (type) {
                    case 'legion_exp':      // 军团经验
                        ...
                        // 增加成员贡献度
                        this.legions[lid].member_list[uid].contribution += parseInt(legion_exp * conf_mgr.gConfGlobal.legionContributeConversionRate);
    ...
    resetByDay: function () {
        ...
        for (var lid in legions) {
            ...
            if (legions[lid].level >= conf_mgr.gConfGlobal.legionBossOpenLimit) {
                this.resetBoss(lid);
            }
        }
    ...
    getBossRankAwards: function (rank) {
        ...
        for (var i = 2; i <= Object.keys(conf_mgr.gConfLegionBossRank).length; i++) {
            curRank = conf_mgr.gConfLegionBossRank[i].rank;
        ...
        if (level == null) {
            level = Object.keys(conf_mgr.gConfLegionBossRank).max();
        }

        var awards = conf_mgr.gConfLegionBossRank[level].award;
        return awards;
    },
    ...
    calcBossAwards: function (lid) {
        ...
        // 参与奖
        var awards_join = conf_mgr.gConfLegionBoss[legion.boss.level].award_join;
        if (awards_join && joinUids.length > 0) {
            var mail = {
                awards: 'legion_boss_join',
                time: time,
                expire: time + conf_mgr.gConfMail[51].time * OneDayTime,
                content: [51],
                attach: {
                    lid: lid,
                    uids: joinUids,
        ...
        if (legion.boss.first_kill_award_level > 0) {
            var awards_first = conf_mgr.gConfLegionBoss[legion.boss.first_kill_award_level].award_firstkill;
            if (awards_first) {
                ...
                var mail = {
                    ...
                    time: time,
                    expire: time + conf_mgr.gConfMail[52].time * OneDayTime,
        ...
        if (bossRank) {
            ...
            if (awards_rank) {
                var mail = {
                    ...
                    time: time,
                    expire: time + conf_mgr.gConfMail[53].time * OneDayTime,
                    content: [53],
                    attach: {
                        lid: lid,
                        uids: joinUids,
    ...
    initBossTimer: function (lid) {
        ...
        var awardTime = bossTime[1] + conf_mgr.gConfGlobal.legionBossAwardTime * 60;
    ...
    resetBoss: function (lid) {
        ...
        if (legion.boss.level < Object.keys(conf_mgr.gConfLegionBoss).max() && legion.boss.daily_kill_count > 0) {
            // boss可以升级
            legion.boss.level++;
        }
        ...
        // 随机一个boss
        var formations = conf_mgr.gConfLegionBoss[legion.boss.level].formation;
    ...
    addLog: function (lid, id, arg1, arg2, arg3) {
        ...
        if (legion.logs.length > conf_mgr.gConfGlobal.legionLogLimit) {
            legion.logs.pop();
        }
    ...
    createLegion: function (name, icons, uid) {
        ...
        var newLegionData = {
            ...
            bonfire_time: [conf_mgr.gConfGlobal.legionBonfire_openTimeDefault, 1],
            next_fire_time: [conf_mgr.gConfGlobal.legionBonfire_openTimeDefault, 1],
            boss: {
                id: 0,     // boss id
                level: 1,  // boss等级
                hp: 0,     // boss当前生命值(万分比)
        ...
        // 初始化军团boss
        if (conf_mgr.gConfGlobal.legionBossOpenLimit == 1) {
            this.resetBoss(lid);
        }
    ...
    pushToLegion: function (lid, user, type) {
        ...
        if (type) {
            ...
            for (var uid in memberList) {
                uid = +uid;
                if (conf_mgr.gConfLegionJurisDiction[memberList[uid].duty][type]) {
                    uids.push(uid);
                }
            }
        } else {
    ...
    calcLevel: function (lid) {
        ...
        var exp = legion.xp;
        var maxLegionLv = Object.keys(conf_mgr.gConfLegionLevel).length;
        for (var i = level; i <= maxLegionLv; i++) {
            var legionLvConf = conf_mgr.gConfLegionLevel[i];
        ...
        if (level != legion.level || exp != legion.xp) {
            if (level != legion.level) {
                if (legion.boss.id == 0 && level >= conf_mgr.gConfGlobal.legionBossOpenLimit) {
                    this.resetBoss(lid);
                }
        ...
        // var level = 0, exp = 0;
        // for (var i in conf_mgr.gConfLegionLevel) {
        //     exp += conf_mgr.gConfLegionLevel[i].xp;
        //     if (exp > legion.xp) {
        //         level = +i;
        //         break;
    ...
    calcBonfireLevel: function (lid) {
        ...
        ;
        var level = 0;
        var maxLevel = Object.keys(conf_mgr.gConfLegioBonfire).length;
        for (var i = 1; i <= maxLevel; i++) {
            if (bonfireData.xp >= conf_mgr.gConfLegioBonfire[i].exp) {
                level = i;
            } else {
                break;
            }
    ...
    bonfireTimeSplit: function (lid) {
        ...
        if (oldLevel != newLevel) {
            ...
            bonfireData.red_gift['level' + newLevel] = conf_mgr.gConfLegioBonfire[newLevel].redPaperNum;
            this.markDirty(util.format('bonfireData.%d', lid));
    ...
    verifyDuty: function (lid, uid, duty) {
        ...
        if (duty == 'deputy') {
            if (dutyNum >= conf_mgr.gConfLegionLevel[legion.level].deputyNum) return false;
        } else if (duty == 'elite') {
            if (dutyNum >= conf_mgr.gConfLegionLevel[legion.level].eliteNum) return false;
        }
        return true;
    },
    ...
    operaDuty: function (uid, otherDuty) {
        ...
        if (!conf_mgr.gConfLegionJurisDiction[duty].controlduty) {
            return false;
        }

        return conf_mgr.gConfLegionJurisDiction[duty].controlduty.indexOf(otherDuty) > -1 ? true : false;
    },
        ...
        for (var uid in bonfireMember) {
            if (awardList[uid].length > 0) {
                var mail = {
                    ...
                    time: time,
                    expire: time + conf_mgr.gConfMail[20].time * OneDayTime,
                    content: [20],
                    attach: {
    ...
    calcMemberAwards: function (lid, joinTime) {
        ...
        var totalTime = conf_mgr.gConfGlobal.legionBonfire_totalTime * 60; // 活动开启多久
        var limitTime = conf_mgr.gConfGlobal.legionBonfire_expInterval;// 多少秒结算一次经验
        ...
        var awards = [];
        // 配置文件
        var confLegionLevel = conf_mgr.gConfLegionLevel[this.calcLevel(lid)];
        if (confLegionLevel) {
    ...
    isBonfireOpen: function (lid) {
        ...
        if (curTime >= todayOpenTime && curTime < todayOpenTime + conf_mgr.gConfGlobal.legionBonfire_totalTime * 60) {
            return true;
        }
    ...
    addBonfireMaterial: function (lid, uid, type, special) {
        ...
        var awards = conf_mgr.gConfLegionLevel[legion.level]['bonfireOperateAward' + type];
...
// 创建军团
exports.create = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var name = req.args.name;
        var icons = req.args.icons;
        if (gNewLegion.isNameExist(name)) {
            resp.code = LEGION_ErrorCode.ERROR_LEGION_NAME_EXIST; resp.desc = 'name is exits'; break;
        }

        var legion = gNewLegion.getUserLegion(uid);
        if (legion) {
            resp.code = LEGION_ErrorCode.ERROR_LEGION_ALREADY_EXIST; resp.desc = 'legion is exits'; break;
        }
...
exports.get = function (req, res, resp, onReqHandled) {
    ...
    do {
        ...
        if (impeachUid && impeachTime) {
            var captainImpeachTime = conf_mgr.gConfGlobal.legionCaptainImpeachTime * 3600;
...
// 修改军团名
exports.modify_name = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var name = req.args.name;
        if (gNewLegion.isNameExist(name)) {
        ...
        var legion = gNewLegion.getUserLegion(uid);
        if (!legion) {
            resp.code = LEGION_ErrorCode.ERROR_LEGION_NOT_EXIST; resp.desc = 'legion is not exits'; break
        }

        var duty = gNewLegion.getDuty(uid);
        if (!conf_mgr.gConfLegionJurisDiction[duty].changeName) {
            resp.code = LEGION_ErrorCode.ERROR_NO_PRIORITY; resp.desc = 'Permission denied'; break
        }
...
// 修改公告
exports.modify_notice = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        ...
        var duty = gNewLegion.getDuty(uid);
        if (!conf_mgr.gConfLegionJurisDiction[duty].changeNotice) {
            resp.code = LEGION_ErrorCode.ERROR_NO_PRIORITY; resp.desc = 'Permission denied'; break
        }
...
// 退出军团
exports.logout = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var legion = gNewLegion.getUserLegion(uid);
        if (!legion) {
        ...
        var duty = gNewLegion.getDuty(uid);
        if (!conf_mgr.gConfLegionJurisDiction[duty].leaveLegion) {
            resp.code = LEGION_ErrorCode.ERROR_NO_PRIORITY; resp.desc = 'Permission denied'; break;
        }

        // 判断加入军团是否超过保护时间
        var member = legion.member_list[uid];
        var limitTime = parseInt(conf_mgr.gConfGlobal.legionLeaveTimeLimit) * 3600;
        var curTime = common.getTime();
        if (member && curTime < member.join_time + limitTime) {
            resp.code = LEGION_ErrorCode.ERROR_LEAVE_TIME_LIMIT; resp.desc = 'leave protect time'; break;
        }
...
// 解散军团
exports.dissolve = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var legion = gNewLegion.getUserLegion(uid);
        if (!legion) {
        ...
        var duty = gNewLegion.getDuty(uid);
        if (!conf_mgr.gConfLegionJurisDiction[duty].breakLegion) {
            resp.code = LEGION_ErrorCode.ERROR_NO_PRIORITY; resp.desc = 'Permission denied'; break
        }

        if (gLegionWar.runtime.joined[legion.lid] && gLegionWar.stage != 3) {
            resp.code = LEGION_ErrorCode.ERROR_CAN_NOT_DISSOLVE; resp.desc = 'Cannot dissolve during legionwar'; break
        }
...
// 申请设置
exports.join_set = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        ...
        if (!conf_mgr.gConfLegionJurisDiction[legion.member_list[req.uid].duty].setApply) {
            resp.code = LEGION_ErrorCode.ERROR_NO_PRIORITY; resp.desc = 'not manage'; break;
        }
...
// 搜索军团
exports.search = function (req, res, resp, onReqHandled) {
    do {
...
// 获取军团列表
exports.get_legions_list = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var lid = 0;
        var legion = gNewLegion.getUserLegion(uid);
        ...
        // 军团列表
        var limit = conf_mgr.gConfGlobal.legionRefreshLimit;
        var legionsListArr = [];
        var legionsList = {};
...
// 加入军团 || 申请
exports.join = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var lid = +req.args.lid;
        var legion = gNewLegion.legions[lid];
        ...
        if (gNewLegion.userLegion[uid]) {
            resp.code = LEGION_ErrorCode.ERROR_ALREADY_HAS_LEGION; resp.desc = 'already has legion'; break;
        }

        if (gNewLegion.applyList[uid] && gNewLegion.applyList[uid].contains(lid)) {
            resp.code = LEGION_ErrorCode.ERROR_ALREADY_APPLY; resp.desc = 'already apply'; break;
        }

        if (gNewLegion.getMemberCount(lid) >= conf_mgr.gConfLegionLevel[legion.level].memberMax) {
            resp.code = LEGION_ErrorCode.ERROR_MEMBER_COUNT_MAX; resp.desc = 'member is max count'; break;
        }
        ...
        if (joinWay == 0) {
            if (user.status.level < legion.join_term.level) {
                resp.code = LEGION_ErrorCode.ERROR_CAN_NOT_JOIN; resp.desc = 'do not join legion'; break;
            }
            ...
            if (gNewLegion.applyList[uid] && gNewLegion.applyList[uid].length >= conf_mgr.gConfGlobal.legionApplyLimit) {
                resp.code = LEGION_ErrorCode.ERROR_APPLY_COUNT_MAX; resp.desc = 'upper limit'; break;
            } else if (legion.apply_list && Object.keys(legion.apply_list).length >= conf_mgr.gConfGlobal.legionApplyMax) {
                resp.code = LEGION_ErrorCode.ERROR_APPLY_COUNT_MAX; resp.desc = 'upper limit'; break;
            }
...
// 撤销申请
exports.undo = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var lid = +req.args.lid;
        var legion = gNewLegion.legions[lid];
...
// 清空申请
exports.clear_apply = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var legion = gNewLegion.getUserLegion(uid);
        if (!legion) {
        ...
        if (!conf_mgr.gConfLegionJurisDiction[gNewLegion.getDuty(uid)].manageApply) {
            resp.code = LEGION_ErrorCode.ERROR_NO_PRIORITY; resp.desc = 'Permission denied'; break
        }
...
// 获取军团申请列表
exports.get_apply_list = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var legion = gNewLegion.getUserLegion(uid);
        if (!legion) {
...
// 申请管理操作
exports.set_apply = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        ...
        if (type == 1) {
            if (oldLegion) {
                resp.code = LEGION_ErrorCode.ERROR_OTHER_ALREADY_HAS_LEGION; resp.desc = 'already join legion'; break;
            }
        }
        ...
        if (!conf_mgr.gConfLegionJurisDiction[member.duty].manageApply) {
            resp.code = LEGION_ErrorCode.ERROR_NO_PRIORITY; resp.desc = 'not manage'; break;
        }
...
// 获取军团成员
exports.get_member_list = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var legion = gNewLegion.getUserLegion(uid);
        if (!legion) {
...
// 职位任免
exports.set_duty = function (req, res, resp, onReqHandled) {
    var opUid = +req.uid;
    do {
        ...
        if (!conf_mgr.gConfLegionJurisDiction[gNewLegion.getDuty(opUid)].operate || !gNewLegion.operaDuty(opUid, duty)) {
            var selfDuty = legion.member_list[opUid].duty;
            if (selfDuty != "leader" || duty != "leader") {
                resp.code = LEGION_ErrorCode.ERROR_NO_PRIORITY;
        ...
        if (!gNewLegion.verifyDuty(lid, uid, duty)) {
            resp.code = LEGION_ErrorCode.ERROR_DUTY_COUNT_MAX; resp.desc = 'member is upper'; break
        }

        var member = legion.member_list[uid];
        if (!member) {
            resp.code = LEGION_ErrorCode.ERROR_MEMBER_NOT_EXIST; resp.desc = 'not member'; break
        }
...
// 弹劾
exports.impeach = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        ...
        var impeachLimit = conf_mgr.gConfGlobal.legionCaptainImpeachLimit * 60 * 60;
        if (common.getTime - gUserInfo.getUser(uid).mark.active_time <= impeachLimit) {
            resp.code = LEGION_ErrorCode.ERROR_CAN_NOT_IMPEACH; resp.desc = 'does not meet the conditions'; break;
        }

        if (!conf_mgr.gConfLegionJurisDiction[gNewLegion.getDuty(uid)].impeachment) {
            resp.code = LEGION_ErrorCode.ERROR_NO_PRIORITY; resp.desc = 'Permission denied'; break
        }

        if (legion.impeach.time && legion.impeach.uid) {
            resp.code = LEGION_ErrorCode.ERROR_ALREADY_IMPEACH; resp.desc = 'already impeach'; break;
        }
...
// 踢出军团
exports.kick_out = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var targetUid = +req.args.uid;
        if (!targetUid) {
        ...
        if (targetUid == uid) {
            resp.code = LEGION_ErrorCode.ERROR_CAN_NOT_KICK_SELF; resp.desc = 'can not kick self'; break;
        }
        ...
        if (!member || !duty || !conf_mgr.gConfLegionJurisDiction[duty].operate || !gNewLegion.operaDuty(uid, member.duty)) {
            resp.code = LEGION_ErrorCode.ERROR_NO_PRIORITY; resp.desc = 'Permission denied'; break;
        }
        ...
        var kickSetOpt = conf_mgr.gConfGlobal.legionKickSet.split('|');
        var kickTime = kickSetOpt[0] * 3600;
        var costs = parseAwardsConfig(conf_mgr.gConfGlobal.legionKickBaseCost);
        ...
        if (legion.xp < Math.abs(costs[0][2])) {
            resp.code = LEGION_ErrorCode.ERROR_CAN_NOT_KICK_MEMBER; resp.desc = 'legion exp not enough'; break;
        }
...
// 更换旗帜
exports.modify_icon = function (req, res, resp, onReqHandled) {
    do {
        var icons = req.args.icons;
        ...
        if (!conf_mgr.gConfLegionJurisDiction[gNewLegion.getDuty(req.uid)].changeFlag) {
            resp.code = LEGION_ErrorCode.ERROR_NO_PRIORITY; resp.desc = 'Permission denied'; break;
        }
...
// 获取军团建设信息
exports.get_build = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var legion = gNewLegion.getUserLegion(uid);
        if (!legion) {
...
// 军团建设
exports.build = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var type = req.args.type;
        if (type == null) {
        ...
        var legion = gNewLegion.getUserLegion(uid);
        if (!legion) {
            resp.code = LEGION_ErrorCode.ERROR_LEGION_NOT_EXIST; resp.desc = 'not has legion'; break;
        }

        var awards = conf_mgr.gConfLegionBuild[type].award;
...
// 建设领取宝箱奖励
exports.get_build_awards = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var type = req.args.type;
        if (type == null) {
        ...
        // 判断是否可以领取奖励
        var buildLimit = conf_mgr.gConfGlobal.legionBuildProgressLimit.split('|');
        if (!buildLimit.factor(legion.build).length) {
            resp.code = LEGION_ErrorCode.ERROR_CAN_NOT_GET_AWARD; resp.desc = 'not awards'; break;
        }

        var level = legion.level || 1;
        var awards = conf_mgr.gConfLegionLevel[level]['buildProgressAward' + type];
...
// 设置篝火开启时间
exports.set_bonfire_time = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var legion = gNewLegion.getUserLegion(uid);
        if (!legion) {
        ...
        if (!hTimeId || !mTimeId || !conf_mgr.gConfGlobal.legionBonfire_openTime.split('|')[hTimeId - 1] || !gNewLegion.ConfBonfireMTime[mTimeId - 1]) {
            resp.code = LEGION_ErrorCode.ERROR_INVALID_ARGS; resp.desc = 'args is error'; break;
        }
...
// 加入篝火活动
exports.join_bonfire = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var legion = gNewLegion.getUserLegion(uid);
        if (!legion) {
        ...
        if (gNewLegion.isUserJoinBonfire(uid)) {
            resp.code = LEGION_ErrorCode.ERROR_ALREADY_JOIN_BONFIRE; resp.desc = 'has join bonfire'; break;
        }

        var curTime = common.getTime();

        var openTime = legion.open_time;    // 开始时间
        var totalTime = conf_mgr.gConfGlobal.legionBonfire_totalTime * 60;
        var readyTime = conf_mgr.gConfGlobal.legionBonfire_prepareTime * 60;  // 准备时常

        if (curTime < openTime - readyTime) {
            // 未开启
            resp.code = LEGION_ErrorCode.ERROR_BONFIRE_NOT_OPEN; resp.desc = 'not open'; break;
        }

        if (curTime > openTime + totalTime) {
            // 已结束
            resp.code = LEGION_ErrorCode.ERROR_BONFIRE_HAS_FINISH; resp.desc = 'already end'; break;
        }
...
// 添加木材 || 火苗
exports.add_material = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var type = req.args.type;
        ...
        if (!gNewLegion.isUserJoinBonfire(uid)) {
            resp.code = LEGION_ErrorCode.ERROR_NOT_JOIN_BONFIRE; resp.desc = 'not join bonfire'; break;
        }
...
// 抢红包
exports.grab_red_awards = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var type = req.args.type;
        ...
        if (!redGift[type] || redGift[type] <= 0) {
            ...
            resp.code = LEGION_ErrorCode.ERROR_RED_GIFT_IS_GONE; resp.desc = 'type 7'; break;
        }
...
// 获取军团日志
exports.get_logs = function (req, res, resp, onReqHandled) {
    do {
        var legion = gNewLegion.getUserLegion(req.uid);
        if (!legion) {
            resp.code = LEGION_ErrorCode.ERROR_LEGION_NOT_EXIST; resp.desc = 'legion is not exits'; break;
...
// 获取贡献记录
exports.get_contribution = function (req, res, resp, onReqHandled) {
    do {
        var legion = gNewLegion.getUserLegion(req.uid);
        if (!legion) {
            resp.code = LEGION_ErrorCode.ERROR_LEGION_NOT_EXIST; resp.desc = 'legion is not exits'; break;
        ...
        for (var uid in memberList) {
            ...
            if (user && user.pos[1]) {
                json[uid] = {
                    ...
                    headpic: user.info.headpic,
                    headframe: user.info.headframe,
                    quality: conf_mgr.getHeroCombatConf(user.pos[1].rid).quality,
                    vip: user.status.vip,
                    level: user.status.level,
                    fight_force: gUserInfo.getUserFightForce(uid),
                    duty: memberList[uid].duty,
...
// 获取成员职位
exports.get_member_duty = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var legion = gNewLegion.getUserLegion(uid);
        if (!legion) {
...
// gm操作
exports.gm_change_legion = function (req, res, resp, onReqHandled) {
    do {
        var legion = gNewLegion.getUserLegion(req.uid);
        if (!legion) {
            resp.code = LEGION_ErrorCode.ERROR_LEGION_NOT_EXIST; resp.desc = 'legion is not exits'; break;
            ...
            if (req.args.level) {
                ...
                var maxLevel = Object.keys(conf_mgr.gConfLegionLevel).length;
                level >= maxLevel && (level = maxLevel);
                var legion_exp = 0;
                for (var i in conf_mgr.gConfLegionLevel) {
                    if (i >= level) break;
                    legion_exp += conf_mgr.gConfLegionLevel[i].xp
                }
...
// 获取军团信息
exports.get_legion_info = function (req, res, resp, onReqHandled) {
    do {
...
exports.update_active = function (req, res, resp, onReqHandled) {
    var uid = req.uid;
    var lid = gNewLegion.userLegion[uid];
    ...
    if (lid) {
        ...
        for (var i = 0, len = ids.length; i < len; i++) {
            active += conf_mgr.gConfDailyTask[ids[i]].active;
        }
        ...
        legion.member_list[uid].active[conf_mgr.gConfLegion.activeDayLimit - 1] += active;
...
// 获取boss信息
exports.boss_get_info = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var legion = gNewLegion.getUserLegion(uid);
        if (!legion) {
            resp.code = LEGION_ErrorCode.ERROR_LEGION_NOT_EXIST; resp.desc = 'has no legion'; break;
        }
...
// 开始挑战boss
exports.boss_challenge = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var legion = gNewLegion.getUserLegion(uid);
        if (!legion) {
...
exports.boss_before_fight = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        ...
        if (id != legion.boss.id) {
            resp.code = LEGION_ErrorCode.ERROR_BOSS_ID_NOT_MATCH; resp.desc = 'boss id not match'; break;
        }
...
exports.boss_fight = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var id = req.args.boss_id;
        ...
        var awards = conf_mgr.gConfLegionBoss[legion.boss.level].award_challenge;

        // 最后一击奖励
        if (lastShot) {
            awards = awards.concat(conf_mgr.gConfLegionBoss[legion.boss.level].award_lastshot);
        }
...
// boss鼓舞
exports.boss_inspire = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var type = req.args.type;   // 鼓舞类型

        if (!conf_mgr.gConfLegionBossAakb[type]) {
        ...
        var addVal = conf_mgr.gConfLegionBossAakb[type].damage;
...
exports.boss_get_damage_rank = function (req, res, resp, onReqHandled) {
    do {
...
exports.rank_list = function (req, res, resp, onReqHandled) {
    ...
    }).slice(0, conf_mgr.gConfGlobal.rankListLimit_legion);
...
// 排行榜
exports.get_legion_leader_info = function (req, res, resp, onReqHandled) {
...
// -------------------------------- 军团战 --------------------------------------------
exports.get_hall = function (req, res, resp, onReqHandled) {
    do {
        var uid = req.uid;
        var legion = gNewLegion.getUserLegion(uid);
        if (!legion) {
        ...
        for (var m_uid in legion.member_list) {
            ...
            var heroCombatConf = conf_mgr.getHeroCombatConf(user.pos[1].rid);
            members[m_uid] = {
                'un': user.info.un,
                'headpic': user.info.headpic,
                'headframe': user.info.headframe,
...
// 获取军团boss信息
exports.get_legion_boss_info = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var lid = gNewLegion.getUserLegionId(uid);
        var legion = gNewLegion.get(lid);
