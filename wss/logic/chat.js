...
function sendChatMessage(req, resp, type, content, user, sendUid, onHandled, title, quality, userIndex, legion_id, legion_name, legion_level, team_id, team_name, team_level) {
    var respData = {
        code: 0,
        mod: 'chat',
        act: 'chat',
    ...
    respData.data.user = {
        uid: req.uid,
        un: req.uid == 10000 ? conf_mgr.gConfGeneralText["text_systemNotice"].text : user.un,
        vip: user.vip,
        level: user.level,
    ...
    if (type == 'world' || type == 'shout') {
        ...
        if (gChatLog.world.length > conf_mgr.gConfGlobal.chatLogCount) {
            gChatLog.world.pop();
        }
        ...
        if (gChatLog.legion[lid].length > conf_mgr.gConfGlobal.chatLogCount) {
        ...
        if (gChatLog.clan[teamId].length > conf_mgr.gConfGlobal.chatLogCount) {
        ...
        if (gChatLog.country[country].length > conf_mgr.gConfGlobal.chatLogCount) {
        ...
        if (gChatLog.recruit.length > conf_mgr.gConfGlobal.chatLogCount) {
        ...
        if (gChatLog.private[privateId].length > conf_mgr.gConfGlobal.chatLogCount) {
...
exports.chat = function (req, resp, onHandled) {
    ...
    do {
        ...
        if (type != 'clan') {
            if (user.level < conf_mgr.gConfGlobal.chatLevelLimit) {
        ...
        if (req.conn || req.args.type == 'shout') {
            if (!isDroid(req.uid) && gChatLog.black[req.uid]) {
                if (now - gChatLog.black[uid] >= conf_mgr.gConfGlobal.banChatDuration * 3600) {
        ...
        if (type == 'shout' || type == 'recruit') {
            ...
            requestGame(gameReq, gameResp, function () {
                do {
                    ...
                    if (type == 'world') {
                        var worldChatMaxTime = conf_mgr.gConfLevel[user.level].worldChatCount;
...
exports.notice = function (req, resp, onHandled) {
    ...
    do {
        ...
        respData.data.user = {
            uid: 10000,
            un: conf_mgr.gConfGeneralText["text_systemNotice"].text,
            vip: 0,
            level: 0,
            headpic: 1,
            headframe: 999,
