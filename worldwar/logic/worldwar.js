...
const { OneWeekTime } = require('../../common/enum.js');
...
WorldWar.prototype = {
    ...
    switchRank: function () {
        ...
        if (timestamp - this.rankUpdateTime > conf_mgr.gConfGlobal.worldwarRefreshRankTime) {
            this.sortRank();
    ...
    calcBattleScore: function (playerScore, enemyScore, succ) {
        ...
        var PD = 1 / (1 + Math.pow(10, -D / conf_mgr.gConfGlobal.worldwarScoreDiff));
        var result = conf_mgr.gConfGlobal.worldwarWaveRatio * (P - PD);
    ...
    register: function (uid, user, score) {
        ...
        if (gWorldWar.statistics[serverId] &&
            gWorldWar.statistics[serverId] >= conf_mgr.gConfGlobal.worldwarPlayerLimit) {
            return 0;
        }
        ...
        for (var key in conf_mgr.gConfExploitWall) {
            if (userInfo.worldwar.count % conf_mgr.gConfExploitWall[key][1].cycle == 0) {
    ...
    getExploitWallResetTime: function (key) {
        var cycle = conf_mgr.gConfExploitWall[key][1].cycle;
    ...
    generateRobots: function () {
        ...
        for (var id = 1; id <= 32; id++) {
            ...
            // 生成机器人
            var fightForce = conf_mgr.gConfGlobal.worldwarRobotPower;
            var posObj = generateRobot(6, conf_mgr.gConfGlobal.worldwarRobotLevel, fightForce);
            ...
            var stages = Object.keys(conf_mgr.gConfWorldWarScore);
            stages.sort(function (a, b) { return (+a) - (+b) });

            for (var i = 0, len = stages.length; i < len; i++) {
                var scoreConf = conf_mgr.gConfWorldWarScore[stages[i]];
            ...
            if (score == 0) {
                score = conf_mgr.gConfWorldWarScore[stages[0]].score;
            }
    ...
    addExploitWallTip: function (uid, type) {
        if (!conf_mgr.gConfExploitWall[type]) {
            return;
        }

        for (var i = 1; i < 100; i++) {
            var targetConf = conf_mgr.gConfExploitWall[type][i];
            if (!targetConf) {
                break;
            }
...
exports.get_fight_info = function (req, res, resp, onReqHandled) {
    var pIdx = [16, 8, 4, 2, 1].indexOf(+gWorldWar.progress);
    if (pIdx != -1 && gWorldWar.round == 2) {
        for (var i = 0, len = gWorldWar.curPlayers.length; i < len; i += 2) {
            // 计算存储record的位置pos
...
exports.get_rank_list = function (req, res, resp, onReqHandled) {
    ...
    if (req.args.last) {
        ...
        gWorldWar.sortRank();
        var rankCnt = conf_mgr.gConfGlobal.rankListLimit_worldwar;
...
exports.get_score_rank_list = function (req, res, resp, onReqHandled) {
    var uid = req.uid;
    var rankList = [];

    var sep = 3;
    var ids = [];
    var len = Object.keys(conf_mgr.gConfWorldWarGlory).length;
    for (var i = 1; i <= len; i++) {
        if (i < len - 1) {
            ids.push(conf_mgr.gConfWorldWarGlory[i + 1].rank - 1);
        }

        if (i > 3 && conf_mgr.gConfWorldWarGlory[i].rank - conf_mgr.gConfWorldWarGlory[i - 1].rank == 1) {
            if (i > sep) {
                sep = i + 1;
            }
        }
...
exports.match_enemy = function (req, res, resp, onReqHandled) {
    ...
    do {
        ...
        while (beginIndex > 0 || gWorldWar.ranks[endIndex]) {
            ...
            if (endIndex - beginIndex >= conf_mgr.gConfGlobal['worldwarMatchRange']) {
...
exports.get_replay_list = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        if (!gWorldWar.isRegisted(uid)) {
            resp.data.replays = [];
        ...
        for (var i = 0, len = reports.length; i < len; i++) {
            ...
            replays.push({
                ...
                quality: conf_mgr.gConfHero[enemyInfo.pos[1].rid].quality,
...
exports.support = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
...
exports.exploit_wall = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
...
exports.get_exploit_award = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        var userInfo = gUserInfo.getUser(uid);
        if (!userInfo) {
        ...
        var target = conf_mgr.gConfExploitWall[type][req.args.target].target;
        if (!target) {
...
exports.price_rank = function (req, res, resp, onReqHandled) {
    do {
        var list = [];
        var top32 = null;
        if (gWorldWar.progress == 'rank') {
...
exports.get_records = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        if (gWorldWar.progress == 'rank') {
...
exports.prepare = function (req, res, resp, onReqHandled) {
    var uid = +req.uid;
    do {
        // 阶段检查
        var progress = +gWorldWar.progress;
...
// 晋级赛战斗
exports.prepare_fight = function (req, res, resp, onReqHandled) {
    var uid = req.uid;
    do {
        // 阶段检查
        var progress = +gWorldWar.progress;
...
exports.worship = function (req, res, resp, onReqHandled) {
    do {
